...
4. **allocation cellの帰納的再生（hint成否ではなく最終cellが保存される）**: run 1で各unitの最終cellは (a) hint成功（captured cellが領域内かつその順番時に空き）ならcaptured cell、(b) 不成立ならsweep cell、のいずれかである。run 2では全unitのhint cellがrun 1の最終cell（＝materialized位置）となる。σ₂ = σ₁（stream制限）の順に処理し、直前のunit群が各自のrun 1最終cellを回収することを帰納法で仮定すると、各unitの順番時のoccupancyは「F' ＋ 直前unit群のrun 1最終cell」となる。unitのhint cellはunit自身のrun 1最終cellであり、形成folder cellと重複しないため（5）、かつ直前occupancyがrun 1の同じ時点と同一であるため、**hint cellはその順番時に必ず空きであり、全unitがrun 1の最終cellを回収する**。run 1でhint不成立だったunitはrun 2では新captured cell（＝run 1配置先）へのhint成功となるため、保存されるのは成否集合ではなく最終assignmentである。
...
- [ ] AC-5: idempotence / determinism / conservation / bounds / overlap / lock / profile isolationが、既存harnessと `CrossStrategyCorpusTest`（registry駆動、新strategy自動対象）に加え、**複数既存folder + 新folder形成 + 非 `1×1` movable + fragmented lock/reservation + 複数page + intent bias item群 を同一fixtureに同居させた専用counterexample fixture** の適用→recapture→replan空差分testで検証されている。fixtureは **「run 1 hint不成立→fallback→run 2 hint成功でも空差分」のcase** と **「成功hintが前方holeを残しそのholeへ形成folderが配置される」case** を直接含む（spec 237前例どおり、shared suiteが通らない状態遷移を直接踏む）。
