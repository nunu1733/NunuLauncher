...
> Status: draft — 本specの必須依存だった **#204 (Context / PersonalizedIntent exchange contract) は2026-09-15にaccepted** となり (6th review Approve、snapshot `52b9097c` 基準)、PR #322 経由で `origin/main` (`0cf82bc1e6`、本revisionのanalysis baseline) に実装込みでmerge済みである ([accepted spec 204](../../specs/204-ai-personalization-context-intent-contract/spec.md))。これに伴い、本specは #204 のschema詳細を暫定扱いから確定契約への参照へ更新した (契約核の変更はない)。D-011 (external LLM): 「privacy/threat modelとoffline behavior承認後まで導入しない」が requirements.md のdecision gateであり (FR-017がD-011対象である旨もrequirements.mdに明記済み)、本機能の実装開始は (1) ~~#204受入~~ (**達成済み** 2026-09-15)、(2) privacy/threat model承認、(3) offline behavior (local deterministic Organizerがnetwork/AIなしで利用可能なまま) の確認を満たすまで禁止される (Issue本文も同じ)。
...
|---|---|---|
| #204 Context / PersonalizedIntent contract | 唯一のAI output contract。adapterの入出力・validationはこれに従う | **accepted (2026-09-15) + 実装済み** ([accepted spec 204](../../specs/204-ai-personalization-context-intent-contract/spec.md)、PR #322 merge済み、`origin/main` = `0cf82bc1e6`)。schema/validator/codec/session store/planner接続は `organizer/personalization/` + `organizer/integration/` に存在 |
| D-011 (external LLM gate) | privacy/threat modelとoffline behaviorの承認が必要 | 未承認 (requirements.md D-011行はFR-017が対象である旨を明記済み)。実装開始のhard blocker (残存) |
