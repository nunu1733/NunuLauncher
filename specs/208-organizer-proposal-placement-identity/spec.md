...
- 2026-09-06: Implementation revision 2 (owner review @ PR #238 の反映): nested identity (FolderChild / AppPairChild) の collision 補助語は parent locator の全体 (page 表示序数 + 行・列表示序数、Dock 親は slot) を含むよう拡張し、別 page × 同 cell の同名 folder、別 parent placement × 同 stage の同名 app pair の反例を閉じた (再レビュー High 1)。AC-9 の instrumentation fixture (F8) を全 placement 種別 (icon / folder unit / folder child / widget / app pair / dock) 6 行の同名 proposal へ拡張した (再レビュー High 2)。
