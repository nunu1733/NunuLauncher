...
- [ ] **HUB-AC-03**: hubに進行中AI依頼、取り込み済み提案、最近のrun結果の表示・操作が
      存在しない（non-goalsの機械的保証としての否定的観測）。（Non-goals）
      2026-09-22改訂: 復元CTAは[D-15（accepted spec 376）](../376-durable-status-recovery-entry/spec.md)
      の所有に移行したため、このACの対象から除外された（Change history参照）。
