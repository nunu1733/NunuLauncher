...
| `application/lifecycle/RetentionPolicy.kt` | Pure retention logic: 24 h (`86_400_000 ms`) from creation, at most three non-expired points, never expire `APPLYING/COMMITTED_UNVERIFIED/RESTORING`, tombstones for 24 h, `RECOVERY_STORE_UNAVAILABLE` when three unresolved points block cleanup. | Spec §“Retention”. |
...
| `tests/unit/app/lawnchair/organizer/application/lifecycle/RetentionPolicyTest.kt` | Clock-driven 24 h / max-three / unresolved-retention / tombstone behavior (spec §“Retention”). |
...
- Retention: 24 h, max three non-expired points, never expire unresolved, tombstones 24 h, `RECOVERY_STORE_UNAVAILABLE` when three unresolved points block cleanup.
