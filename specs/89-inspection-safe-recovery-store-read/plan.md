...
| `.../protocol/RecoveryStoreReconciliationSession.kt` **(new)** | Define an opaque, private-constructor reconciliation capability bound by identity to one active `RunMutex.ReconciliationLease`; expose the existing legal reconciliation operations and full snapshot rebuild only through this scoped session. | `LayoutApplicationModule.reconcileAtStart()` is the sole production issuer. The session is closed/consumed with the lease and cannot be constructed, downcast, cached, or supplied by UI/coordinator/apply/recover/preview callers. No boolean bypass or public flag exists. |
...
| `.../store/RecoveryStore.kt` | Before every authoritative mutation, mark the fence dirty. After its established read-back trust point, build/publish a full projection and set matching valid generation. Ordinary `RecoveryStorePort` mutation entries reject while dirty/unknown; only methods invoked through the active opaque reconciliation session can perform legal reconcile/retention/rebuild work. | The store verifies the capability's private identity and active lease on every privileged operation. It retains the authoritative SQLite lifecycle; `readRecordForInspection`, `readTombstoneForInspection`, and `inspectExistingDatabase` are removed from the #84 inspection path. |
| `.../protocol/Ports.kt` | Replace the #84 inspection-only live-store read pair with one private typed `readInspectionProjection(pointId)` outcome. Keep ordinary `RecoveryStorePort` free of bypass parameters; add a separate private reconciliation-session seam only for `RestartReconciler`. | `Value`, `Unavailable`, and `Incompatible` remain typed private values; raw exceptions never escape. UI/coordinator and ordinary protocols cannot obtain the session type. |
...
| `.../protocol/RestartReconciler.kt` | Receive the already-issued private reconciliation session rather than ordinary `RecoveryStorePort` mutation authority; run all legal reconciliation/retention/rebuild work through it while the bound `RunMutex.ReconciliationLease` is active. Add a private `ReconciliationSummary.Incompatible` outcome when legal authoritative validation establishes an unsupported format. | Prevents reconciliation/preview stale race and accidental ordinary-writer healing; does not publish from preview. |
...
| `tests/unit/app/lawnchair/organizer/application/store/RecoveryInspectionSnapshot*Test.kt` **(new)** | Add codec, fence, pinned AndroidX `AtomicFile`, publication, failure, canonicality, atomicity, and restart-rebuild tests. | Assert base/`.new` publication and `.bak` inventory/read compatibility on API-independent library code; no UI/coordinator test reaches store state. |
| `tests/unit/app/lawnchair/organizer/application/protocol/RecoveryStoreReconciliationSessionTest.kt` **(new)** | Add session issuance, identity binding, lease close/consume, ordinary-port rejection, illegal reuse/forgery, and dirty recovery tests. | Proves only `LayoutApplicationModule.reconcileAtStart()` can issue the private session and only active session calls bypass ordinary dirty/unknown rejection. |
...
|---|---|---|
| F0: acquire | Apply, recover, reconciliation, and preview contend for the shared `RunMutex`. Only `LayoutApplicationModule.reconcileAtStart()` can issue `RunMutex.ReconciliationLease`, which creates one opaque reconciliation session. Preview fails `Concurrent` if a writer holds it. | No snapshot read or file change by preview; no ordinary caller can obtain a dirty/unknown bypass. |
| F1: invalidate | Before any recovery-store mutation or rebuild, set `DIRTY(nextGeneration)`. While dirty/unknown, ordinary `RecoveryStorePort` mutation entries reject before SQLite; only an active identity-bound reconciliation session may continue to F2. | A later inspection returns unavailable rather than reading an older file, and an ordinary later writer cannot accidentally heal the fault. |
| F2: authoritative mutation | Ordinary calls perform existing DB transaction/lifecycle logic only while valid. The active reconciliation session alone may perform legal unknown/dirty reconciliation/retention work. `checkpoint()` covers all intermediate transactions through `READY`; existing mutation-specific close/reopen validation remains mandatory. | If no authoritative commit occurs, clean the pending AndroidX `AtomicFile` write and keep/return fence unavailable until an active session performs a legal full rebuild. |
...
| F5: rehydrate | A new process starts `UNKNOWN`. `LayoutApplicationModule.reconcileAtStart()` first acquires `RunMutex.ReconciliationLease`, issues one private reconciliation session, and passes only that session to `RestartReconciler` for legal recovery reconciliation/retention/rebuild. It then sets valid or failed readiness. A private `ReconciliationSummary.Incompatible` sets fence incompatible and admits only the existing typed preview path; apply/recover remain stopped by authoritative availability. | The session becomes unusable when the lease closes; residual valid snapshots never bypass authoritative validation. |
...
4. **Implement the reconciliation capability and `RecoveryStore` trust points.** Add private-constructor `RunMutex.ReconciliationLease` and identity-bound `RecoveryStoreReconciliationSession`; make `LayoutApplicationModule.reconcileAtStart()` the sole issuer and pass it only to `RestartReconciler`. Mark dirty before every store mutation and reject ordinary `RecoveryStorePort` entries while dirty/unknown. Publish only after existing close/reopen/read-back validation; add equivalent validation where retention/prune/tombstone currently lack it. Treat the checkpoint's intermediate `CREATING` state as unpublishable.
5. **Integrate full startup rehydration.** Acquire the reconciliation lease, pass its scoped session to `RestartReconciler`, and use that session for legal recovery reconciliation/retention plus complete snapshot rebuild. Make readiness succeed only if the resulting fence is valid. Add a private `ReconciliationSummary.Incompatible` outcome that opens only #84's existing typed incompatible-preview path after authoritative validation; apply/recover still stop at their store-availability checks. All absent/corrupt/unreadable paths remain unavailable.
...
7. **Add production physical oracle and capability matrices.** Cover AndroidX base/`.new` plus legacy `.bak` inventory, both valid WAL sidecar states, post-commit failure, residual valid snapshot plus invalid authoritative store, snapshot codecs, busy/concurrency, reconciliation capability misuse, and API 26/API 35.
...
|---|---|---|
| IS-AC-01, IS-AC-03, IS-AC-04, IS-AC-07, IS-AC-08, IS-AC-09 | JVM fence/codec/pinned-AndroidX-AtomicFile/reconciliation-capability/public-boundary/publication tests. | `./gradlew testLawnWithQuickstepGithubDebugUnitTest --tests 'app.lawnchair.organizer.application.store.RecoveryInspectionSnapshot*'` |
...
Stop implementation and open the owning contract/ADR follow-up if the pinned AndroidX `AtomicFile` cannot provide the named same-directory base/`.new` final-file protocol on an API 26/API 35 target, if `LayoutApplicationModule` cannot make the scoped reconciliation session the sole dirty/unknown bypass, if the shared `RunMutex` cannot encompass every recovery store mutation/reconciliation path, if the fence cannot remain dirty across every uncertain result, if a required runtime/restart path needs to treat an older/residual snapshot as success, if the physical oracle detects a touched covered file, or if a public recovery/mutation value would need to expose projection internals. Do not solve any such condition by relaxing #84 RP-AC-04, hiding an unsupported device state, using URI/native SQLite without a new decision, or adding a UI/coordinator store access path.
