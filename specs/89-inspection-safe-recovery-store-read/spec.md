...
The source-level authority boundary is fixed as follows. `ApplyProtocol`, `RecoveryProtocol`, and #84 preview receive only `RunMutexPort { tryAcquire, release }`; the concrete `RunMutex` instance and startup reconciliation issuer are private to `LayoutApplicationModule` composition. `RecoveryStoreReconciliationSession` is an internal opaque capability with no public factory and is created only from an active startup-reconciliation lease issued by that concrete mutex. The contract does **not** rely on an impossible cross-file private constructor: constructor/factory visibility may be internal as required by Kotlin compilation, while every privileged store operation must validate that the session carries the active lease identity for the exact module-owned `RunMutex` instance. `RestartReconciler` receives the session only as a method-scoped `reconcileAll(session)` argument, receives no issuer/factory, and must not retain it. Closed, reused, cached, forged, or foreign-mutex sessions fail before SQLite. No boolean bypass, public flag, downcast, or ordinary protocol constructor may expose the issuer. Only this active identity-bound session may replace a dirty/unknown fence with a valid generation.
...
Startup distinguishes **pristine absence** from **suspicious absence** before any ordinary operation can run. Pristine absence is narrowly defined as: the authoritative main recovery DB does not exist, neither `-wal` nor `-shm` exists, and the dedicated inspection-snapshot directory is absent or empty. This is the normal fresh-install/never-used-recovery state. While holding the startup reconciliation issuer scope, the reconciliation session may legally initialize an empty authoritative recovery store through the existing writer-owned `RecoveryStore`/`SQLiteOpenHelper` path, validate the empty compatible store, run retention, publish an empty inspection snapshot, and establish `VALID(generation)`. DB/schema/WAL creation in this case is **startup writer work**, not inspection; #84 inspection remains forbidden from creating any recovery file. Once initialized, subsequent restarts have an authoritative DB and do not depend on remembering that the prior process was pristine.

Suspicious absence is any missing authoritative main DB accompanied by a recovery sidecar, snapshot final, `.new`, `.bak`, or another snapshot-directory artifact. It is not treated as an empty store. Startup fails closed without trusting or cleaning the residual snapshot artifacts, and preview remains unavailable. An existing zero-length main DB is likewise not classified as pristine solely because `RecoveryDbVersionGate` currently maps it to `CreateNew`; Stage B must treat it as uncertain unless the implementation can prove it is part of the same active startup initialization transaction.

At process construction the fence is `UNKNOWN`, regardless of the existence, timestamp, generation, or checksum of a final snapshot file. `LayoutApplicationModule.reconcileAtStart()` enters its private startup-reconciliation issuer scope and runs the legal authoritative classification above. For pristine absence it initializes and validates the empty store before publishing the empty snapshot. For an existing compatible store it runs the existing legal recovery reconciliation and retention work through the session, then rebuilds, validates, and publishes the complete snapshot while the same session and concrete mutex ownership remain active. It transitions the fence to `VALID` only after this succeeds. Corrupt/unreadable storage, suspicious absence, or validation/publish failure leaves readiness failed and preview unavailable. A checksum-valid residual snapshot must never produce a successful preview in those cases. If legal authoritative validation instead establishes a compatible-to-report but unsupported recovery-store format, reconciliation records private `INCOMPATIBLE` without parsing any residual snapshot and permits #84's existing typed `NotRestorable(INCOMPATIBLE_VERSION)` path; apply/recover remain protected by their ordinary authoritative store availability checks and cannot write.
...
### Pristine first start with no recovery DB

Given a process starts with no `organizer_recovery.db`, no `-wal`/`-shm`, and no snapshot artifacts,

When startup reconciliation runs before readiness,

Then the startup reconciliation session may create and validate an empty authoritative recovery store, publish a matching empty snapshot, and establish `VALID`,

And the first ordinary `apply`/checkpoint remains legal without requiring a second reconciliation cycle.
...
Then it does not set `VALID` from that residual file; absent-with-residual is suspicious and corrupt/unreadable storage returns typed unavailable, while verified authoritative incompatibility returns typed incompatible-version,
...
| IS-AC-03a | `CheckpointResult.PointIdCollision` is a proven-no-commit outcome based on no committed authoritative logical-state change: retrying with a newly generated point ID in the existing apply run remains legal and can reach `Ready`; it must not require reconciliation or expose a stale successful preview. |
...
| IS-AC-10a | A true pristine start—main DB absent, sidecars absent, and snapshot directory absent/empty—legally initializes and validates an empty authoritative recovery store during startup reconciliation, publishes an empty matching snapshot, reaches `VALID`, and allows the first ordinary checkpoint. Missing DB plus any residual sidecar/snapshot artifact is suspicious and remains unavailable. |
...
| Proven-no-commit tests | Point-ID collision rolls back all committed logical transaction work, restores the previous valid generation without publishing, permits the same apply run to retry a new point ID, and reaches `Ready`; committed/ambiguous/read-back/publish failures remain dirty. No byte-for-byte SQLite-file equality is required for the writer rollback itself. |
| Startup rehydration tests | Pristine no-DB/no-sidecar/no-snapshot start initializes and validates an empty authoritative store, publishes an empty snapshot, reaches `VALID`, and permits first checkpoint; missing main DB with residual sidecar/snapshot artifact fails closed without trusting or cleaning it; existing zero-length main DB is uncertain unless proven part of the active initialization. |
| Reconciliation capability tests | Ordinary `RecoveryStorePort` mutations reject while dirty/unknown; an active identity-bound `RecoveryStoreReconciliationSession` alone can reconcile, retain, rebuild, and publish; closed, reused, forged, cached, foreign-mutex, or reflection-created capability attempts fail before SQLite. Source-boundary compilation tests prove ordinary protocols depend on `RunMutexPort` and cannot invoke the module-only issuer. |
...
| Production Android instrumentation | pristine startup initialization followed by first checkpoint; production-created WAL store with sidecars present; valid closed store with sidecars absent; residual valid snapshot with authoritative DB absent/corrupt/incompatible; invalid/unpublished snapshot; `.new`/`.bak`/unexpected-entry no-cleanup; concurrent writer/publication; exact physical oracle on all inspection cases. |
| Upgrade/downgrade/backup tests | no-backup placement, restore with no recovery artifacts initializes an empty store, restore/residual mismatch fails closed, snapshot rebuild from compatible recovery store, snapshot-codec incompatibility unavailable, authoritative incompatibility result, and rollback without Launcher-layout mutation. |
...
- pristine no-store startup cannot be initialized and validated under the startup writer/reconciliation authority without exposing DB creation to inspection, or missing-store residual artifacts cannot be distinguished and failed closed;
- the module cannot enforce reconciliation-session authority by active exact-`RunMutex` lease identity while exposing only `RunMutexPort` to ordinary protocols;
