...
本 plan は `main` `3076bdae7e`（2026-09-19 re-entry 時点の `origin/main`、PR #364 merge）を確認基準とした。初回作成時の確認基準は `0cf82bc1e61c1874b280a7120dff9594be4fef71`、前回 re-entry は `aab0d293d1a98bf59f5b164693f54ee1a63e3f0b`、`703afe3f4c1f5387f768832ea422c7b681c3775a`。確認済みの事実と実装への含意は次のとおり。
...
| #348/#327/#328/#337 の実装 merge により exchange 系 UI copy がさらに追加・改訂され、`exchange_import_accepted` は削除された。 | [spec 348](../../specs/348-exchange-ai-facing-contract/spec.md)（PR #349、merge `8fd05a40d5`）/[spec 327](../../specs/327-agent-exchange-interview-first/spec.md) は implemented（PR #350、merge `a9ec3c2cf9`）/[spec 328](../../specs/328-exchange-import-success-state/spec.md)・[spec 337](../../specs/337-exchange-category-group-proposals/spec.md) の実装は main に merge 済み（PR #353 merge `34ba8ff447`、PR #355 merge `b728ed4d9f`）。`exchange_*` は両言語 114 entry（`lawnchair/res` root のみ。name set は両言語で一致、2026-09-19 検証）。#327 で capability 説明 7 string（`exchange_capability_*`）、transport success / framing hint の interview-first 文言改訂（marker lines → ```json code block）、`exchange_privacy_labels` の「app and category names」化、disclosure 2 string のカテゴリ言及化、`exchange_import_accepted` 削除。#328 で取り込み成功状態・破棄確認・summary plurals・strategy busy / frozen 案内を追加。#337 で `exchange_failure_unknown_category_ref` を追加。`ExchangeFlowUi.kt` は `stringResource` 経由のみ（85 箇所、hardcoded UI text なし）。`ExchangeCapabilityCopyTest` が capability copy の構造と両 locale key 存在を pin する。CONTEXT.md に取り込み成功状態・取り込み破棄・判断なし項目・整理方針確認・exportカテゴリ参照・既存カテゴリ参照・提案グループ・昇格 の用語が追加された。`ja-glossary.tsv` は 22 term のまま exchange / custom category / interview-first 用語を含まない。 | inventory は 114 entry の新しい exchange 集合を対象に full pass 開始時に再生成する。capability 説明・transport success・framing hint・取り込み成功状態・破棄確認・summary plurals・カテゴリ参照失敗は safety class（warning / failure / recovery / stale）として semantic guard 対象かつ evaluation set（D3）候補。`exchange_import_accepted` 削除のように copy が削除・統合される case も inventory の disposition で追跡する。copy 改訂が `ExchangeCapabilityCopyTest` 等 pin 済み test の対象に触れる場合は同一変更 set で test を整合させる。未登録用語は glossary 追加提案として扱う（spec Domain language）。 |
| #361 の TO-BE UX decision が accepted となり、#356 の AS-IS audit が存在する。#362 が既存正本の処分と migration 順序を所有する。 | `docs/product/organizer-to-be-ux.md`（Status: accepted、owner review 2026-09-18〜2026-09-19、PR #364、merge `3076bdae7e`）。`docs/assessment/organizer-as-is-ux-data-flow-audit.md`（Status: draft、audited HEAD `b728ed4d9f`、PR #363、merge `ce848f3486`）。 | TO-BE decision は優先順位 tier 1 の accepted product decision 文書であり、用語・語彙の統一方針（Back / 破棄語彙の統一、AS-IS audit §10 の vocabulary 不整合観察を含む）を behavior context として読む。ただし TO-BE の UI 反映は #362 以降が所有するため、re-audit は base SHA 時点の現行 surface を対象とし、migration を先取りしない。実装開始時に #362 の進行状況を確認し、TO-BE 由来の surface 変更が入った場合は base SHA + rebase 差分 re-review 規則で扱う。AS-IS audit は inventory の surface mapping と隣接 copy 特定の参考入力に使える（fact base は draft であり正本ではない）。 |
| #161 resource oracle が現行 main で失敗しており、finding は 15 件で不変。 | `python tools/localization/verify_nunu_ja_resources.py --baseline 505dbc40e6154c05158b5d0271c45f6a885a411b` が 15 件の finding で FAIL、self-test の `test_real_repository_passes` も失敗（2026-09-19 再実行、worktree tree = merge `5c1591bbcb` ≒ main `3076bdae7e`。前回 2026-09-17 実行と同一結果）。内訳: (1) `manual_organization_*` 12 件（不変、placeholder/plural 契約）、(2) `manual_organization_rejection_invalid_category_provenance` の missing Japanese 1 件（不変。#336 が `lawnchair/res/values/strings.xml` の default にのみ追加。ja 側不変。`ManualOrganizationPreferences.kt` の rejection code 表示で使用）、(3) `organizer_custom_category_entry_count` / `organizer_custom_category_delete_text` の only-other plural 契約 2 件（不変。ja の正しい複数形 class と oracle の厳格等価比較の食い違いで、(1) と同クラス）。#327/#328/#337 の新規 exchange copy は required set 対象のため finding に寄与していない。`NUNU_PREFIXES`（`organizer_`、`manual_organization_`、`organization_onboarding_`）に `exchange_` が含まれず exchange strings は required set 対象外。一方 custom category strings は `organizer_` prefix で required set に含まれており、oracle が実際に (3) を検出している。oracle は `.github/workflows/` から参照されていないため回帰が未検知。 | AC-324-12 は「新規 finding ゼロ + 既存 15 finding の明示的 disposition + required set の post-#161 prefix 被覆」を条件とする。required set 拡張が要るのは `exchange_` のみ（custom category は既被覆）。(2) は locale 差ではなく訳出欠落の新クラスであり、D6 で resource 修正 / split の owner 判断を要する。oracle の CI 組込みは本 Issue の初版 scope 外とし、必要なら split Issue とする。 |
| CI の merge gate 構成が変化している。 | 2026-09-19 時点の `.github/workflows/ci.yml` は #332 の `ExchangeImportSurfaceInstrumentationTest` を実行する独立 job `organizer-instrumentation-issue332-tests`（line 663）を維持し、さらに issue52 lane（line 463）が #328 の `ExchangeImportSuccessInstrumentationTest` と `StrategyPickerFreezeInstrumentationTest` を追加実行する（localization oracle の組込みは still なし）。 | AC-324-13 の「既存 test の維持」判定対象に #332 instrumentation lane と issue52 lane 内の #328 instrumentation test が加わった。oracle の CI 非組込みは D6 の背景事実として維持。 |
...
2. **Model identifier 確認と bake-off。** 下記の手順で実行 runtime の候補を確認し、fixed evaluation set（#161 bake-off context 24件を種に、#161 以降の新 surface — exchange の送信前確認・置換確認・typed failure 分類に加え、#327 の capability 説明 / transport success / framing hint、#328 の取り込み成功状態・破棄確認・summary plurals、#337 のカテゴリ参照失敗・disclosure copy を含む — から 8 分類を横断して 20〜40件に再構成）で比較する。本計画の作成時点では hosted LLM 評価を実行していない。実装は spec 承認後に行う。
3. **Inventory 再生成。** full pass 開始時の base SHA を記録し、`verify_nunu_ja_resources.py` と同等の required set 定義（active かつ user-visible かつ `translatable != false`、両 root）で対象を確定する。required set は #161 監査後に追加された Nunu 固有 prefix（少なくとも `exchange_`。custom category は既存 `organizer_` prefix で被覆済み）を含むように構成し、oracle 側の被覆拡張（`NUNU_PREFIXES` 追加または同等の再構成）を反映 PR と同じ変更 set で行う（spec D6）。実装開始時に [#362](https://github.com/nunu1733/NunuLauncher/issues/362)（TO-BE 正本の処分・migration）の進行状況を確認し、migration 由来の surface 変更が進行中の場合も base SHA 時点の tree で review し、反映時に rebase 差分を再 review する。
...
| AC-324-13 | 代表画面 capture（normal + enlarged font scale）、a11y 確認、build / test。 | #123 evidence の emulator 手順（per-app locale、font scale 2.0、screencap / uiautomator）、`./gradlew spotlessCheck`、`./gradlew testLawnWithQuickstepGithubDebugUnitTest --tests 'app.lawnchair.organizer.*'`（`ExchangeCapabilityCopyTest` 等 copy 構造を pin する test を含む）、`./gradlew assembleLawnWithQuickstepGithubDebug`、PR `CI / final-status`（#332 の `organizer-instrumentation-issue332-tests` lane、issue52 lane 内の #328 instrumentation test を含む）。 |
...
- [ ] #362（TO-BE 正本の処分・migration）の進行状況を確認し、full pass の base SHA 選定と re-review 規則の適用可否を記録する。
