...
[#161][2] で日本語 UI LQA の style guide / glossary / 再利用可能な workflow が整備され、初回監査（223 unit、2026-08-28時点）まで完了している。しかし正本は `docs/localization/` の review contract であり、**言語表現を専任で見る subagent を通常の開発フローに組み込み、実装 worker / functional reviewer とは異なる model family に継続的に担当させる構造**にはなっていない。また #161 以降、strategy selection / preview、missing-app selection、restore 確認、re-entry hint、backup preview、外部エージェント交換（[#205][5]: entry / privacy / 送信前確認 / session 置換確認 / generate / transport / import と typed failure 分類、[#331][6]: scoped entry / 選択freeze / `SCOPE_MISMATCH`、[#329][7] + [#332][8]: clipboard / file import の入力 UI、認識形式表示、raw detail、normalization 失敗 copy、[#330][9] の意味変更に追従した `exchange_failure_incomplete_coverage` の文言改訂、[#327][11]: interview-first の capability 説明・会話 flow 案内・framing hint の文言改訂と `exchange_import_accepted` の削除、[#328][12]: 取り込み成功状態・破棄確認・summary plurals・strategy busy / frozen 案内、[#337][13]: カテゴリ参照失敗 copy と privacy / disclosure copy のカテゴリ言及改訂）、custom category の authoring / assignment（[#336][10]: 作成・rename・削除・削除の部分状態・capacity / conflict 等 28 entry + plurals 2件、root `res` 両言語）等の新しい user-visible surface が追加されており、初回監査時に存在しなかった文言が蓄積している（`exchange_*` は両言語 114 entry、2026-09-19 時点）。この間の copy 追加は専任の language review を経ておらず、実際に #161 が checked-in した resource oracle（`tools/localization/verify_nunu_ja_resources.py`）は、監査完了後の PR で追加された strings に対して 2026-09-19 時点でも 15 件の finding（#161 監査後の PR 由来の `manual_organization_*` 12 件の placeholder/plural 契約、[#336][10] 由来の日本語 resource 未提供 1 件、同 only-other plural 契約 2 件）を出し、oracle と self-test が現行 main で失敗している（oracle は CI に組み込まれていないため回帰は検知されず、#327/#328/#337 の新規 exchange copy は required set 対象外のため新たな finding を出していない。詳細は plan current evidence）。[#199][3] の Vision UX review は見た目・第一印象・affordance を独立評価するが、product copy の編集・言語監修を主責務としない。
...
再監査は #161 監査後に追加された surface（外部エージェント交換、import 入力 UI、custom category authoring、interview-first の capability 説明、取り込み成功状態を含む）も対象にするが、その user-facing 用語の一部は `docs/localization/ja-glossary.tsv` にまだ登録されていない（2026-09-19 時点で glossary は 22 term であり、exchange 系・ユーザー定義カテゴリ系・interview-first 系の用語を含まない）。一方で CONTEXT.md は exchange 系 domain concept（持ち帰りIntent取り込み、インポート正規化、交換フレーミング、取り込み成功状態、取り込み破棄、判断なし項目、整理方針確認等）と custom category 系 domain concept（カテゴリidentity、ユーザー定義カテゴリ、アクティブカテゴリカタログ、exportカテゴリ参照、提案グループ、昇格等）の日本語名を既に定義している。reviewer は UI 文言の用語を CONTEXT.md の domain 語彙との整合で評価し、既存 glossary entry・CONTEXT.md のいずれにも対応づけできない user-facing 用語は、glossary 追加提案（terminology decision）として扱う。accepted terminology の追加・変更は `ja-glossary.tsv`（#161 正本）に記録し、Skill や adapter へ複製しない。
...
tier 1 には accepted な product decision 文書も含まれる。2026-09-19 時点で `docs/product/organizer-to-be-ux.md`（[#361][14]、accepted）が Organizer の TO-BE 情報設計・遷移・data timing の長期正本であり、既存正本の処分と migration 順序は [#362][15] が所有する。reviewer は TO-BE decision（用語・語彙の統一方針を含む）を behavior context として読み、それと矛盾する提案をしない。一方、TO-BE の UI への反映は #362 以降の migration が所有するため、re-audit は base SHA 時点の現行 surface を対象とし、migration を先取りした surface の作り替え提案を language review の名目で行わない。
...
Then string name、placeholder 数・型・順序、plural、escaping、`translatable` 契約が維持され、既存の被覆・placeholder oracle が新規 finding なく pass する（2026-09-19 時点で既存の 15 finding は AC-324-12 の disposition が完了していることを前提とする。copy 改訂が copy 構造を pin する既存 test（例: `ExchangeCapabilityCopyTest`）の対象 string に触れる場合は、当該 test も同一変更 set で整合させる）
...
- [ ] **AC-324-12 — resource 反映と oracle:** accepted `REVISE` items が resource に反映され、name、placeholder/plural/translatable semantics が維持され、`tools/localization/verify_nunu_ja_resources.py` が新規 finding を出さない。あわせて、(a) oracle の required set が再監査対象の post-#161 Nunu 固有 prefix を被覆する（`organizer_custom_category_*` は既存 `organizer_` prefix で被覆済み。追加を要するのは少なくとも `exchange_`）、(b) 2026-09-19 時点で main に既存する 15 件の oracle finding（`manual_organization_*` の placeholder/plural 契約 12 件、[#336][10] 由来の日本語 resource 未提供 1 件、only-other plural 契約 2 件）が、accepted revision による解決・oracle 契約の修正・split Issue のいずれかに明示的に disposition され、disposition 完了後は oracle と self-test が pass する。
...
| AC-324-12 | `python3 tools/localization/verify_nunu_ja_resources.py`（baseline `505dbc40e6154c05158b5d0271c45f6a885a411b`）と self-test の実行結果（既存 15 finding の disposition 前後）、required set の post-#161 prefix 被覆、resource diff review。 |
...
- **D3: evaluation set の具体構成。** 件数と軸（本 spec AC-324-04）は固定したが、採用する具体 unit は full pass 開始時の inventory から #161 bake-off context を含めて選定する。#332 の import 入力 UI（clipboard / file の failure copy、認識形式表示）、#336 の custom category（作成・削除の部分状態、capacity / conflict）、#327 の capability 説明・transport success・framing hint、#328 の取り込み成功状態・破棄確認・summary plurals、#337 のカテゴリ参照失敗・privacy/disclosure copy など、#161 以降に追加された surface も候補集合に含める。
...
- **D6: 既存 oracle finding と required set 被覆の扱い。** 2026-09-19 時点の main でも `verify_nunu_ja_resources.py` は 15 件の finding を出し（前回 snapshot から不変。#327/#328/#337 の新規 exchange copy は required set 対象外のため finding に寄与しない）、self-test の real-repository assertion が失敗している。(i) `manual_organization_*` 12 件の placeholder/plural 契約 finding（#161 監査完了後の PR #195/#228 等に由来）、(ii) [#336][10] 由来の `manual_organization_rejection_invalid_category_provenance` の日本語 resource 未提供 1 件（`lawnchair/res` の default のみに存在し、`ManualOrganizationPreferences` の rejection code 表示で使用される。locale 差ではなく訳出欠落であり、resource 修正が素直な解と考えられるが、本 spec は実行しない）、(iii) [#336][10] 由来の `organizer_custom_category_entry_count` / `organizer_custom_category_delete_text` の only-other plural 契約 2 件（(i) と同じ、ja の正しい複数形 class と oracle の厳格等価比較の食い違いクラス）。これらを (a) #324 の accepted revision で resource 側を解決する、(b) locale 別 plural category・positional order の正当な差異として oracle 契約側を修正する、(c) 別 maintenance Issue に分離する、のいずれで解くか（(ii) については即時 resource 修正の split も含む）、および required set の `exchange_` prefix 拡張の実装方法（`NUNU_PREFIXES` への追加か同等の再構成か）は、実装開始時に owner が判断する。
...
- 2026-09-17 (re-entry): baseline を `703afe3f4c1f5387f768832ea422c7b681c3775a` へ再錨定。#329/#330/#332/#336 の実装 merge（PR #338/#340/#341/#344）を反映: exchange entry の 63 → 80 増加（import 入力 UI 15 string + normalization failure 2 string）、`exchange_failure_incomplete_coverage` の意味変更に伴う文言改訂、root `res` への custom category 28 entry（両言語）追加、CONTEXT.md の インポート正規化・カテゴリidentity・ユーザー定義カテゴリ・アクティブカテゴリカタログ 用語追加。oracle finding の 12 → 15 増加（#336 由来の日本語未提供 1 件と only-other plural 契約 2 件）を Problem・AC-324-12・D6 へ反映し、custom category が既存 `organizer_` prefix で required set に含まれることを AC-324-12 (a) に明記。D3 へ evaluation set 候補として新 surface を追記。
- 2026-09-19 (re-entry): baseline を `3076bdae7e` へ再錨定。#348（PR #349）/ #327（PR #350）/ #328（PR #353）/ #337（PR #355）の実装 merge を反映: exchange entry の 80 → 114 増加（両言語、name set 一致）、#327 の capability 説明 7 string・transport success / framing hint の文言改訂・`exchange_import_accepted` の削除、#328 の取り込み成功状態・破棄・summary・strategy busy / frozen 案内、#337 の `exchange_failure_unknown_category_ref` と privacy/disclosure copy のカテゴリ言及改訂。CONTEXT.md の 取り込み成功状態・取り込み破棄・判断なし項目・整理方針確認・exportカテゴリ参照・既存カテゴリ参照・提案グループ・昇格 用語追加（glossary は 22 term のまま）。#361 の TO-BE UX decision（accepted、PR #364）と #356 の AS-IS audit（draft、PR #363）を取り込み、優先順位 tier 1 へ accepted product decision 文書を明示し、#362 migration を先取りしない旨を追加。oracle finding は 15 件のまま不変（新規 exchange copy は required set 対象外）を 2026-09-19 実行で再確認。AC-324-09 / D3 へ新 surface を反映し、copy 構造を pin する test（`ExchangeCapabilityCopyTest` 等）の整合を scenario へ追記。
...
[7]: https://github.com/nunu1733/NunuLauncher/blob/main/specs/329-import-normalizer/spec.md "Issue #329 specification (implemented)"
[8]: https://github.com/nunu1733/NunuLauncher/blob/main/specs/332-exchange-import-input-ui/spec.md "Issue #332 specification (implemented)"
[9]: https://github.com/nunu1733/NunuLauncher/blob/main/specs/330-partial-intent-authoring/spec.md "Issue #330 specification (implemented)"
[10]: https://github.com/nunu1733/NunuLauncher/blob/main/specs/336-user-defined-categories/spec.md "Issue #336 specification (implemented)"
[11]: https://github.com/nunu1733/NunuLauncher/blob/main/specs/327-agent-exchange-interview-first/spec.md "Issue #327 specification (implemented)"
[12]: https://github.com/nunu1733/NunuLauncher/blob/main/specs/328-exchange-import-success-state/spec.md "Issue #328 specification"
[13]: https://github.com/nunu1733/NunuLauncher/blob/main/specs/337-exchange-category-group-proposals/spec.md "Issue #337 specification"
[14]: https://github.com/nunu1733/NunuLauncher/blob/main/docs/product/organizer-to-be-ux.md "Organizer TO-BE UX decision (#361, accepted)"
[15]: https://github.com/nunu1733/NunuLauncher/issues/362 "Issue #362: 既存正本の処分とmigration順序"
