...
> 前提spec draft: [Issue #374][2]（durable取り込み済み提案。branch `issue-374-spec-plan`、
> commit `4b28b45543`）と[Issue #369][3]（T-08/T-13表示統合・canonical順序。branch
> `issue-369-spec-plan`、commit `3c39ceb2f8`）。本specの執筆はこれらのdraftとの整合で
> 可能である（現時点でそうしている）。実装着手はdisposition §8依存graph
> （`#369 → … → #374 → #375`）と「正本を先に」（#365 merge後）に従う。

## Problem

現行（baseline `a2b6aba318`、spec 331 implemented `addb25d8181e`）では、scope binding gate
...
4. **process死後の継続経路がない**: run・選択はprocess-local（spec 228 D-1、TO-BE §8.2）で
   あり、process死後の回復は「同じ回答textの再取り込み」のみである。[Issue #374][2]が
   取り込み済み提案をdurable化しstatus card経由の再開面（`Hub → ImportReview`）までを所有するが、
   **再開面に継続CTAは存在しない**（#374 Contract notes 2が継続/rebindを本Issueへ委譲）。
   process死後も取り込み済み提案が残るのに続行できない状態が、#374適用後も残る。
...
- **再開面CTAの有効化**: #374のImportReview再開面に「この提案で続ける」CTAを追加する
  （語彙はTO-BE T-18 / spec 328 rev.2 D-3の統一copy）。reconcile通過済み（#374契約）の提案に
  のみ表示する。CTAはfresh run admission（既存`start(intent)` seam。RUN lease・単一active run
  gate・Busy時typed拒否を含む）へ接続し、成功時にrun面へ遷移する。**CTAの成功は提案を消費
  （削除）しない**（提案の消失系は#374契約どおり破棄・期限切れ・置換のみ）。
- **継続時のfail-closed再検証**: 再開CTAによる継続（rebind）では、binding前に依頼sessionの
  `sourceContextDigest` と現行構造digestの等価検証（import時 `CONTEXT_STALE` と同一意味論）を
  再適用する。不一致はtyped失敗（依頼の作り直しへ案内）であり、提案は削除されない
  （Contract notes 1）。
...
- **#374 storeのschema拡張**: 復元材料は依頼session側の正本
  （`ExportSession.scopeCandidates`）から導出でき、recordへの新fieldは不要なことをplanで確認済み
  （#374が既に保存する`entryKind`でidle/run-in由来の判定が足りる）。
...
`CONTEXT.md` への追加用語案（受入時に反映。正本改訂は#365が所有するため、本specでは案の記載に
留める）。
...
経て提案のpreferenceを新しいrunへ結合すること。同一process内の生存runへのattachとは区別される。
...
- 完全一致gateの評価はconfirm時（選択確定点）とcomposition後（投影digest）の既存2点で行われ、
  ...
  本Issueは検証点を増やさない（継続時構造digest再検証は「import時検証の再適用」であり、
  gateの新設ではない）。
- 復元初期値の材料（依頼scope候補identity）と再開面の表示は、#374の読取時reconcile
  （`pending.exportId == 現行active sessionのexportId`・TTL・破棄mark）通過後の値のみを使う。
  CTA押下からfresh run検出までの間にsessionが失効した場合は、gate・reconcileの既存fail-closed
  経路で捕捉される（検証の二重化はしない）。
- 単一active run gate（`start()`の`Busy`）がrebindと通常開始・attachの競合を構造的に排他する。
  CTAのsingle-flightはUI層の規律であり、構造的排他はrun gateが担保する（spec 328の
  「UI disabledはaffordanceにすぎない」原則の継承）。

## Data and state

- **読むdata**: #374のdurable record（`exportId`・`entryKind`。継続可否とidle/run-in由来判定）。
  現行active session（`ExportSession.scopeCandidates` = 依頼scope候補identityの正本、
  `sourceContextDigest`・`scopeCandidateDigest`、`itemRefs` / `categoryRefs`、失効時刻）。
  検出cut（identity + availability）。現行構造digest。既存clock注入に従う。
...
- [ ] **SR-AC-03**: process死後に取り込み済み提案（run-in由来）から再開面CTAでfresh runを開始し、
      ...
      idle由来には復元がなくunchecked初期値・件数案内であること。（Issue受入3）
...
| SR-AC-03 | instrumentation: 冷起動 → status card行 → 再開面 → CTA → 検出 → 復元初期値の選択面 → confirm → 確認面到達の統合test＋冷起動emulator evidence。unit: 復元導出の純粋関数（依頼scope ∩ 検出cut、決定性）。idle由来の非復元test |
...
| SR-AC-07 | holder/instrumentation test（reconcile不通でCTA非表示、single-flight、Busy拒否、成功後のstatus card行残存・再継続可） |
| SR-AC-08 | unit: 構造digest再検証の純粋判定（一致/不一致/session不在）+ instrumentation（CTA押下でtyped失敗・run不在の否定的観測） |
...
3. **#374 storeのschema拡張は不要**: 復元材料は依頼sessionの`scopeCandidates`から導出でき
   ...
   `entryKind`で足りる。本Issueでrecord schemaを変更しない。
...
2. 継続不能・構造変化・差分強調の最終copy（ja/en）は実装PRのstring diffで確定する
   （語彙規約への従属のみ拘束）。
3. #369のT-13統合失敗面・#373の手段別再投影との文言供給の接続順序は、各Issueの実装順
   （disposition §8）に従う。本Issueのcause別文言は先行実装環境では現行面へ、後続環境では
   統合面へ表示される（表示面の抽象は実装PRで確定）。
...
- [Spec 328: import success state](../328-exchange-import-success-state/spec.md)（accepted。CTA single-flight / D-2破棄語彙の正本。revision 2は#374が所有）
...
- 先行spec draft: [Issue #374][2]（durable提案・再開面・entry種別）、[Issue #369][3]（T-08/T-13統合・canonical順序）、
  Issue #373（T-18表示・手段別再投影。branch `issue-373-spec-plan`）、Issue #366（hub/status card。branch `issue-366-spec-plan`）
