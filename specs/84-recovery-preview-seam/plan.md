...
| `.../protocol/Ports.kt` | Implemented #89 `RecoveryStorePort.readInspectionProjection(pointId)` exposes only `Value`, `Unavailable`, or `Incompatible` from a private projection; `LayoutWriterPort` supports non-blocking lease acquisition and authoritative capture. | Preview must consume only that projection while holding `RunMutexPort`; it must not reach a live record/tombstone reader, an issuer/session capability, or a helper path. Acquiring the existing lease remains capture-only. |
| `.../store/RecoveryStore.kt` | Implemented #89 writer publishes a non-authoritative projection only after authoritative validation and maintains an in-process `UNKNOWN`/`DIRTY`/`VALID(generation)`/`INCOMPATIBLE` fence. Inspection reads only an exact final app-private snapshot through direct `FileInputStream`; startup reconciliation owns any authoritative SQLite work. | #84 adds no recovery DB schema or record format change. Inspection must never probe/open the recovery DB or its WAL sidecars. Unknown/dirty/invalid/companion inventory state maps to typed `Unavailable`; trusted authoritative incompatibility maps to the existing typed incompatible result. |
...
| `lawnchair/src/app/lawnchair/organizer/application/protocol/Ports.kt` | Consume the implemented typed `readInspectionProjection(pointId)` outcome. | The internal port remains hidden from UI and exposes `Value`/`Unavailable`/`Incompatible` only; it leaks neither DB exceptions nor reconciliation authority. Existing writer/store write methods are unchanged. |
| `lawnchair/src/app/lawnchair/organizer/application/store/RecoveryStore.kt` | Reuse the implemented #89 fenced snapshot/direct-reader path; do not add a preview-specific SQLite reader. | Production adapter evidence from #89 proves every inspection outcome avoids DB/schema/WAL/sidecar creation, tombstone retention changes, and record lifecycle mutation. No schema/format change. |
...
| `tests/unit/app/lawnchair/organizer/application/adapter/FakeRecoveryStore.kt` | Implement typed inspection-projection fixtures and counters for every mutating method. | Allows tests to prove no checkpoint, lifecycle advance, restore mark, prune, retention, live-store read, or projection publication is reached by preview. |
| `tests/organizer-instrumentation/app/lawnchair/organizer/application/store/RecoveryStoreInspectionInstrumentationTest.kt` | Reuse #89 production-adapter physical oracle. | Verifies fenced direct-reader inspection leaves production WAL sidecars-present/absent, invalid/residual authority, and snapshot inventory physically unchanged without `SQLiteOpenHelper` repair or write. |
...
3. `RecoveryPreviewProtocol` attempts `RunMutex`. Failure returns `Concurrent` immediately. On success, it calls only `readInspectionProjection(pointId)`: a matching valid projection supplies one record/tombstone/missing classification; authoritative incompatibility is typed; all unknown, dirty, malformed, companion, generation, or I/O uncertainty is `Unavailable(RECOVERY_STORE_UNAVAILABLE)` without any live SQLite probe/open/query.
...
| Reuse any live record/tombstone SQLite read for inspection | A live WAL reader cannot establish #84’s physical no-write contract. Preview consumes only the fenced #89 projection; confirmation-time recovery remains authoritative. |
...
| Recovery preflight/store port/adapter | Consume #89’s typed inspection projection and retain existing authoritative retention re-evaluation only at confirmation. | Makes physical no-write inspection and final expiry enforcement mechanically testable without a live SQLite preview read. |
...
| #89 organizer instrumentation evidence | Reuse the merged API 26/API 35 physical no-write and publication-failure matrix. | Validates the production snapshot/direct-reader and lease boundary without a preview-specific SQLite harness. |

## Migration and recovery

No data migration is required. The recovery DB schema, `FORMAT_VERSION`, record codec, retention constants, Launcher schema, permissions, backup allowlists, and recovery lifecycle remain unchanged. #84 relies on #89’s already merged derived inspection snapshot and performs no live-store SQLite read; the only recovery-protocol change remains enforcement of the existing pure retention rule before its mutation boundary.
...
| RP-AC-04 | Fake port counters prove every I0–I5 row has zero writes, lifecycle changes, prune/retention calls, reloads, and diagnostics. #89 production instrumentation proves the complete snapshot/direct-reader path leaves production WAL sidecars-present and sidecars-absent stores, invalid/residual storage, and snapshot companions physically unchanged on API 26/API 35. | Focused JVM tests plus #89’s API 26/API 35 `RecoveryStoreInspectionInstrumentationTest` and `RecoveryInspectionSnapshotPublicationInstrumentationTest` evidence recorded in PR #90/audit. |
