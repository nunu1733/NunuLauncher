...
| I2 | Probe recovery-store availability and use typed no-create inspection reads for only the requested record/tombstone. A missing recovery DB is absent without opening `SQLiteOpenHelper`; an existing compatible DB is queried only through a fresh explicit `OPEN_READONLY` handle. No inspection lookup may lazy-purge. | `NotRestorable(INCOMPATIBLE_VERSION)` for an incompatible store; `Unavailable(RECOVERY_STORE_UNAVAILABLE)` for version/open/query failure; exact missing/tombstone result otherwise. | None, including no DB/schema/WAL creation. |
