...
Then `LayoutApplicationModule` consumes the private-registry binding for the one-shot token capability and uses the same private application-level recovery behavior as public `recover(RecoveryRequest)`, without exposing the locally constructed request,
...
Inspection reads only the recovery store’s typed #89 inspection projection, `Clock`, pure retention policy, and one authoritative current capture. The projection is available only from the matching in-process valid fence and exact final app-private snapshot; inspection never opens, probes, creates, repairs, or queries the authoritative recovery DB, its WAL sidecars, or `SQLiteOpenHelper`. Recovery manifests, digests, and raw rows remain protocol-private inputs to the existing writer/recovery implementation and are never projected into the public result. `RecoveryPreviewConfirmation` is a private-constructor, in-memory, opaque, non-serializable random token with no point/revision/request field or accessor. A private module registry binds that token by identity to `{pointId, revision}` and removes the entry before one confirmation attempt; an unregistered or reused token cannot enter recovery. It creates no recovery-store row, no retention entry, no schema/format version, snapshot publication, or backup/restore artifact.

There is no Launcher schema migration, recovery DB migration, retention-policy change, rollback action, or backup behavior change. Inspection uses #89’s SQLite-free projection, while existing confirmation-time recovery preflight continues to enforce the accepted retention boundary against authoritative state. A failed, expired, stale, or busy preview leaves both databases, model state, lifecycle, snapshot artifacts, and diagnostics journal unchanged.
...
| RP-AC-04 | Inspection performs zero recovery-store/lifecycle writes, retention/prune operations, DB/schema/WAL creation, layout writes, model reloads, recovery authorizations, and diagnostic emissions on every result path. |
| RP-AC-05 | Public/UI-facing values cannot access `RecoveryStorePort`, `StoredRecord`, manifest, DB row, payload, digest, raw `RevisionId`, item/profile identity, or a mutable writer/lease capability. Confirmation is private-constructor/token-only, binds point/revision only in a private one-shot registry, cannot be reconstructed into a valid recovery request, and delegation never returns `RecoveryRequest`. |
...
- [Issue #89: inspection-safe recovery-store read](https://github.com/nunu1733/NunuLauncher/issues/89)
