...
`docs/assessment/issue-442-android16-17-fitness-research.md` が正本として存在し、(1) upstream同期対象の有無の確定、(2) 16-dev差分の概観（rebaseコスト評価の入力）、(3) 既知問題（#304/#418）のfailure signature単位での影響範囲の切り分け、(4) 適性チェックリストのagent実行分（API 36 emulator）の結果、を証跡（対象commit、出典link、確認日）つきで記録する。その上で、選択肢A/B/Cの判断基準（[Issue #442](https://github.com/nunu1733/NunuLauncher/issues/442)）の適用表と暫定結論を記録する。結論がC（Lawnchair 16へのrebaseを専用Epic+ADRで計画）方向の場合、専用Epicの起票範囲とADR起草要件（[upstream-strategy.md](../../docs/engineering/upstream-strategy.md) §Upgrade policyの比較軸を満たす）が明記される。保守者が行う実機7日観測（Pixel 9a / API 37）の記録枠と最終結論の確定条件も同じ文書に設ける。

完了の区分は次のとおりである:

- **agent実行分のresearch PR（中間成果）**: 上記の調査結果と暫定結論、実機観測の記録枠を含む。このPRは `Refs #442` であり、Issueをcloseしない。
- **最終PR**: 保守者の実機観測結果（Pixel 9a / API 37。日数とcrash収集の結果を含む）がresearch文書に記録され、判断基準の適用として**最終結論**が確定していることを内容とする。このPRだけが `Closes #442` を使う。最終結論の確定をもって、#447（ADR-0014）は結論を受入の前提として起草・受入に進めることができる（暫定結論の段階でも起草は開始できるが、受入は最終結論を前提とする。メモ §5）。
...
- **api36/37既知問題の棚卸し**（調査method 4）: Issue #304（OPEN、root cause未確定）と #418（API 36 organizer instrumentationの断続的failure）の記録を、**Issue単位ではなくfailure signature/family単位**で照合する。各signatureについて (a) 発生環境（CI emulator / 実機）、(b) user-facingな日常利用への影響の直接証拠の有無、(c) production exposureの静的根拠（発生箇所がproduction pathに存在するか）、(d) 分類（「CI計測環境の問題」「実機日常利用への波及」「未確定」）を記録する。Issue単位の要約を付ける場合も、signature別の結果を潰さない。
- **適性チェックリストのagent実行分**（調査method 1のうちagent実行可能な補助証拠）: API 36 emulator（system image `android-36`、既存AVD `nunu_qpr2_api36_1` 等）にdebug APK（`assembleLawnWithQuickstepGithubDebug`）を導入し、[Issue #442](https://github.com/nunu1733/NunuLauncher/issues/442)のチェックリストのうちUI自動化で確認できる項目（ホーム基本操作、アプリ起動、フォルダ作成・開閉、Remove drop targetとundo snackbar、ウィジェット追加、検索、回転、recents無効の確認）を実施して、項目ごとの結果（実施/未実施と理由）、端末、build、commit SHA、確認日を記録する。API 37用system imageはlocalに存在しないため、**API 37（Android 17）の確認は保守者の実機（Pixel 9a）に依存する**ことを明記する。実機観測の実行は保守者の手作業であるが、その結果の記録と最終結論の確定は本Issueの終了条件である（AC-8。最終PRで完了する）。
- **research文書の新設**: `docs/assessment/issue-442-android16-17-fitness-research.md` に上記全結果、選択肢A/B/Cの判断基準適用表、暫定結論（推奨）、保守者の実機観測の記録枠（日数・crash収集方法の保守者判断欄と、最終結論の確定条件）、実機観測結果の記録欄と最終結論をまとめる。
...
- 実機7日観測のagent実行・自動化（観測そのものは保守者の手作業で行う。ただし観測結果の記録と最終結論の確定は本Issueの終了条件であり、最終PRで完了する。agent実行分のresearch PRは中間成果として `Refs #442` とする）。
...
### Scenario: 既知問題がfailure signature単位でCI計測環境か実機日常利用かで切り分けられる

Given #304（api36 window focus、root cause未確定）と #418（API 36 instrumentation断続的failure）の記録（Issue本文、capture、run link）
When 発生記録をfailure signature/family単位（例: SystemUI ANR/focus-gate系、Compose timeout系、`SlotWriter`/Activity-destroy process-crash系、window focus保持occluder系）に分解する
Then 各signatureについて (a) 発生環境（CI emulator / 実機）、(b) user-facingな日常利用への影響の直接証拠の有無、(c) production exposureの静的根拠、(d) 分類（CI計測環境の問題 / 実機日常利用への波及 / 未確定）が根拠（run link、captureの証拠行）つきでresearch文書に記録される
And 分類が未確定のsignatureは未確定として明記される（推測で補わない）
And Issue単位の要約を付ける場合もsignature別の結果が潰されない
...
And API 37（Android 17）の確認と7日観測が保守者の実機作業であり、その結果の記録と最終結論の確定が本Issueの終了条件（AC-8、最終PR）であることが明記される
...
And agent実行分のresearch PRは `Refs #442` でmergeされ、実機観測結果の記録と最終結論を含む最終PRだけが `Closes #442` を使う

### Scenario: 保守者の実機観測と最終結論がresearch文書に記録される

Given 暫定結論と実機観測の記録枠がmerge済みのresearch文書に存在する
When 保守者が実機（Pixel 9a / API 37）での観測（日数・crash収集は記録枠の確定値）を完了し、結果を記録する
Then 実機観測結果（端末、build/commit、期間、crash/ANRの有無と根拠、チェックリスト未実施項目の結果）がresearch文書に記録される
And 選択肢A/B/Cの判断基準を全証拠（agent実行分+実機観測）へ適用した最終結論が根拠つきで記録される
And 最終PRだけが `Closes #442` を使い、specのstatusが遷移する
...
- [ ] AC-3: #304/#418の記録がfailure signature/family単位で分解され、各signatureについて発生環境、user-facingな日常利用への影響の直接証拠の有無、production exposureの静的根拠、分類（CI計測環境の問題 / 実機日常利用への波及 / 未確定）が根拠つきで記録されている。未確定のsignatureは未確定と明記されている（PV-AC-03）。
- [ ] AC-4: チェックリストのagent実行分がAPI 36 emulatorで実施され、項目ごとの結果（実施/未実施と理由）、端末、build、commit SHA、確認日がresearch文書に記録されている。recents無効の確認がbuild設定との整合つきで含まれる。API 37の確認と7日観測が保守者作業であり、その記録がAC-8の終了条件であることが明記されている（PV-AC-04）。
- [ ] AC-5: 選択肢A/B/Cの判断基準適用表と暫定結論（推奨）が根拠つきで記録されている。結論がC方向の場合、専用Epicの起票範囲とADR起草要件が明記されている。agent実行分のresearch PRが `Refs #442` であり、`Closes` を使わないことがPR本文で確認できる（PV-AC-05）。
...
- [ ] AC-8: 保守者の実機観測結果（Pixel 9a / API 37。端末、build/commit、期間、crash/ANRの有無と根拠、チェックリスト未実施項目の結果）がresearch文書に記録され、全証拠への判断基準適用として最終結論が確定している。このACは実機観測後の最終PRで満たされ、その最終PRだけが `Closes #442` を使う（PV-AC-08）。
...
| AC-3 | research文書の既知問題節（signature別分類表。#304/#418のrun link・capture証拠との照合） |
| AC-4 | research文書のチェックリスト節（項目別結果、screenshot、端末/build/SHA/確認日） |
| AC-5 | research文書の結論節（判断基準適用表、暫定結論、Epic/ADR起草要件）+ PR本文のIssue関係（`Refs #442`） |
| AC-6 | research文書の実機観測節（記録枠と確定条件） |
| AC-7 | PR本文のcommand実行結果 |
| AC-8 | research文書の実機観測結果記録と最終結論（最終PR本文の受入条件表とIssue終了条件の対応付け） |
