...
data class PersonalizationSignalSnapshot(/* spec の契約: schemaVersion, contentDigest,
    usageAccess, launcherOriginAvailability, profileAvailability,
    entries (availability のある section のみ — sparse object 契約。entry field は non-null な
    SignalField<T> = Value / Absent — 2026-09-15 review Blocking 1 と (2nd) review Blocking 2。
    nullable 表記は使わない。clock 由来の表示 metadata は snapshot 外) */)
...
- bucket 計算は usage stats の raw 値と単一の window anchor のみから決定論的に行う pure 関数。window anchor は source が read 開始時に1回だけ読む clock 参照とし、composition 内で2つ目の時点参照を作らない。quantile algorithm は spec U-5 確定値 (nearest-rank: `B_k = x_⌈kN/5⌉`、`bucket(v) = |{k : B_k ≤ v}|`) を実装し、interface test に N=5/6/7/11 の期待値を記載する。recency 境界は半開区間 `[0,1d) / [1d,7d) / [7d,30d) / [30d,∞)` (launcher-origin は同境界の暦日量子化)。interval 境界は local calendar day 単位 + full-containment フィルタで実装し、probe (実 `UsageStatsManager` 実測 — 実装開始禁止の対象外、U-5 依存 PR の merge 前の完了条件あり) で検証する。
...
- `UsageStatsManager.queryUsageStats` の実 returns 値・集計粒度・interval 境界の挙動は現行 main の code からは確認できない (probe/instrumentation — 実装開始禁止の対象外 — で実測し、U-5 依存実装 PR の merge 前に probe 完了条件 (a)〜(c) の記録を完了する)。
