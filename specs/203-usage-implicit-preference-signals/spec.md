...
> 2026-09-21 amendment ([Issue #371](https://github.com/nunu1733/NunuLauncher/issues/371) — Usage Access 要求の just-in-time 化): U-2 を「常設管理row（hub T-06）＋初回 signal 読み取り直前の JIT 要求 1回」へ改訂し、Permission and fallback behavior 表の「opt-in (初回)」行の rationale 要件を修飾形（raw 履歴非送出 / bucket 化 / 送信前確認経由）へ更新のうえ JIT 要求行を追加、受入条件 AC-17〜AC-19 を追加した。snapshot・provenance 契約（AC-11〜AC-16 / U-4）への変更はない。処分文書 §3.10 の二段階所有（配置は #367、JIT は #371）どおり。
...
> 2026-09-15 (probe amendment): U-5 probe を API 36 emulator で実行し、完了条件 (a)〜(d) を記録した ([probe evidence](../../docs/assessment/pr-321-u5-probe-evidence.md))。**逸脱を検出**したため、acceptance 契約の定める merge 前修正として U-5 の window semantics を修正した: 実測により `queryUsageStats(INTERVAL_DAILY)` の interval は local calendar day に整列しない ~24時間 rolling bucket (697 件中 430 件が暦日跨ぎ) であり、暦日ベースの full-containment フィルタは過半の interval を喪失する (`crossBegin=crossEnd=0` で interval の範囲逸脱・重複は観測されず)。したがって window は rolling 24h 単位 ([anchor − N·24h, anchor]) とし、返された interval を edge フィルタなしでそのまま集計する。active-days は「foreground>0 の interval 数」の近似に変更。day anchor の device 現行 timezone 追従 (DST zone / 日付変更線越え含む3 zone で確認) は契約どおりであり修正不要。
...
| opt-in (初回) | 明示的な user action のみ。導線は **2種** — (a) **Organizer hub (T-01) 材料セクションに常設の管理row** (U-2 — 配置は [Issue #367](https://github.com/nunu1733/NunuLauncher/issues/367) の材料集約による。状態表示・遷移・`ON_RESUME`再読取・再付与の管理契約)、(b) **初回 signal 読み取り直前の JIT 要求** ([Issue #371](https://github.com/nunu1733/NunuLauncher/issues/371) — process で 1回のみ。同一 run 内での自動的な再促しは行わない)。許可 flow はいずれも system の usage-access settings (`ACTION_USAGE_ACCESS_SETTINGS`) への遷移。rationale 文言は「raw usage 履歴 (event/timestamp 粒度) は端末外へ出さない」「Organizer 内部では coarse な normalized/bucket signal へ変換する」「AI 相談に含まる場合も送信前確認を通る」「整理精度の向上が目的」「許可しなくても Organizer の全機能は利用可能」を含む (2026-09-21 #371 amendment — 旧「usage 情報は端末内でのみ使用される (local-only)」の無条件表現は TO-BE §7.2「signal の保存なし・bucket のみ外部送出」との整合で撤去) |
| JIT 要求 (#371) | 当該 process で最初に signal を読む composition (整理 run の composed phase / AI 依頼生成) が実行される**直前**で、`usageAccess` が未許可の場合に限り 1回だけ表示される任意要求。run 開始操作そのものでは表示されない (検出が選択面を開けた場合は選択確認の直前まで表示しない)。表示は process の in-memory latch で 1回に固定され (提示で消費。提示後は再表示されない)、永続化しない。断った場合・付与せずに戻った場合は本表「拒否」行と同一の継続 (personalization 由来の `NotReady` は存在しない)。要求の文言は opt-in 行と同じ rationale 要件を 1文で満たす |
...
- [ ] AC-17 ([#371](https://github.com/nunu1733/NunuLauncher/issues/371)): JIT 要求が「当該 process で最初に signal を読む composition の直前」でのみ 1回表示されること — run 開始操作・選択面の表示では表示されず、選択面の中断では要求機会が消費されないこと、付与済みの初回 trigger 評価で機会が消費されること (後続の権限取消でも表示されない)、および同時 trigger でも提示が同時に 1つであることが test で示されている。
- [ ] AC-18 ([#371](https://github.com/nunu1733/NunuLauncher/issues/371)): JIT 要求の表示・応答が journal / diagnostics event・persistent store への書込みを発生させないこと (zero-write)、断った場合・付与せずに戻った場合に launcher-origin section が保持され system usage section のみが利用不能になること (AC-11/AC-13 回帰) が test で示されている。
- [ ] AC-19 ([#371](https://github.com/nunu1733/NunuLauncher/issues/371)): JIT 要求と常設rowの rationale 文言 (EN/ja) が修飾形の privacy 要件 (raw 履歴非送出 / bucket 化 / 送信前確認経由) と TO-BE §7.3 の 3 要素を満たすことが、実装 PR の contract commit での文言確定と意味要素 checklist の記録で示されている。
...
- **U-2 permission UX — draft (2026-09-21 #371 amendment: 常設row＋JIT要求)**: 導線は **Organizer hub (T-01) 材料セクションに常設の管理row** (usage access の現状表示 + rationale + system usage-access settings への遷移 + `ON_RESUME`再読取。配置は [Issue #367](https://github.com/nunu1733/NunuLauncher/issues/367) が確定) **＋ 当該 process で最初に signal を読む composition の直前の JIT 要求 1回** ([Issue #371](https://github.com/nunu1733/NunuLauncher/issues/371) が所有。run 開始操作には結合しない。提示で process 内消費 — 提示後は同一 process で再表示しない。manual run 内での自動的な再促しは行わない「騒がせない」方針は JIT を含めて維持)。要求stateは process-local であり永続化しない。rationale 文言の要件は Permission and fallback behavior 節のとおり (privacy は修飾形 — raw 履歴非送出 / bucket 化 / 送信前確認経由)。accessibility 要件は Accessibility 節のとおり。
...
- **U-5 bucket・rank semantics — 確定値提示 (acceptance で確定、probe は実装ゲート)**: 各 bucket の区分数・境界に加え、相対順位の **rank universe**、tie 処理、少数サンプル時の挙動、timezone / calendar boundary、`queryUsageStats` の interval 境界 semantics、launcher-origin day anchor の day boundary を含む。確定値:
  - **rank universe**: 当該 window の query 結果内に `totalTimeInForeground > 0` の interval を1件以上持つ launchable app (request 集合は不可 — 2026-09-13 review Required specification)。usage record の有無が変わるのは当該 app の usage 自体が変化したときのみであり、request 集合由来の不穏当な bucket 変動を避ける。
  ...
  - **active-days bucket (30d)**: 30d query 窓内で返された `totalTimeInForeground > 0` の interval 数 (各 interval ≈ 1日分の bucket — 2026-09-15 probe amendment) を観測日数の近似とし、0 / 1–3 / 4–9 / 10–19 / 20–30 の 5 段階に分類する (上限 30)。
  ...
  - **window semantics (2026-09-15 probe amendment — 実測に基づく修正)**: 単一 window anchor (capture 時の device local time) を基準に **rolling 24h 窓 `[anchor − N·24h, anchor]`** で `queryUsageStats(INTERVAL_DAILY)` を query し、**返された interval をそのまま集計する** (partial edge フィルタを行わない)。初版の「local calendar day 単位 + 完全包含のみ集計」は probe の実測 (interval は暦日に整列しない ~24h rolling bucket、697 件中 430 件が暦日跨ぎ。probe 環境では `crossBegin=crossEnd=0` — 一般 API 契約上は interval が begin/end 境界へ拡張され得るため、この観測は probe 環境の事実として扱う。重複は probe の計測範囲で 0 件) により棄却された — 暦日 containment は過半の interval を喪失する。決定論は「同じ usage data + 同じ anchor + 同じ query 範囲 → 同じ集計」で保たれる。window 端 ±1 interval 分の計上誤差は既知 limitation であり、bucket 化 (相対 5 分位) が緩和する。timezone 変更後の capture は現行 device timezone の anchor で再計算する (snapshot は永続しないため自然に従う)。
  - **launcher-origin day anchor**: device local calendar day の epoch-day (day boundary は device 現行 timezone の暦日)。
  - **probe の完了条件 (実装ゲート) — 2026-09-15 に全件実行・記録済み** ([probe evidence](../../docs/assessment/pr-321-u5-probe-evidence.md)): (a) `queryUsageStats(INTERVAL_DAILY)` の実 returns の interval 境界・粒度の実測記録 — 完了 (24h rolling bucket、暦日非整列を確認)、(b) full-containment フィルタの実データでの動作確認 — 完了 (暦日 keyed では過半の interval を喪失する逸脱を検出し、本節の修正へ反映)、(c) 30日分の取得可否 (保持期間) の確認 — **完了 (判定基準を「30d query の受理 + available retained range の記録」へ amendment — 2026-09-15 re-review)**: platform 保持期間は device 依存であり API が 30日分を保証しないため、bucket 化された signal 契約として undercount 既知 limitation (U-5 / Risks) の下で許容する。実測: 35日 query を受理、本 emulator の retained range は約9日、(d) launcher-origin day anchor の DST / timezone 変更時の挙動確認 — 完了 (3 zone: Tokyo / New York (DST) / Midway で anchor が現行 timezone に追従)。逸脱 (b) については本 amendment がその spec 変更である。probe 自体は実装開始禁止の対象外 (本節先頭)。
