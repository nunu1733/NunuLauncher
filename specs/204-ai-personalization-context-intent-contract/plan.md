...
> Status: draft
> Baseline: `origin/main` = `9ea2ba0eb4d9ef61bd96ef2b480bbd20ed055edd` (2026-09-15時点、#203実装を含む)。3rd review (ChatGPT review "Request changes", 2026-09-15, snapshot `12f773ad` 基準) 対応を含むrevision。過去の再入場検証履歴は「Current evidence」節を参照。
...
├── ContextExportBuilder.kt   # canonical inputs + RefAllocator -> export (pure, tier-aware, mobility投影, usageSignals投影)
...
```text
canonical inputs --(ContextExportBuilder + RefAllocator, tier)--> PersonalizationContextExportV1 (state fingerprintを含むusageSignals投影付き。signal identityは文書に現れない) + export session (durable作成; structural sourceContextDigestはSourceContextIdentityが計算し、signal provenanceとともにsessionのみへ)
...
|---|---|---|
| `organizer/personalization/` (追加) | Context/Intent model、builder、RefAllocator seam、SourceContextIdentity (structural)、codec、validator、identity、adapter、ExportSessionStore **interface** | 契約本体。純粋package (例外なしのpurity対象。#203既存fileと同居) |
...
2. child A: `personalization/` 純粋package — models + builder (RefAllocator注入、usageSignals投影、signal provenance) + SourceContextIdentity (structural) + codec + validator + identity、`personalization/` 全体を例外なしでcoverするpurity guard、全contract/property/security test (AC-2,3,4,5,7,8,12,13,14)。mobility投影のFIXED/MOVABLE判定とplanner整合のproperty test、`fixReason` cause precedenceの個別testを含む。
3. child B: provenance統合 (`PERSONALIZED_INTENT` 第7 input、sentinel default) + `OrganizationInput.intentPreferences` 追加 + adapter + `PlanningResult` echo + composer stable cut 組入れ (AC-6,9)。`risk: layout-data` + high-risk gate。
...
- RefAllocatorのproduction乱数源の具体実装 (暗号論的強度の選定) とref形式 (長さ・alphabet)。child Aで確定。契約条件 (生成ごとの新鮮さ・export内一意性・entropy要件・ItemId逆算不可) はtestで検証する。
