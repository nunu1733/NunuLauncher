...
- keyboard/DPAD/switch accessから全actionが到達可能であることを維持する。
...
- [ ] AC-5: AC-1〜AC-4を満たすproduction UI regression testが存在し、実touch gesture注入を使用する。新testは修正前code上でfailすることを確認してPRへ記録する。
...
| AC-4 | recreation/process再起動系instrumentationの既存caseをpost-fix挙動で維持 |
...
- 反復tapでclickが発火しないframework-level機構の正確な分類(H1: 毎tap focus再取得 / H2: gesture途中intercept-cancel)。plan Phase 0の診断対象であり、受入条件は観測可能振る舞いで定義済みのためnon-blockingである。
