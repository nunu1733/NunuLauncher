...
- 2026-09-06: [Issue #209](https://github.com/nunu1733/NunuLauncher/issues/209) ([spec 209](../209-organizer-decision-action-affordance/spec.md)) による decision action 表現の変更: confirm / cancel が preview 見出し直後の Material3 button decision group へ移動したことに伴い、keyboard / Switch traversal の契約順序を status → confirm → cancel → 展開 action へ更新 (AC-5 と scenario)。変更一覧の grouping / counts truth / truncation / a11y 契約は無変更。
- 2026-09-07: [PR #239](https://github.com/nunu1733/NunuLauncher/pull/239) review 対応: spec 209 の degraded モード描画順変更 (見出し → 告知行 → count-only summary → decision group) を D1 の契約として明記 — degraded path では欠落告知と summary が confirm より先に読まれる。具体一覧モード (`details != null`) の描画順 (decision group 先頭) は無変更。
- 2026-09-07: [Issue #234](https://github.com/nunu1733/NunuLauncher/issues/234) による destination 表示の更新 ([spec 234](../234-organizer-destination-anchor-specificity/spec.md) を正本とする): 移動行の workspace destination が `page + 領域 + 1-based 行/列表示序数` の anchor 一意語になり、D5 の残余リスク「同帯・同行序数の列方向調整は before/after が同一文言」の destination 部分が解消した。same-band 行の「«領域» 内で位置を調整」形式は destination が anchor 一意語を担う形で継続し、行序数 note 分岐は無変更。grouping / counts truth / truncation / descriptor 契約 (D2 / D6 / spec 208) は無変更。
