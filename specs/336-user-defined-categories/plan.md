...
| Exchange projection | User-defined classifications export as **absent categories** (no raw ID, no display name) in `ContextExportBuilder`; `ScopeBindingGate`/`SessionExportReconstructor` apply the same projection rule, so user-defined catalog edits never invalidate an active session; #204 intents keep built-in-only addressing. Parity/privacy tests cover both sides. |
...
| `organizer/personalization/ContextExportBuilder.kt`, `personalization/exchange/ScopeBindingGate.kt`, `SessionExportReconstructor.kt` | Shared null-projection rule for user-defined classifications (export + gate + session reconstruction); no raw ID/name on the exchange surface | #331 typed mismatch semantics unchanged; parity-tested both sides |
...
5. **Title binding + exchange projection.** Per-composition resolver binding in `LayoutApplicationModule`/`PlanPreviewProtocol`; shared null-projection rule across `ContextExportBuilder`/`ScopeBindingGate`/`SessionExportReconstructor` with parity tests.
...
| User-defined-classified candidate in exchange export | Exported with no category (no ID, no name); gate applies the same rule | Active sessions unaffected by user-defined catalog edits; genuine divergence still fails typed |
...
| Exchange parity tests | `ContextExportBuilder` emits no category for user-defined classifications and identical bytes for built-in-only runs; `ScopeBindingGate`/`SessionExportReconstructor` apply the same rule; no `UserCategoryId`/display name in export text or session records; catalog edits (create/rename/delete of unused category) do not invalidate an active session |
