...
> Issue #367 (entry notation only, contract unchanged): since the organizer materials relocation (TO-BE §5.2), the settings-side entry to this destination is the Organizer hub (T-01) materials group instead of a Home-screen Layout-group row. Every “Home Screen settings entry” wording in this spec, including AC-2, is satisfied by that hub-routed entry; the row text, destination, AUTHORING lease, and authoring contract are unchanged.
