...
Before any ordinary authoring mutation, `CategoryOverrideAtomicAccess` reads and validates the legacy schema-v1 source without modifying it; physical absence is its defined generation-0 state. It writes and verifies the equivalent complete AtomicFile snapshot, then synchronously commits and verifies legacy `schema = 2`. Until that marker is durable, AtomicFile content is only an equivalent compatibility copy and no user mutation may be accepted. If the marker commit returns `false` or throws, the boundary enters typed `MigrationBarrierUncertain`: **in that process it admits neither legacy/AtomicFile override reads nor authoring mutation**, reports no migrated/saved success, and requires a fresh durable reload before authority is selected again. After that reload, marker absent restores legacy authority; marker present establishes AtomicFile authority. Once marker success has been durably re-read, new binaries read only AtomicFile and old #83 readers see `schema = 2` and return `UnsupportedSchema` before reading entries. The migration tests must prove same-process override-read rejection after marker failure, then cover a later AtomicFile mutation followed by an old-reader restart and prove fail-closed behavior.
...
| Rollback/downgrade tests | Migrate valid legacy data, perform one or more AtomicFile authoring mutations, start the old #83 reader in-process and after restart, and assert `UnsupportedSchema` rather than stale/empty `Ready`; marker `false`/exception must reject every same-process override read and mutation as `MigrationBarrierUncertain`, then a fresh durable reload must select only marker-absent legacy or marker-present AtomicFile authority. |
...
| Compatibility tests | First-run generation-0 behavior; exclusion of legacy and AtomicFile override paths from backup; physical-absence and valid-legacy upgrade; durable `schema = 2` barrier before post-migration mutation; old-reader downgrade `UnsupportedSchema` without stale/empty S1; marker-failure same-process read/mutation rejection followed by durable-reload authority selection; process recreation after interrupted/committed mutation. |
