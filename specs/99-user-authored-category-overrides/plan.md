...
| Commit model | Publish one complete immutable snapshot through the established AndroidX `AtomicFile` base/temporary-file protocol, then full-store verify; never publish through mutable `SharedPreferences`. | Ensures a pre-finish failed generation cannot reach the #83 reader in-process or after restart. [4] |
...
|---|---|---|
| `lawnchair/src/app/lawnchair/organizer/rules/CategoryOverrideSnapshot.kt` | Retain `CategoryOverrideSnapshotSource` filtered read behavior while adding a compatibility reader over one AndroidX `AtomicFile` complete-snapshot file, a private full-store codec, and typed writer results carrying stored and verification-visible identities. | Composer retains its read-only `Ready`/`Unreadable`/`UnsupportedSchema` semantics and filtered digest while never observing the AtomicFile temporary record. |
...
Extract the current parser and canonical renderer into a private full-store codec used by the writer. The decoder continues to reject blank/malformed components, duplicate `(package, profile)` keys, negative generations, incompatible schema, and runtime storage faults. Its digest covers the complete immutable snapshot entry set, sorted by profile and then package according to existing canonical UTF-8 ordering. The compatible public reader opens only the AndroidX `AtomicFile` final/base snapshot and then applies the existing profile-filtered map/digest semantics. [4]

Under a process-local writer mutex and an admitted organization-operation lease, the writer reads/validates the final complete snapshot, compares `expectedStoredIdentity`, checks taxonomy binding, materializes a new full map, and increments generation exactly once only for a state-changing request. It uses the existing organizer-recovery-style AndroidX `AtomicFile` boundary: `startWrite()`, complete encoded write plus fsync, then `finishWrite()`; a write/finish exception invokes `failWrite()`. `SharedPreferences.apply()` and `SharedPreferences.commit()` are prohibited as authoring publication paths because a failed write may otherwise leak into same-process reader state. Before success, the writer re-opens the final snapshot through the full-store codec and verifies its digest; a post-finish validation error makes the standard reader typed unreadable and #83 non-write. Only after full-store success does the compatible standard reader run with explicit verification profiles to obtain the separate verification-visible identity. If the established AtomicFile protocol cannot prove pre-finish failure isolation on target Android versions, stop Stage B and create a Decision/ADR rather than weakening AC-6.
...
|---|---|
| Rule-store unit tests | Full-store parser/renderer round-trip; all 34 valid IDs; unknown category rejection; set/change/remove/no-op; canonical complete-map stored digest; monotonically increasing generation; duplicate/corrupt/unsupported final snapshot; `startWrite()`/write/`finishWrite()` failure injection with `failWrite()`; post-finish full-store verification failure; expected-stored-identity conflict; no partial publication. |
| Composer integration tests | Same-profile S1 precedence; removal falls through to S2–S6; retained orphan proves stored versus filtered composer-visible digest distinction; a pre-finish failed AtomicFile temporary snapshot is invisible to the same-process #83 reader and after process restart; post-finish invalid content returns typed `NotReady`; profile isolation and unavailable-profile fail-closed behavior. |
