...
| MFO-18 | Confirmation or recovery-preview decision is offered (spec 209) | The confirm/cancel (and restore/cancel) decision actions render as Material3 buttons; with a concrete change list they sit directly below the state heading and remain visible together regardless of change-list expansion (they scroll with the list — no persistent surface is required), report the button role to TalkBack, and traversal reaches status → confirm → cancel → expand actions. Every run-state transition returns the list to its head before restoring focus, so the heading and its decision pair are visible right after `Applied` → `RecoveryPreview` without corrective scrolling. A `details == null` preview announces the missing list and its count-only summary before offering the decision (spec 195 D1). |
