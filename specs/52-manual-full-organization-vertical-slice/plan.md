...
The coordinator has four dependencies: a manual capture/input port, `OrganizationPlanner`, an application façade restricted to `apply`/`recover`/readiness state, and `DiagnosticsPort`. It does not receive a writable database, `RecoveryStorePort`, `LayoutWriterPort`, Android cursor, or raw persistence manifest.
...
On `Applied`, the UI offers an explicit recovery action for the returned point. Before confirmation, it obtains a fresh current revision and shows the expected recovery effect/safety message. It constructs `RecoveryRequest(pointId, expectedCurrentRevision)` and invokes only `LayoutApplicationModule.recover`. It maps every `RecoveryResult` variant directly, avoiding a raw backup, an unbound restore, or an extra recovery writer.

## Implementation sequence

1. **Confirm the target commit and input-source ownership.** Re-read Issue #52 and its comments, this accepted spec, the accepted planner/application/diagnostics/empty-folder/lock artifacts, and `git status`. Identify the production owner of rules, taxonomy, signals, and target membership. If the owner is absent or not accepted, create the blocking follow-up and stop.
...
| RUN-11 | Explicit recovery with matching revision | Preview/confirm → `Restored`; DB/model verified through existing seam. |
| RUN-12 | Explicit recovery stale/expired/corrupt/busy/failure | Distinct `RecoveryResult`, no blind write, safe next action. |
| RUN-13 | Process recreation at preview, checkpoint, commit, and verification boundaries | No blind replay; reconciliation precedes new operation; truthful safe UI state/focus. |
...
|---|---|
| JVM coordinator/unit | `ManualOrganizationRunTest`: state transitions, fresh retry, cancellation, stale invalidation, no change, result mapping, explicit recovery request. `ManualOrganizationCaptureAdapterTest`: canonical input assembly, target/profile/device/lock preservation, rule-source absence failure. `ManualOrganizationPlanMaterializerTest`: conservation, exact revision/state, no delete, folder/page/action integrity. `ManualOrganizationDiagnosticsTest`: phase order/run-ID correlation and fail-open/non-containment. `ManualOrganizationMessagesTest`: typed mappings without journal dependence. |
| Existing application protocol | Extend only through public/internal application seam tests: supplied run-ID reaches `ApplyProtocol`; existing checkpoint/A0–A8/failure behavior remains unchanged. No test calls a new UI-side writer path. |
