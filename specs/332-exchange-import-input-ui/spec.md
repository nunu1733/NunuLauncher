...
- [x] AC-8: TalkBack (各source操作の識別・typed失敗の読み分け)、Switch Access/keyboard完結、200% font (主要CTA到達性・bounded scroll) のevidenceがある。D-4の実寸確定根拠を含む。evidence: [assets-345-import-evidence](../../docs/assessment/assets-345-import-evidence/README.md) (`11`–`14`: TalkBack有効下のsurface描画・Switch Accessのswitch型setup要求・keyboard focus・Tab+Enterでの `From file` 起動) + [assets-332-import-ui](../../docs/assessment/assets-332-import-ui/README.md) (`06`/`07`: 200% font・D-4実寸)。source操作の識別とtyped失敗のlive regionは `ExchangeImportSurfaceInstrumentationTest` のsemantics assertionが正本。読み上げ音声walkthroughとSwitch入力の通し操作は、合成入力がTalkBack/Switch Accessに採用されず、emulatorにswitch実機・camera switchが無いため未取得 (残差として明示)。
- [x] AC-9: representativeなChatGPT/Gemini mobile appからのcopy → NunuLauncher import (clipboard読込による) のdevice evidenceがある。file経由 (AI appの回答をfile保存→読込) も併記することが望ましい。evidence: [assets-345-import-evidence](../../docs/assessment/assets-345-import-evidence/README.md)。ChatGPT mobile web (native appはPlay非搭載AVDへ導入不可のため代表surfaceとして使用) のmessage copy (`03`/`04`) とコードブロックcopy (`07`/`08`) → 本appの「Import from clipboard」1押下で、認識framing (Marker lines / Standalone JSON) とtyped failureが実測された (`06`/`09`)。file経由は契約適合のAI製replyを `From file` で取り込み **「Proposal validated」** まで到達 (`10`)。AI出力の契約不適合 (行末 `\` エスケープ、`globalPreference.organization`/`grouping` 等の契約外キー、小文字enum、小数confidence) は本app側の拒否が意図どおりであることを示す。clipboard経由での *成功* importは本sessionでは得られず、成功パスはfile経由で取得した (残差として明示)。D-3の `text/*` 拡張は不要と判明。
...
- 2026-09-17: [Issue #345](https://github.com/nunu1733/NunuLauncher/issues/345) evidence pass。残置していたAC-8/AC-9の手動/物理evidenceを取得し、device evidenceを [assets-345-import-evidence](../../docs/assessment/assets-345-import-evidence/README.md) に記録: (1) ChatGPT mobile webからのcopy (message copy・コードブロックcopy) → 「Import from clipboard」1押下 → 認識framing + typed failureの実測 (`03`–`09`)、(2) 契約適合のAI製replyのfile取り込み成功 (「Proposal validated」) (`10`)、(3) TalkBack有効下のsurface描画・Switch Access setup要求・keyboard完結 (Tab+Enterで `From file` 起動) (`11`–`14`)。合わせて `ExchangeImportSurfaceInstrumentationTest` をCI lane `organizer-instrumentation-issue332-tests` へ組み込み、`final-status` の対象にした。残差: TalkBackの読み上げ音声walkthrough・Switch入力の通し操作は環境制約 (合成入力の非採用、switch実機なし) で未取得、clipboard経由の成功importはAI出力が契約不適合だったため未成立 (成功パスはfile経由)。
