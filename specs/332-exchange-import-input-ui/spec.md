...
> **境界 (issue #373 改訂)**: 本specのImport入力面はTO-BEの **T-17 (取り込み)** に対応する (V-33の後継)。T-17の**入力**契約 (clipboard/file/manual paste・1 MiB envelope gate・bounded editor・parse-first presentation・source別typed失敗のin-place表示) は本specが所有し続ける。一方、取り込み**結果**表示 (T-18失敗面の手段別remedy再投影・成功面の件数サマリ) の正本は spec 205 (AC-5、#373改訂) / [spec 328](../328-exchange-import-success-state/spec.md) (成功面) / [#373 spec](../373-import-display-reprojection/spec.md) (accepted) が所有する。#373により、T-18失敗面の認識情報 (framing/version/entry数) とraw detailはprimary面から**詳細展開内**へ移設された (入力面の契約・実寸は不変)。
...
- 2026-09-21: **結果表示の正本境界の明記 ([issue #373](https://github.com/nunu1733/NunuLauncher/issues/373) 改訂、disposition §5 更新順序 #7)** — T-17配置表記 (V-33 → T-17) と、取り込み結果表示 (T-18失敗面・成功面) の正本が spec 205 (#373改訂) / spec 328 / #373側である旨の境界節を追加。AC・Decisions・D-4実寸は不変 (認識情報とraw detailの詳細展開内への移設は表示位置のみの変更で、入力契約・retention boundary・1 MiB gateは不変)。
