...
- 2026-09-16: **v3 bump追従 ([spec 330](../330-partial-intent-authoring/spec.md) 所有)** — Issue #330のaccepted specによる `personalization-context-v3` / `personalized-intent-v3` bumpへの追従記録。本specが導入したscope binding gate (`SCOPE_MISMATCH`)・candidate subject・`CANDIDATE preserve` rejectの規則は無変更 (candidateもv3の部分authoring対象であり、未言及candidateはcanonical unresolvedとなる)。v1→v2移行と同じく、v2 era session宛回答はbump後 `SCHEMA_MISMATCH` で再export案内となる。
