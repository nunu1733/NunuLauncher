...
- **`organizer/personalization/exchange/`**:
  ...
  - `SessionExportReconstructor.kt`: session ref mapのcandidate entryを現在状態へ解決 (installed / launchable / AVAILABLE / snapshot未表現) し、candidate item core (固定projection) を再構築viewへ含める。解決不能candidateはtyped結果 (candidate解決失敗)。placed部分のparity契約は無変更。
  - `ExchangeImportPipeline.kt`: 失敗統合resultへ `ScopeMismatch` とcandidate解決失敗を追加 (17種)。既存段階 (envelope → framing → decode → session → digest → reconstruct → validate) の順序は無変更。
  - `ScopeBindingGate.kt` (新規pure): spec D-2/D-5の判断部。入力: session記録のexport scope candidate集合、run確定選択集合、各candidateの解決結果。出力: `Pass` / `Mismatch(欠落候補)` / `CandidateUnresolved(候補)`。run確定点 (confirm後のcomposition) とfresh run再構築pathの両方から呼ばれる単一の正本。
...
- **`organizer/ui/` + `ManualOrganizationRun`**:
  - 選択surface (`MissingAppSelectionScreen` 近傍) にrun内exchange導線を追加: 「AI相談」明示開始 → freeze状態 (選択編集無効。status行でannounce) → 生成 (`ExchangeGenerationGate` 既存契約: activity session存在時はsession置換確認) → Pre-send Disclosure → transport (既存adapter流用) → import → **validated intentの当該runへの接続** (新typed run action。`State.Selecting` で1回のみ、単一active session宛のみ受理。fresh runを開始しない) → confirm → composition → `ScopeBindingGate` → planning (intent投影込み)。
  - 中止: 未送信package取消は既存規則 (`ExportSessionStore.invalidate`) + freeze解除。
  - `ManualOrganizationRun` への変更は最小: `State.Selecting` でのintent attach actionと、`runComposedPhase` のscope-composed経路での `ScopeBindingGate` 評価 (違反時はtyped zero-write失敗で選択状態へ戻す)。既存state遷移・`OrganizationOperationLease`・preview/confirm/apply pathは無変更。
  - fresh run再構築path (idle import → `run.start(trigger, intent)`): 既存flowに、confirm後compositionでの `ScopeBindingGate` 評価を追加するのみ。
...
MissingAppSelectionSurface (選択) → ユーザー明示「AI相談」
  ...
  → ExchangeGenerationGate (既存) → composeForExport(selection) [read-only canonical composition]
      → ContextExportBuilder.build (v2: placed + candidate items、session記録にcandidate集合)
  ...
  → confirmSelection → composeInternal (既存canonical composition)
      → ScopeBindingGate (⊇ + 解決可能性)
          ├─ Pass → planning (intentPreferences: placed ItemId宛 + candidate planning ID宛)
          └─ Mismatch / Unresolved → typed失敗 (zero-write、選択状態へ戻す、再export案内)
[process death後]
idle import → run.start(intent) (既存fresh run) → 検出 → 再選択 → confirm
  → ScopeBindingGate (同一正本) → planning
[idle entry]
...
- **digest入力に選択集合を追加するoption**: import時 (idle・process death後) には選択が存在せずdigest照合が成立しないため非採用 (spec D-4)。scope変化の検出は明示的candidate集合比較 (gate) が担い、役割が分離される。
- **完全一致のscope binding**: 再選択時の追加候補まで拒否すると再export loopが常態化し、AI権限も拡大しないため非採用 (spec D-2)。
...
- DB schema変更なし。session storeのrecord formatはsealed ref map + scope candidate集合への拡張 (app-private・backup対象外)。旧v1 session recordはschema識別によりimport不可 (TTL 24時間で自然失効。移行処理は不要 — 本spec受入から実装releaseまでの間にactivityなv1 sessionが存在し得ない単一ユーザー配布形態)。
...
| AC-6 | pipeline分類test (3段stale) + `ScopeBindingGate` unit test (Pass/包含違反/追加/無効化) + 失敗表示17種 UI test | unit + UI test |
| AC-7 | 空workspace統合test (export → import → plan → preview) | unit (planner/統合) + instrumentation |
...
5. **`ScopeBindingGate`**: 純粋gate unit test (AC-6) 先行。
