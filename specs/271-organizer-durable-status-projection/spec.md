...
- 2026-09-10: Backlog snapshot ported to `issue-271-spec-plan` (baseline
  `6b6bf8dd9fa0c42399185dbb13c30192f1e15962`) from the open PR #276 branch
  `issue-271-organizer-durable-status-projection`, where this spec previously
  passed a phase-1 review and was marked accepted
  (`4a1b0dd9a2`). The snapshot is reset to `draft` here because acceptance
  belongs to the workflow on the lineage that will merge it; on `origin/main`
  this spec is not yet accepted. The PR #276 branch's merge-base equals this
  baseline, so the content is grounded against the same code. Future workers
  must reconcile this snapshot with PR #276 rather than duplicating the
