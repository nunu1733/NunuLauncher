...
| `tests/organizer-instrumentation/app/lawnchair/organizer/application/Issue265ManualEditRecoveryInstrumentationTest.kt` | Keep Path A as the first-organize recovery oracle: assert real writer normalization, strict `Restorable → Restored`, and exact pre-run manifest; add an independent manual-move → second-organize → repeated-organize flow with reload, exact raw-manifest comparison, and verification, plus the Path B control | Exercises the complete production flow requested by Issue #269 without allowing the second organize to replace Path A's recovery point |
...
| AC-269-03 | The independent regression runs organize → manual move → second organize → reload → repeated organize → reload, with failing assertions for unavailable capture, non-verified apply, reload failure, post-reload canonical capture, and raw-manifest divergence | Same API 36.1 instrumentation target; use state assertions, not sleeps as the oracle |
...
- [x] The independent manual-move → second-organize path reaches verified
      apply, remains canonically capturable after reload, and produces an
      identical raw manifest on a repeated organize pass.
