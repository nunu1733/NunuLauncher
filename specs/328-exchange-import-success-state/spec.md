...
- **import attempt生存中の競合freeze**: 成功状態表示後だけでなく、import attempt開始 (validation中) から終了 (成功状態終了・破棄・キャンセル) までの全期間、idle entryの「整理を開始」rowを無効化する (run内entryのstrategy picker freezeは [#368](https://github.com/nunu1733/NunuLauncher/issues/368) のpicker run面撤去により対象消失)。#368以降、strategy書込はrun面に存在せず、`AUTHORING` admission leaseによりCTAのrun seam (`start`/`attachIntent`) と単一admission domainで相互排他する。
...
- 取り込み成功状態表示中の競合affordanceの無効化: idle entryの「整理を開始」row (run内entryのstrategy picker経路は [#368](https://github.com/nunu1733/NunuLauncher/issues/368) で廃止済み)。
...
And **strategy書込との競合は単一admission domainに還元される** (#368 amend、2026-09-19。旧「commit時gate/書込開始gate/arbiter busy拒否」規定は置換): strategy pickerはrun面から撤去されT-05のみに存在し、strategy書込は `dismiss()`/`start(trigger)` を呼ぶ経路ごと廃止されたため、import continuation中のstrategy変更がvalidated intentを持たない別runへrunを差し替える競合は原理的に存在しない。strategy書込は `OrganizationOperationLease` の `AUTHORING` tokenを書込開始〜全終端まで保持するため、CTAのrun seam (`start`/`attachIntent`) が書込中に呼ばれた場合は既存契約どおり `Busy`/`NotAttachable` のtyped failure settleとなり (処理中解除・成功状態維持・再試行可能)、run操作active中のstrategy選択はtyped non-writeとして拒否される。書込とimport validation自体 (run seamを触らないzero-write検証) の重なりは競合ではない、
...
And 成功状態の表示中にowning runを差し替える経路は存在しない (#368 amend: strategy pickerによるrun差し替え経路そのものが廃止済み。残る競合affordanceは次scenarioのidle start row freeze)。

### Scenario: 競合affordanceの無効化 (idle start row、両entry。#368 amend)

> #368 (2026-09-19) amend: strategy pickerのrun面撤去により、本scenarioからstrategy picker関係の
> 競合規定 (run内entry picker freeze・idle picker continuation中無効化・commit時gate・arbiter busy
> import/CTA拒否・書込開始gate・`RestartReserved`/`Restarting`状態機械) は削除された。strategy書込との
> 競合は `OrganizationOperationLease` の単一admission domain (AUTHORING token) に還元される。freeze
> 再設計 (status card＋T-18中心) は #374 (spec 328 rev.2) が所有する。
...
When 同一surfaceにrunを開始する競合affordanceが存在する、
Then **idle entry** では「整理を開始」rowが無効化される (#331の選択freezeと同じ拘束パターン。validation中も含む — validation中に開始された新runの上に後から成功状態が載ることを防ぐ)、
And **strategy書込との競合はadmission domainで直列化される**: strategy pickerは本specのsurface上に存在せず (#368)、strategy書込はrun seam (`start`/`attachIntent`) と同一admission domainを共有する。書込が `AUTHORING` tokenを保持している間にCTAが押された場合、`start`/`attachIntent` は `Busy` を返し、既存契約どおりtyped failure settleで処理される (処理中解除・成功状態維持・再試行可能)、
...
- 追加permission・外部送信なし。
- summaryのitem-level情報は件数のみ。加えてplanner-effectiveな `minimizeMovement` のboolean 1値だけを全体方針行として表示する (summary scenario / AC-4と同一のprivacy境界)。app label・folder title・`ref`・AI自由文 (`rationale`)・`confidence` を取り込み成功状態に表示しない (理由はsummary scenario参照)。出力modelへの混入をcontract testで防止する。
...
- 競合affordanceの無効化 (idle「整理を開始」row。run内strategy picker経路は #368 で廃止) はTalkBackに無効であることと理由が読み取れること。
...
- [ ] AC-1: import成功 (validation通過) 時、両entry (idle / run内) で取り込み成功状態が表示され、run接続 (attach/start) はCTA押下まで実行されないことがtestされる (**CTA押下前** のrun state不変・zero-writeを含む。CTA押下後のrun state変化は既存seamの正常な挙動)。validation settleのattempt anchorがtestされる: import → キャンセル → 遅延 `Validated` で成功状態が再出現しないこと、import A → import B の逆順settleでB (最新attempt) のみが成功状態になりAの遅延settleで置換されないこと、run内でsettle時にowning runIdが `State.Selecting` を外れていたら成功状態を表示しないこと、**入力変更のみ** (再importなし) の場合も検証される: import A → editor変更/クリア → attemptが失効し Aの遅延settle (table-drivenで `Validated` / `Failure` / `InputNotReady` 全種) が画面を変更しないこと。
...
| AC | Evidence |
|---|---|
| AC-1 | `ExchangeFlowStateHolder` unit test (validated → 取り込み成功状態、run state不変、attach/start未呼出; **blocking fake import による (a) import → cancel → 遅延Validated drop、(b) A/B逆順settleで最新attemptのみが成功状態、(c) run-inでsettle時owning runId不一致drop、**(d) 入力変更のみ/クリア → attempt失効 → 遅延settle (Validated/Failure/InputNotReady 全種table-driven) 非適用**) + hosting instrumentation test |
...
- 2026-09-18: Re-entry revision 10 (10th review 中×2・低×1対応)。**(1) 入力変更によるattempt失効をPlan/testへ明記** (中): 現行 `ExchangeImportField` はvalidation中も `onImportTextChange`/Clearが操作可能なため、「editor表示textが変わる場合、active attemptをMain-confinedにinvalidate (`importAttemptActive = false`) してから `Importing(newText)` を採用する」手順をPlan Design/Change setへ追加 (Clearも同一path)。AC-1へ「import A → editor変更のみ → A遅延settle (全種table-driven) 非適用」「import A → Clear → 遅延settle非適用」のblocking fake oracleを追加。**(2) arbiter「全終端解除」列挙の統一** (中): CTA scenario・Data and state・Plan Verification AC-3 (d4)・Risks を「restart成功 / Committed-no-restart / 非commit / `start()` throw / coroutine cancel」の5種へ統一 (AC-5のみ完全版だった不整合を解消)。**(3) Permissions節の「summaryは件数のみ」残存を修正** (低): item-level件数 + planner-effective boolean 1値の全体方針行という確定privacy境界へ統一。
