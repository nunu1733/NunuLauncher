...
And **commit時gateは単一のarbiter状態機械 (`Idle → Writing → RestartReserved/Restarting → Idle`) で動く**: restart可否の判定とreservation取得はMain-confined直列化点上で原子的に実行し、`dismiss()` → `start(trigger)` の実行はIO上で行う (startは同期heavy — Mainで実行しない)。`Writing` / `RestartReserved` / `Restarting` の全期間、import開始・CTA開始ともtypedに拒否される。arbiterはrestart成功・Committed-no-restart・非commit・`start()` throw・coroutine cancelの全終端で解除される。import開始とrestart判定は必ず全順序を持ち (import attempt先行なら書込開始gateによりW自体が開始しない / W先行ならimportは拒否され重ならない)、「W側がgateを読んで可と判断した直後、restart実行前にimportが開始し、attemptが内部競合で黙ってdropされる」競合窓が存在しない、
