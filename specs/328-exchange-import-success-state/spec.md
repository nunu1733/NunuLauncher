...
> Status: **draft** (2026-09-18 re-entry revision 5)。**D-2/D-3 (およびOpen question 3) はowner decisionとして確定済み** ([Issue コメント](https://github.com/nunu1733/NunuLauncher/issues/328#issuecomment-5723309396)、本spec「Decisions」に具体化)。本revisionで **strategy書込との直列化をrun内entryのattempt全期間へ拡張 (commit時gate・書込開始gate・attach前のowning runId再照合)**・**Scopeのseam例外文言を確定契約へ統一** を行った。import validation自体のattempt anchor・CTA single-flight・CTA処理中の破棄/Back不受理・strategy変更との相互排他 (idle含む)・CEを含むseam例外契約・summary内訳4種V1必須 + global preference表示・process death分岐は前revisionで固定済み。未決のproduct decisionは残っていない。
...
And **UIのdisabledに依存しないcommit時gateが存在する**: strategy書込が (a) import continuation中 (両entry)、または (b) run内entryではimport attempt生存中の全期間 (validation中・成功状態表示中を含む。`continuing` の有無によらない) にcommitした場合でも、commit時のrun active判定による `dismiss()` → `start(trigger)` は発火しない (strategyのcommit自体は有効で次回以降のrunに反映される。imported run・owning runを黙って差し替えない)、
...
- **strategy書込との相互排他**: strategy選択変更の書込 (既存 `onStrategySelected` → `store.select`) とimport continuationは直列化する。holder/hosting間で共有するstateは2つ: `importContinuationActive` (CTA flipからsettle終端まで) と `strategyWriteInFlight` (書込開始からcommit/no-opまで)。**commit時gateの抑止条件**: `importContinuationActive` 中 (両entry) **または run内entryでは `importAttemptActive` 中の全期間** にstrategy書込がcommitした場合、commit時のrun判定による `dismiss()` → `start(trigger)` を発火しない。CTA側は `strategyWriteInFlight` 中の開始を拒否する。**書込開始gate**: `onStrategySelected` はrun内entryの `importAttemptActive` 中およびidle entryの `importContinuationActive` 中は新しい書込を開始しない。いずれもUI disabledとは独立したstate gateである。
