...
> Status: **draft** (2026-09-18 re-entry revision 6)。**D-2/D-3 (およびOpen question 3) はowner decisionとして確定済み** ([Issue コメント](https://github.com/nunu1733/NunuLauncher/issues/328#issuecomment-5723309396)、本spec「Decisions」に具体化)。本revisionで **strategy書込のrestart判定とimport開始をMain-confined直列化点上で原子的に排他 (strategyWriteInFlight はrestart判定完了まで維持・import開始時のin-flight拒否)**・**run state不変の適用範囲を `continuing` 開始までへ限定**・**Scopeのsummary要約をglobal preference表示と統一** を行った。attempt anchor・CTA single-flight・CTA処理中の破棄/Back不受理・strategy変更との相互排他 (commit時gate/書込開始gate/attach前owning runId再照合)・CEを含むseam例外契約・summary内訳4種 + global preference・process death分岐は前revisionで固定済み。未決のproduct decisionは残っていない。
...
And **commit時gateのgate読取 → dismiss/start実行は原子的である**: strategy書込のcommit後のrestart判定・実行はMain-confined直列化点上へ移し、`strategyWriteInFlight` はrestart判定完了まで維持する。import開始側も同一直列化点を通るため、「W側がgateを読んで可と判断した直後、restart実行前にimportが開始し、attemptが内部競合で黙ってdropされる」競合窓が存在しない (import開始とrestart判定は必ず全順序を持つ。importが先ならstrategy restartは抑止され、restart判定が先ならimport開始はin-flight拒否される)、
...
And **UIのdisabledに依存しないcommit時gateが存在する**: strategy書込が (a) import continuation中 (両entry)、または (b) run内entryではimport attempt生存中の全期間 (validation中・成功状態表示中を含む。`continuing` の有無によらない) にcommitした場合でも、commit時のrun active判定による `dismiss()` → `start(trigger)` は発火しない (strategyのcommit自体は有効で次回以降のrunに反映される。imported run・owning runを黙って差し替えない)。**restart判定・実行はMain-confined直列化点上で行われ、`strategyWriteInFlight` は判定完了まで維持される** — gate読取とrestart実行の間にimport開始が割り込む競合窓は存在しない、
...
- **strategy書込との相互排他 (原子的直列化)**: strategy選択変更の書込 (既存 `onStrategySelected` → `store.select` → commit → run判定 → `dismiss()`/`start(trigger)`) とimportの開始・継続は直列化する。holder/hosting間で共有するstateは2つ: `importContinuationActive` (CTA flipからsettle終端まで) と `strategyWriteInFlight` (**書込開始からrestart判定完了まで** — commit後の判定・実行まで含む)。**commit後のrestart判定・実行はMain-confined直列化点上で実行し**、直列化点上で「gate読取 (`importContinuationActive` または run内 `importAttemptActive`) → dismiss/start」を原子的に閉じる (gate読取とrestart実行の間にMain側のimport開始が割り込めない)。**import開始側も同一直列化点を通り**、`strategyWriteInFlight` 中は新規importの開始自体をtypedに拒否する (再試行可)。これによりimport開始とstrategy restartの相互作用は必ず全順序を持ち、「gate通過後にrunを差し替えられる」競合窓および「内部競合による黙ってdrop」を排除する。**書込開始gate**: `onStrategySelected` はrun内entryの `importAttemptActive` 中およびidle entryの `importContinuationActive` 中は新しい書込を開始しない。いずれもUI disabledとは独立したstate gateである。
