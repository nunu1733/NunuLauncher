...
> Status: **draft** (2026-09-18 re-entry revision 4)。**D-2/D-3 (およびOpen question 3) はowner decisionとして確定済み** ([Issue コメント](https://github.com/nunu1733/NunuLauncher/issues/328#issuecomment-5723309396)、本spec「Decisions」に具体化)。本revisionで **strategy変更とimport continuationの相互排他 (idle含む・UI非依存のcommit時gate)**・**`CancellationException` を含むseam例外契約の精緻化**・**global preference (`minimizeMovement`) のplanner-effective表示** を追加固定。import validation自体のattempt anchor (採番はimport開始時、owning runId込み)・CTA single-flight・CTA処理中の破棄/Back不受理・summary内訳4種V1必須・process deathのcontinuing分岐は前revisionで固定済み。未決のproduct decisionは残っていない。
...
And **strategy変更とimport continuationは相互排他の競合操作** である: idle CTA押下で `run.start` が実行されると、settle前にrunはactive (`State.Capturing` → `CandidateDetection`/`Selecting`) となるため、処理中のstrategy選択変更は既存の `dismiss()` → `start(trigger)` pathで **validated intentを持たない別run** へ差し替えられ得る。よって (1) **idle entryも `continuing` の間はstrategy pickerを無効化** し (「idle pickerは常に操作可」ではない)、(2) UIのenabledに依存しない **commit時gate** を置く: CTA開始前に実行されたstrategy書込が後からcommitしても、import continuationがactiveな間はcommit時のrun判定による `dismiss()` → `start(trigger)` を **発火させない** (strategy自体のcommitは無効化しない — 次回以降のrunに反映)、(3) **strategy書込がin-flightの間はCTAを開始しない** (typed案内で再試行)。いずれもUI表示だけでなくstate上のgateで保証する。
...
And **UIのdisabledに依存しないcommit時gateが存在する**: CTA開始前に実行されたstrategy書込がimport continuation中に遅延commitした場合でも、commit時のrun active判定による `dismiss()` → `start(trigger)` は発火しない (strategyのcommit自体は有効で次回以降のrunに反映される。imported runを黙って差し替えない)、
...
- **strategy書込との相互排他**: strategy選択変更の書込 (既存 `onStrategySelected` → `store.select`) とimport continuationは直列化する。holder/hosting間で共有するstateは2つ: `importContinuationActive` (CTA flipからsettle終端まで) と `strategyWriteInFlight` (書込開始からcommit/no-opまで)。commit時gateは `importContinuationActive` 中の `dismiss()` → `start(trigger)` 発火を阻止し、CTA側は `strategyWriteInFlight` 中の開始を拒否する。いずれもUI disabledとは独立したstate gateである。
...
- [ ] AC-5: CTAを押さずに閉じる場合のlifecycleがD-2確定内容 (明示ボタン「破棄して閉じる」は追加確認なし、system Backは確認dialog) どおり実装される: 破棄が明示的であること、exchange sessionがinvalidateされないこと (破棄後に同じ回答textの再取り込みが成立すること)、競合affordance (idle「整理を開始」row、run内strategy pickerのrun差し替え) が **import attempt生存中の全期間** (validation中の `Importing` を含む) 無効化されること、**idle entryのstrategy pickerが `continuing` 中に無効化されること**、**strategy書込の遅延commitがimport continuation中に `dismiss()`/`start(trigger)` を発火させないこと** (commit時gate・UI非依存)、**`strategyWriteInFlight` 中のCTA開始が拒否されること** がtestされる。CTA処理中の破棄拒否 (`discardImport()` 不受理) を含む。
...
| AC-5 | holder unit test (discard → session invalidate未呼出、再取り込み成立の往復test、破棄後pending intent消失、**continuing中のdiscard不受理**) + idle競合row無効化UI test (**validation中含む**) + run内strategy picker無効化UI test (**validation中含む**) + **idle pickerのcontinuing中無効化UI test** + **commit時gate test** (strategy書込の遅延commit → continuation中にdismiss/start非発火) + **`strategyWriteInFlight` 中のCTA拒否test** + system Back確認dialog経路 (確定で破棄・キャンセルで成功状態維持) test |
