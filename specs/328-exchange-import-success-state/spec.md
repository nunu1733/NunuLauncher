...
> Status: **draft** (2026-09-18 re-entry revision 7)。**D-2/D-3 (およびOpen question 3) はowner decisionとして確定済み** ([Issue コメント](https://github.com/nunu1733/NunuLauncher/issues/328#issuecomment-5723309396)、本spec「Decisions」に具体化)。本revisionで **restart実行をMainで行わないreservation方式 (Main上では判定とreservation取得のみを原子的に、dismiss/start実行はIO・全終端でfinally解除)**・**strategy書込とimportの排他policyに沿ったAC-3(e)等のoracle再編 (in-flight中import拒否と旧「in-flight中import成功」oracleの矛盾解消)**・**「summaryは件数のみ」の残存表記統一** を行った。attempt anchor・CTA single-flight・CTA処理中の破棄/Back不受理・strategy変更との相互排他 (commit時gate/書込開始gate/attach前owning runId再照合)・CEを含むseam例外契約・summary内訳4種 + global preference・process death分岐は前revisionで固定済み。未決のproduct decisionは残っていない。
...
And **commit時gateは「Main上で判定とreservation取得まで原子的、実行はIO」の2段構成である**: strategy書込のcommit後、restart可否の判定とreservation取得をMain-confined直列化点上で原子的に実行し、`dismiss()` → `start(trigger)` の実行はIO上で行う (startは同期heavy — Mainで実行しない)。reservation中は `strategyWriteInFlight` (またはreservation flag) が立っており、import開始・CTAともtypedに拒否される。reservation/in-flightはrestart成功・非commit・`start()` throw・coroutine cancelの全終端で解除される。import開始とrestart判定は必ず全順序を持ち (importが先ならrestart抑止、判定が先ならimport拒否)、「W側がgateを読んで可と判断した直後、restart実行前にimportが開始し、attemptが内部競合で黙ってdropされる」競合窓が存在しない、
...
- **strategy書込との相互排他 (原子的直列化)**: strategy選択変更の書込 (既存 `onStrategySelected` → `store.select` → commit → run判定 → `dismiss()`/`start(trigger)`) とimportの開始・継続は直列化する。holder/hosting間で共有するstateは2つ: `importContinuationActive` (CTA flipからsettle終端まで) と `strategyWriteInFlight` (**書込開始からrestart判定完了まで** — commit後の判定・実行まで含む)。**commit後のrestart処理は2段に分かれる**: (i) Main-confined直列化点上で **restart可否の判定とreservation取得までを原子的に実行** (gate読取: `importContinuationActive` または run内 `importAttemptActive` — いずれでもなければ `restartReserved` を取得。判定と取得の間にimport開始は割り込めない)、(ii) `dismiss()` → `start(trigger)` の **実行はIO上で行う** (startは同期heavyのため、Mainでは実行しない)。reservation中・IO実行中は `strategyWriteInFlight` が立ったまま (またはreservation flag) であり、**import開始側も同一直列化点を通って `strategyWriteInFlight`/reservation 中は新規importの開始自体をtypedに拒否する** (再試行可)。import開始とrestart判定が同一直列化点で全順序を持ち、「gate通過後にrunを差し替えられる」競合窓および「内部競合による黙ってdrop」を排除しつつ、Main上で同期heavy seamを実行しない。**reservation/in-flightの解除は全ての終端で行う** (restart成功・非commit・`start()` throw・coroutine cancel — `finally` 相当でMainへ戻して解除し、import/CTA拒否を残留させない)。**書込開始gate**: `onStrategySelected` はrun内entryの `importAttemptActive` 中およびidle entryの `importContinuationActive` 中は新しい書込を開始しない。いずれもUI disabledとは独立したstate gateである。
- **run内entryのattempt context**: import開始時にcaptureしたowning runIdはattempt tokenとともに成功状態へ保持され、validation settle時・`attachIntent` 呼出直前の双方で `State.Selecting.runId` と照合される。不一致時は成功状態を表示しない / `attachIntent` を呼ばずtyped failure settleとする。
...
- [ ] AC-3: 次のOrganizer操作へ進む主要CTAが存在し、CTAで既存seam経由の接続が **一度だけ** 行われることがtestされる (idle: `start(trigger, intent)`、run内: `attachIntent`)。CTAのsingle-flightがtestされる: 二重押下でseamが複数回呼ばれないこと、および成功状態が閉じた後の遅延settleが表示へ適用されないこと。settleのanchorがattempt tokenであることが **同一 `validated.identity` の再import (ABA) test** で固定される: 旧attemptの遅延settleが、同一identityで作り直された新しい成功状態attemptへ適用されないこと。CTA処理中 (`continuing`) の破棄・Backが不受理であることがtestされる (seam開始 → settle前に破棄/Back試行 → 不受理で成功状態維持、seamは影響を受けずsettleへ進行)。**seam例外時** もtestされる: (a) fake seamが非`CancellationException` をthrow → 処理中解除・再試行/破棄可能に復帰・run abort済み・旧attempt誤settleなし、(b) fake seamが **`CancellationException` をthrowするがholder scopeは生存** → 同様にfailure settleへ落ち `continuing = false` で復帰する (取り残しなし)、(c) holder scope自体をcancelした場合はUI settleを要求しない。**strategy変更との相互排他** もtestされる: (d) idle CTAでfake `run.start` をdetection/planning中で停止させ、strategy変更試行 → `dismiss()`/`start(trigger)` が非発火でvalidated runが維持されること、(d2) **原子的直列化 (確定policy)**: blocking fakeで「strategy書込のrestart判定がgate読取を通過しようとする瞬間」と「import開始」を競合させ、**import開始が先に成立した場合はowning runがdismiss/restartされないこと (restart抑止)、restart判定・reservationが先に成立した場合はimport開始がtypedに拒否されること** (全順序保証。どちらの順序でも誤attach・黙dropが起きない)、(d3) `strategyWriteInFlight`/reservation 中の新規import開始がtypedに拒否されること、(d4) **reservation/解除**: fake `start` をIO上で停止中はimport・CTAがtyped拒否されること、restart成功時と `start()` throw時の双方でreservation/in-flightが解除され (finally相当) import・CTAが再開可能になること、(e) **run内のpre-existing書込とimportは重ならない (確定policy)**: R1 `Selecting` → strategy書込W開始 (in-flight) → import試行 → **typed拒否・validation未開始** → Wのrestart判定/必要ならIO上でrestart完了 → in-flight解除 → import再試行 → 現在のowning runで `ImportSuccess` 表示、(f) `strategyWriteInFlight` 中のCTA開始が拒否されること、(g) continuation flip後の直接 `onStrategySelected` 呼出で書込もrestartも開始されないこと (書込開始gate)、(h) **attach前のowning runId再照合**: 成功状態表示中にowning runIdのrunが差し替えられた状態 → CTAで `attachIntent` を呼ばずtyped failure settleになること。CTA時のgate拒否 (`Busy` / `NotAttachable`) は処理中状態を解除し取り込み成功状態を維持したままtyped案内され、解除後に破棄・Backが再度可能であることがtestされる。
...
- [ ] AC-5: CTAを押さずに閉じる場合のlifecycleがD-2確定内容 (明示ボタン「破棄して閉じる」は追加確認なし、system Backは確認dialog) どおり実装される: 破棄が明示的であること、exchange sessionがinvalidateされないこと (破棄後に同じ回答textの再取り込みが成立すること)、競合affordance (idle「整理を開始」row、run内strategy pickerのrun差し替え) が **import attempt生存中の全期間** (validation中の `Importing` を含む) 無効化されること、**idle entryのstrategy pickerが `continuing` 中に無効化されること**、**commit時gateがrun内entryのimport attempt全期間 + 両entryのimport continuation中に効くこと** (strategy書込の遅延commit → `dismiss()`/`start(trigger)` 非発火。run内の「W開始 → import成功 → W commit → R1維持」を含む)、**`strategyWriteInFlight`/reservation 中のCTA開始が拒否されること**、**`strategyWriteInFlight`/reservation 中の新規import開始が拒否されること**、**restart判定のMain-confined直列化とIO実行分離** (blocking fakeでgate読取直後とimport開始の競合 → 全順序保証。`dismiss()`/`start(trigger)` はIO上で実行されること)、**reservation/in-flightの全終端解除** (restart成功・非commit・`start()` throw・cancelで解除され拒否が残留しないこと)、**書込開始gate** (run内attempt中/idle continuation中の `onStrategySelected` が新しい書込を開始しないこと) がtestされる。CTA処理中の破棄拒否 (`discardImport()` 不受理) を含む。
...
| AC-3 | holder unit test: CTA → seam呼出 (`Started`/`Attached`)、`Busy`/`NotAttachable` で状態維持・処理中解除。**二重押下でseam 1回のみ** (continuing flag guard)、**遅延settleが閉じた/置き換わった成功状態へ適用されない** (attempt token anchor)、**ABA: 同一identityで再importした新attemptに旧attemptの遅延settleが適用されない**、**CTA処理中の破棄/Back不受理**、**seam例外** (非CE throw → failure settle / **CE throwだがscope生存 → failure settle** (取り残しなし) / scope cancel → UI settle要求なし)、**strategy相互排他** (blocking fakeで (d) idle CTAの `start` をdetection/planning中に停止 → strategy変更 → dismiss/start非発火、(d2) restart判定とimport開始の競合 → 確定policyどおり全順序保証 (import先ならrestart抑止 / 判定先ならimport拒否)、(d3) in-flight/reservation中のimport開始typed拒否、(d4) reservation解除 (restart成功・throw双方でfinally解除)、(e) **pre-existing書込はimportと重ならない**: W開始 → import試行 → typed拒否・validation未開始 → W restart判定/完了 → 解除 → import再試行 → owning runでImportSuccess、 **run内pre-existing書込: W開始 → import成功 → ImportSuccess(R1) → W commit → R1維持 → CTAでR1へattach成立**、(f) `strategyWriteInFlight` 中のCTA拒否、(g) continuation後の直接 `onStrategySelected` 呼出 → 書込/restartとも開始されない、(h) attach前owning runId再照合 → 不一致でattachせずfailure settle)、**runId不一致settle** (`Started.runId` ≠ 現在runのrunId → failure settle扱い)。`ManualOrganizationRunTest` 連携 |
...
| AC-5 | holder unit test (discard → session invalidate未呼出、再取り込み成立の往復test、破棄後pending intent消失、**continuing中のdiscard不受理**) + idle競合row無効化UI test (**validation中含む**) + run内strategy picker無効化UI test (**validation中含む**) + **idle pickerのcontinuing中無効化UI test** + **commit時gate test** (run内attempt全期間 + 両entrycontinuation中の遅延commit → dismiss/start非発火。「W開始 → import成功 → W commit → R1維持」含む) + **`strategyWriteInFlight` 中のCTA拒否test** + **書込開始gate test** (run内attempt中/idle continuation中の `onStrategySelected` が書込を開始しない) + system Back確認dialog経路 (確定で破棄・キャンセルで成功状態維持) test |
