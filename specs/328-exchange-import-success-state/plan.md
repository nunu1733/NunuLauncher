...
- **`ui/preferences/destinations/ManualOrganizationPreferences.kt` — hosting調整 (Back interception + 競合freeze)**:
  ...
  - **strategy書込との相互排他gate (UI非依存)**: holderとhostingが共有するstate 2点を追加する。**(a) `importContinuationActive`** — holderのpredicateで、CTA flipからsettle終端までtrue。**(b) `strategyWriteInFlight`** — hostingのpredicateで、`execute { store.select(id) }` の開始からcommit/no-opまでtrue。この2つでhosting/holder側の4つのgateを実装する:
    - **commit時gate (原子的)**: `onStrategySelected` のcommit後、**restart判定・実行をMain-confined直列化点 (`withContext(uiDispatcher)` 相当) へ移し**、同一点上で `importContinuationActive` (両entry) **または run内entryでは `exchangeHolder.importAttemptActive`** を読み、いずれかがtrueなら `dismiss()` → `start(trigger)` を発火しない (strategyのcommit自体は有効で次回以降のrunに反映)。判定と実行が同一直列化点上にあるため、「gate読取 → restart実行」の間にMain側のimport開始が割り込む競合窓が存在しない。run内の「import開始前に始まった書込Wが成功状態表示中 (`continuing=false` を含む) にcommitしてowning runを差し替え、CTAが別runへintentをattachする」競合と、idleの「CTA直前に始まった書込がcontinuation中にcommitする」競合の双方を閉じる。
    - **`strategyWriteInFlight` のlifetime**: 書込開始から **commit後のrestart判定完了まで** 維持する (commitで解除しない)。解除はMain-confined直列化点上の判定完了後とする。
    - **import開始拒否**: holderの `import()` / `receiveAndImport()` は開始時に `strategyWriteInFlight` を検査し、trueなら新規importの開始自体をtypedに拒否する (再試行可。envelope gate以前の入口で検査 — validation coroutineを起動しない)。pre-existing書込とimport attemptの重なりを起点で排除する。
...
import (clipboard/file/貼付 → 共通receipt path、#332実装済み)
  ...
  └─ Validated (token current + run-inはowning runIdがSelecting保持) → summary導出 (pure、
       ...
       └─ CTA (single-flight: 同期continuing flip → IO seam呼出 → attempt token anchored settle)
             ...
             │     strategy書込がcommitしても dismiss/start は発火しない。書込開始gateも有効)
...
| AC-5 | holder unit test: discard → invalidate未呼出・再取り込み往復成立・pending intent消滅・**continuing中のdiscard不受理**。idle競合row無効化 (**validation中含む**) + run内strategy picker凍結 (**validation中含む**) + **idle pickerのcontinuing中無効化** + **commit時gate** (run内attempt全期間 + 両entrycontinuation中の遅延commit → dismiss/start非発火。「W開始 → import成功 → W commit → R1維持」含む) + **`strategyWriteInFlight` 中のCTA拒否** + **`strategyWriteInFlight` 中の新規import開始拒否** + **restart判定のMain-confined直列化** (blocking fakeでgate読取直後とimport開始の競合 → 全順序保証) + **書込開始gate** (run内attempt中/idle continuation中の `onStrategySelected` が書込を開始しない) のUI test + system Back確認dialog経路 (確定で破棄・キャンセルで維持) test | unit + instrumentation |
...
- strategy相互排他gateの実装漏れ (commit時gateの検査忘れ、`strategyWriteInFlight` の解除忘れ、restart判定のMain-confined移行漏れ) はimported run差し替えの再発・内部競合によるattempt黙drop・CTA/import永久拒否になる — AC-3/AC-5のblocking fake testで両方向と全順序保証を検証する。strategyWriteInFlightのlifetime延長により、書込中はimport・CTAとも開始できない短い窓が生じる (typed案内で再試行を案内。書込は通常即時完了)。
...
- [ ] summary純粋関数とholder遷移の失敗testを先行追加 (canonical計数: bare/omission、内訳4種算式、global-only intent、single-flight: 二重押下・遅延settle・ABA、validation段階のattempt anchor: cancel後遅延drop/A→B順不同/owning runId不一致、continuing中の破棄/Back不受理、seam例外settle: 非CE/CE-scope生存/scope cancel、strategy相互排他: idle CTA中restart非発火/run内pre-existing書込W→success→commit→R1維持/in-flight CTA拒否/書込開始gate/attach前owning runId再照合/restart判定とimport開始の全順序保証/in-flight中import開始拒否)。
- [ ] Minimal implementation (attempt token採番/照合 + ImportSuccess (token継承) + single-flight CTA (例外failure settle・strategyWriteInFlight拒否・runId一致確認込み) + discard (continuing guard) + `importAttemptActive`/`importContinuationActive` + 画面level Back interception (continuing中no-op/非continuing確認dialog) + strings (D-2/D-3確定文言・内訳4種・全体方針行・seam例外typed status) + 競合row/picker無効化 (validation中含む・idle pickerはcontinuing中) + commit時gate (Main-confined直列化点・strategyWriteInFlight lifetime延長) + import開始時のin-flight拒否 + strategy select/restart arbiter seam)。
