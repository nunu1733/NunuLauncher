...
> Status: draft — spec D-2/D-3 (およびOpen question 3) はowner decision確定済み ([Issue コメント](https://github.com/nunu1733/NunuLauncher/issues/328#issuecomment-5723309396))。spec承認を待ってimplementation-ready。
...
- **`organizer/ui/exchange/ExchangeFlowUi.kt` — 取り込み成功状態**:
  - `ExchangeScreen.ImportSuccess(summary, validated, entryKind, continuing: Boolean = false, generation: Long)` を追加 (`entryKind`: idle / run内)。holderはvalidated intentを **process-localで一時保持** (新規永続化なし)。
  - holder内にprocess-localな **単調増加generation counter** を持たせ、`ImportSuccess` stateを生成するたびに新しいtokenを採番する。**settle anchorはこのtoken** である。`validated.identity` はcontent digest (#330 D-5 — 同一semantic内容の再importで同一値) でありattempt区別に使えないためanchorにしない。counterへのアクセスは全てuiDispatcher上のscreen遷移と同じ場所に置き (Main-confined)、競合調整を不要にする。
  ...
  - 新規操作 **`continueImport()` — single-flight**:
    ...
    4. settleは `uiDispatcher` へ戻り、**開始時にflipした `ImportSuccess` stateのgeneration token** に紐付く: settle時の `screen` が **同じgeneration token** を持ち `continuing` のときのみ適用。token不一致 (閉じた/置き換わった/同一identityの回答で作り直された成功状態) の遅延settleはdrop (`settleBelongsTo` と同じ構造。exportIdがdisclosure generationごとに一意であるのに対し、identityはcontent digestで再importで再現されるため、tokenが必須)。
  ...
  - 成功状態composable: heading (成功、または判断なし合算 > 0ならwarning語彙) + summary行 (plurals) + 「未適用」明示行 + CTA (`Button`、`enabled = !continuing`、文言はD-3確定: idle「この提案で整理を進める」/ run内「選択に戻って整理を確定する」) + 破棄 (`OutlinedButton`「破棄して閉じる」、`enabled = !continuing`) + 再取り込み案内 (破棄時)。testTag・live region・focus (既存 `FocusTargetText`/`liveRegion` パターン準拠)。
...
import (clipboard/file/貼付 → 共通receipt path、#332実装済み)
  ...
  │     (generation token採番 — settle anchor)
  ...
  └─ CTA (single-flight: 同期continuing flip → IO seam呼出 → generation token anchored settle)
        ├─ idle: run.start(trigger, intent) / run内: run.attachIntent — 各1回のみ
        │     ├─ 成功 → Closed → 既存flow (選択 → planning → preview → confirm → apply)
        │     ├─ Busy/NotAttachable → continuing解除 + typed status (成功状態維持・再試行可・破棄/Back再可)
...
- **CTAを素通しでIO coroutineに投げる** (review Required 4): 複数回押下で複数coroutineが同じ成功状態を読み、`start`/`attachIntent` を複数回呼ぶ。settle順により成功後にstale `RUN_BUSY` が残る等のUI競合。`continuing` flagの同期check-and-set + generation token anchored settleで構造的に排除。
- **`validated.identity` をsettle anchorにする** (前回revision案、2nd review Required 2): identityはcompleted表現へのcontent digest (#330 D-5) であり、同一semantic内容の再importは同一identityになる。旧attempt Aと同一内容の新attempt Bが併存し得るABA形競合で、`identity一致 && continuing` ではAの遅延settleをBと区別できない (既存 `settleBelongsTo` が安全なのはexportIdがdisclosure generationごとに一意なため)。`ImportSuccess` 生成ごとに採番するprocess-local単調増加generation tokenへ変更。
...
| `organizer/ui/exchange/ExchangeFlowUi.kt` | `ImportSuccess(summary, validated, entryKind, continuing, generation)` state、generation counter採番、`continueImport` (single-flight + token anchored settle) / `discardImport` (continuing中不受理)、成功composable (D-3確定文言・破棄label)、`import()` Validated分岐変更、`IMPORT_ACCEPTED` 即時表示廃止 | exchange flow所有のUI state machine。single-flightは `beginTransport`/`settleBelongsTo` と同一ファイルの既存パターンに準拠 |
| `ui/preferences/destinations/ManualOrganizationPreferences.kt` | 画面level Back interception (成功state表示中のみenabled、`ManualOrganizationBackHandler` より後に登録、continuing中はBack取り込みno-op、非continuingは確認dialog)、idle競合row無効化、run内strategy picker凍結 | Back保証・競合排除はhosting画面の常時compositionが正しい場所 (lazy item内不採用) |
...
| AC-3 | holder unit test: CTA → seam呼出 (`Started`/`Attached`)、`Busy`/`NotAttachable` でcontinuing解除+状態維持。**二重押下 (continuing中の再呼出) でseam 1回のみ**、**閉じた成功状態への遅延settle drop** (既存 `lateResultFromAnOldWriteCannotTouchANewerDisclosure` と同型test)、**ABA: 同一identityの回答を再importした新attemptに旧attemptの遅延settleが適用されない** (generation token不一致drop)、**CTA処理中の破棄/Back不受理** (settle前のdiscard試行が不受理・成功状態維持・seamは継続settle、settle解除後に破棄可)。`ManualOrganizationRunTest` 連携 | 同上 |
...
| AC-5 | holder unit test: discard → invalidate未呼出・再取り込み往復成立・pending intent消滅・**continuing中のdiscard不受理**。idle競合row無効化 + run内strategy picker凍結のUI test + system Back確認dialog経路 (確定で破棄・キャンセルで維持) test | unit + instrumentation |
...
含める観察: unit (state machine遷移・summary導出・discard往復・single-flight・continuing中の破棄/Back不受理・ABA遅延settle分離)、contract (summary非混入、failure表示regression)、failure injection (CTA時Busy/NotAttachable、二重押下、遅延settle、破棄→再取り込み)、a11y/large font (画面外Back含む)、e2e device evidence。
...
- generation counterはprocess-localのUI stateであり、永続化・復元の対象にしない (process death時は成功状態ごと消失、回復は再取り込み — spec scenarioどおり)。採番・比較は全てuiDispatcher上のMain-confinedな遷移に置き、synchronized/atomicを導入しない (現行holderのscreen遷移と同じ実行model)。
- `continuing` 中のBack取り込み (no-op) は、settleまでの短時間窓で「Backが反応しない」体感を生む — CTA/破棄のdisabled表示と同時に示されるため混乱は限定的。settleは `start` 完了または `Busy`/`NotAttachable` 拒否のいずれかで必ず到達する (seamが例外で抜けた場合もsettle相当の解除を入れる — 実装時に確認)。
...
- [ ] summary純粋関数とholder遷移の失敗testを先行追加 (canonical計数: bare/omission、single-flight: 二重押下・遅延settle・ABA、continuing中の破棄/Back不受理)。
- [ ] Minimal implementation (ImportSuccess + generation token + single-flight CTA + discard (continuing guard) + 画面level Back interception (continuing中no-op/非continuing確認dialog) + strings (D-2/D-3確定文言) + 競合row/picker無効化)。
