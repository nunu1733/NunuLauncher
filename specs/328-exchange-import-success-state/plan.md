...
- `lawnchair/src/app/lawnchair/ui/preferences/destinations/ManualOrganizationPreferences.kt`
  ...
  - `onStrategySelected` (L235–254): store書込が `Committed` かつrun active (`Idle`/`Cancelled` 以外) のとき `coordinator.dismiss()` → `coordinator.start(trigger)` (**実行中runを取り消して新runで差し替える**)。radio再選択 (同一strategy) はno-op。**書込は `execute { store.select(id) }` の非同期path** (`execute` = `scope.launch { withContext(Dispatchers.IO) { action() } }`、L208–212) であり、開始とcommitの間に時間差がある — CTA開始後にcommitした書込もcommit時点のrun判定でdismiss/restartを発火し得る。**commit後のrun判定・restart実行もIO上で動く** ため、gate読取とrestart実行の間にMain-confinedなimport開始が割り込む競合窓が現行構造に存在する (Designの原子的直列化で閉じる)。
...
- **`organizer/ui/exchange/ExchangeFlowUi.kt` — 取り込み成功状態 + import attempt anchor**:
  ...
  - 新規操作 **`continueImport()` — single-flight**:
    ...
    3. `scope.launch(Dispatchers.IO)` 内で既存seamを実行 (run内: `run.attachIntent(validated)` / idle: `run.start(trigger, intent)`。`start` は同期 heavy なのでIO wrapperは現行 `import()` と同一の監査対応)。**run内entryでは `attachIntent` 呼出直前にowning runIdを再照合する** — `(run.state as? State.Selecting)?.runId` がattempt contextに保持したowning runIdと一致しない場合 (成功状態表示中のrun差し替え等)、`attachIntent` を呼ばずtyped failure settleへ落ちる (処理中解除・破棄/再取り込み可能。既知のfreezeだけにrun identity保証を依存しない)。**seam呼出の直前に追加するguardはこれのみ** — `continuing` 中は破棄・Backが不受理 (次項) のため、開始されたseamが途中で撤回される経路が存在しないことが構造で保証される。seam呼出は **try/catchで包む**: catchした例外に対し、まず **呼出coroutine自体のcancel判定** (`currentCoroutineContext().ensureActive()` 相当) を行い、cancel済みなら例外をそのまま再伝播させる (生存scopeのUI stateは触らない)。contextが生存している場合は **`CancellationException` を含む全ての例外** をsettle時点のattempt token照合を通してfailure settleへ変換する (例外前に `start` が `abort(operation)` を完了するためrun側はzero-writeに復帰、`attachIntent` はbind前に判定してrefuseするためbind残骸もない)。seam内部発CEで `continuing = true` に取り残される経路を構造で排除する。
...
- **`ui/preferences/destinations/ManualOrganizationPreferences.kt` — hosting調整 (Back interception + 競合freeze)**:
  ...
  - **strategy書込との相互排他gate (単一のarbiter状態機械・UI非依存)**: holderとhostingが共有するarbiter状態は **単一の状態機械** `Idle → Writing → RestartReserved/Restarting → Idle`:
    - **`Writing`**: `execute { store.select(id) }` の呼出〜commit/no-op判定。開始時 (`onStrategySelected` 内) に書込開始gateを検査し、run内entryの `importAttemptActive` 中またはidle entryの `importContinuationActive` 中は **書込自体を開始しない** (UIのrecompose遅延・queued click等をUI disabledだけに依存せず遮断)。
    - **`RestartReserved` (原子的取得)**: commit後、**Main-confined直列化点 (`withContext(uiDispatcher)` 相当) 上で** `importContinuationActive` (両entry) **または run内entryでは `exchangeHolder.importAttemptActive`** を読み、いずれでもなければrestart権を取得する — 判定と取得は同一点上で原子的であり、その間にMain側のimport開始は割り込めない。いずれかのgateがtrueならreservationを取得せずrestartを抑止する (strategyのcommit自体は有効で次回以降のrunに反映)。
    - **`Restarting` (IO実行)**: reservation済みの場合のみ、`coordinator.dismiss()` → `coordinator.start(trigger)` を **IO上で実行する** (`start` は同期heavyのためMainでは実行しない — 現行UIもIO wrapperで呼ぶ)。
    - **拒否と解除**: `Writing` / `RestartReserved` / `Restarting` の全期間、holderの新規import開始 (`import()` / `receiveAndImport()`、envelope gate以前の入口で検査 — validation coroutineを起動しない) と `continueImport()` (step 1) をtypedに拒否する。**arbiterの解除は全ての終端で `finally` 相当から行う** (restart成功 / 非commit / `start()` throw / coroutine cancel — 拒否が残留しない)。
    - run内の「import開始前に始まった書込Wが成功状態表示中 (`continuing=false` を含む) にcommitしてowning runを差し替え、CTAが別runへintentをattachする」競合、idleの「CTA直前に始まった書込がcontinuation中にcommitする」競合、および「gate読取とrestart実行の間にimport開始が割り込む」競合を、この単一状態機械で閉じる。deterministicなtestのため **strategy select/restartのarbiter (store呼出とrestart判定) を注入可能な小さなseamへ切り出す** (現行の `LayoutStrategySelectionModule.store(context)` 直呼出しはhard-wiredのため)。
...
import (clipboard/file/貼付 → 共通receipt path、#332実装済み)
  ...
  └─ Validated (token current + run-inはowning runIdがSelecting保持) → summary導出 (pure、
       ...
       └─ CTA (single-flight: 同期continuing flip → IO seam呼出 → attempt token anchored settle)
             ...
             │    (run内はimport attempt生存中の全期間 + 両entrycontinuation中、
             │     strategy書込がcommitしても dismiss/start は発火しない — Main上で
             │     reservation取得まで原子的、実行はIO。書込開始gateも有効)
...
|---|---|---|
| `organizer/personalization/exchange/` (新規summary file) | `ExchangeImportSummary` + 純粋導出関数 (入力 = `validated.completed` + `session` + `validated.intent.globalPreference?.minimizeMovement` のplanner-effective boolean 1値。Design/AC-4と同一) | canonical基準のprivacy-safe summary (item計数 + global preference)、pure test |
...
| `ui/preferences/destinations/ManualOrganizationPreferences.kt` | 画面level Back interception (成功state表示中のみenabled、`ManualOrganizationBackHandler` より後に登録、continuing中はBack取り込みno-op、非continuingは確認dialog)、idle競合row無効化 (**validation中含む** — `importAttemptActive` 連動)、run内strategy picker凍結 (**validation中含む**)、idle picker凍結 (**continuing中**)、`strategyWriteInFlight` 管理、`onStrategySelected` への書込開始gate (run内attempt中/idle continuation中は書込を開始しない) とcommit時gate (2段構成: Main-confined直列化点上で `importContinuationActive` または run内 `importAttemptActive` を読み **restart reservation取得まで原子的に**、`dismiss()`/`start(trigger)` の実行はIO上 — `start` は同期heavyのためMainで実行しない。`strategyWriteInFlight`/reservation はIO実行完了・非commit・throw・cancelの全終端で `finally` 解除)、strategy select/restart arbiterの注入可能seam化 (deterministic test用) | Back保証・競合排除はhosting画面の常時compositionが正しい場所 (lazy item内不採用)。strategy書込との相互排他は書込のcommit pathがhostingに存在するため |
...
|---|---|---|
| AC-1 | holder unit test: validated → `ImportSuccess`、`start`/`attachIntent` 未呼出、run state不変 (**CTA押下前**)。**blocking fake import**: (a) import → cancel → 遅延Validated → 成功状態非出現、(b) import A → import B の逆順settle → B (最新) のみが成功状態・Aはdrop、(c) run-inでsettle時owning runId不一致 → drop。hosting instrumentation test | `./gradlew :lawnchair:testLawnchairGithubDebugUnitTest` (既存exchange test suite拡張) |
| AC-2 | string解決 + 表示test (unit/instrumentation) | 同上 |
| AC-3 | holder unit test: CTA → seam呼出 (`Started`/`Attached`)、`Busy`/`NotAttachable` でcontinuing解除+状態維持。**二重押下 (continuing中の再呼出) でseam 1回のみ**、**閉じた成功状態への遅延settle drop** (既存 `lateResultFromAnOldWriteCannotTouchANewerDisclosure` と同型test)、**ABA: 同一identityの回答を再importした新attemptに旧attemptの遅延settleが適用されない** (attempt token不一致drop)、**CTA処理中の破棄/Back不受理**、**seam例外** ((a) 非CE throw → failure settle、(b) **CE throwだがscope生存** → failure settleでcontinuing解除・取り残しなし、(c) scope自体をcancel → UI settle要求なし)、**strategy相互排他** (blocking fakeで (d) idle CTAの `start` をdetection/planning中に停止 → strategy変更試行 → dismiss/start非発火・validated run維持、(d2) **排他の2方向** — (i) W先行: arbiter状態 (`Writing`/`RestartReserved`/`Restarting`) 全期間でimport開始typed拒否・書込とimportの重なり成功なし、(ii) import先行: 書込開始gateでW自体が開始せずowning run維持、(d3) arbiter状態中のCTA開始・import開始typed拒否、(d4) **arbiter解除 table-driven** (restart成功 / 非commit / `start()` throw / cancel の全終端でfinally解除され再開可能)、(e) **pre-existing書込はimportと重ならない**: W開始 → import試行 → typed拒否・validation未開始 → W restart判定/IO restart完了 → 解除 → import再試行 → owning runでImportSuccess、(f) `strategyWriteInFlight` 中のCTA開始拒否、(g) continuation後の直接 `onStrategySelected` 呼出 → 書込/restartとも開始されない、(h) attach前owning runId再照合 → 不一致でattachせずfailure settle)、**runId不一致settle** (`Started.runId` ≠ 現在runのrunId → failure settle扱い)。`ManualOrganizationRunTest` 連携 | 同上 |
...
| AC-5 | holder unit test: discard → invalidate未呼出・再取り込み往復成立・pending intent消滅・**continuing中のdiscard不受理**。idle競合row無効化 (**validation中含む**) + run内strategy picker凍結 (**validation中含む**) + **idle pickerのcontinuing中無効化** + **commit時gate** (run内attempt全期間 + 両entrycontinuation中の遅延commit → dismiss/start非発火) + **arbiter状態中 (`Writing`/`RestartReserved`/`Restarting`) のCTA拒否・import開始拒否** + **restart判定のMain-confined直列化とIO実行分離** (dismiss/startはIO実行) + **arbiter全終端解除 (table-driven: restart成功 / 非commit / `start()` throw / cancel)** + **書込開始gate** (run内attempt中/idle continuation中の `onStrategySelected` が書込を開始しない) のUI test + system Back確認dialog経路 (確定で破棄・キャンセルで維持) test | unit + instrumentation |
...
- strategy相互排他gateの実装漏れ (arbiter状態の二重定義、reservation取得のMain外実行、non-commit終端の解除忘れ、第2段restartのMain実行化) はimported run差し替えの再発・内部競合によるattempt黙drop・CTA/import永久拒否・UI thread上のheavy実行になる — AC-3/AC-5のtable-driven blocking fake test (成功 / 非commit / throw / cancel) で全順序保証・解除・IO実行分離を検証する。arbiter状態中はimport・CTAとも開始できない短い窓が生じる (typed案内で再試行を案内。strategy書込は通常即時完了)。
...
- [ ] summary純粋関数とholder遷移の失敗testを先行追加 (canonical計数: bare/omission、内訳4種算式、global-only intent、single-flight: 二重押下・遅延settle・ABA、validation段階のattempt anchor: cancel後遅延drop/A→B順不同/owning runId不一致、continuing中の破棄/Back不受理、seam例外settle: 非CE/CE-scope生存/scope cancel、strategy相互排他: idle CTA中restart非発火/pre-existing書込はimportと重ならない (W開始→import拒否→解除→再試行)/arbiter状態中のCTA・import拒否/書込開始gate/attach前owning runId再照合/排他の2方向 (W先行・import先行)/arbiter全終端解除 table-driven (成功・非commit・throw・cancel)/restart実行のIO分離)。
- [ ] Minimal implementation (attempt token採番/照合 + ImportSuccess (token継承) + single-flight CTA (例外failure settle・strategyWriteInFlight拒否・runId一致確認込み) + discard (continuing guard) + `importAttemptActive`/`importContinuationActive` + 画面level Back interception (continuing中no-op/非continuing確認dialog) + strings (D-2/D-3確定文言・内訳4種・全体方針行・seam例外typed status) + 競合row/picker無効化 (validation中含む・idle pickerはcontinuing中) + commit時gate (単一arbiter状態機械: Main上でreservation取得まで原子的・dismiss/start実行はIO・全終端finally解除) + import開始時のin-flight拒否 + strategy select/restart arbiter seam)。
...
- 2026-09-18: Re-entry revision 6 (6th review 高×1・中×1・低×1対応)。Current evidenceへ `execute` ヘルパーの事実を追記 (`scope.launch` + `withContext(IO)`、L208–212 — commit後のrun判定・restart実行もIO上で動き、gate読取とrestart実行の間にMain-confinedなimport開始が割り込む競合窓が現行構造に存在)。**(1) 原子的直列化** (高): `strategyWriteInFlight` のlifetimeを **restart判定完了まで** へ延長し、commit後のrestart判定・実行を **Main-confined直列化点** へ移動 (gate読取 → dismiss/start を原子的に閉じる)。holderの `import()`/`receiveAndImport()` は `strategyWriteInFlight` 中の新規import開始をtypedに拒否 (pre-existing書込とimportの重なりを起点で排除)。deterministic testのためstrategy select/restart arbiterを注入可能seamへ切り出すようChange setへ明記。AC-3 (d2)(d3)・AC-5へblocking fake test追加。**(2) run state不変の適用範囲を限定** (中): 「validation開始〜 `ImportSuccess` 表示中の `!continuing` まで」へ限定し、CTA開始後は「別操作による差し替えは禁止」へ書き分け (AC-1へ「CTA押下前」明記)。**(3) Scope (spec) のsummary要約をglobal preference表示と統一** (低)。
- 2026-09-18: Re-entry revision 7 (7th review 高×2・低×1対応)。**(1) restart実行のIO分離 (reservation方式)** (高): revision 6の「Main-confined直列化点上で dismiss/start まで原子的に」は同期heavyな `run.start` をMainで実行するため、「Main上ではrestart可否の判定とreservation取得までを原子的に (gate読取とreservation取得の間にimport開始は割り込めない)、`dismiss()` → `start(trigger)` の実行はIO上」へ変更。`strategyWriteInFlight`/reservation はIO実行完了・非commit・throw・cancelの全終端でfinally解除 (import/CTA拒否の残留防止)。Design・Change set・data flow・risks・checklist・AC-3/AC-5 oracleへ反映。**(2) oracle (e) の矛盾解消** (高): in-flight中import開始拒否の確定契約と「W in-flight中にimport成功」の旧oracleは両立しないため、(e) を「W開始 → import試行 → typed拒否・validation未開始 → W restart判定/完了 → 解除 → import再試行 → owning runでImportSuccess」へ置換、(d2) も確定policyの期待結果へ統一、(d4) reservation解除testを追加。**(3) 「summaryは件数のみ」残存表記の統一** (低): planのsummary model説明を「privacy-safeなscalar fieldのみ (item-level counts + minimizeMovement flag)」へ更新。
- 2026-09-18: Re-entry revision 8 (8th review 高×1・中×2対応)。**(1) 旧「W in-flight中にimport成功」oracleの残存除去** (高): plan Verification AC-5の旧sequenceを削除し、確定policyのsequenceへ一本化 (spec AC / Test oracle / Plan Verificationの3層で同一sequence)。**(2) arbiter状態機械の一本化** (中): Designの相互排他gateを `Idle → Writing → RestartReserved/Restarting → Idle` の単一状態機械として再定義 (`strategyWriteInFlight` のlifetime二重定義を解消)。`Writing` (select〜commit判定・書込開始gate) / `RestartReserved` (Main-confined直列化点上で原子的取得) / `Restarting` (dismiss/startをIO実行) の全状態でimport/CTA開始をtyped拒否。AC-3 (d2) を「排他の2方向」へ書き換え、判定 vs import開始の両方向競合oracleは削除。**(3) non-commit終端の解除をoracleへ追加** (中): AC-3 (d4)/AC-5/Execution checklistの解除testをtable-driven (restart成功 / 非commit / `start()` throw / cancel) へ拡張。
- 2026-09-18: Re-entry revision 5 (5th review 高×1・中×1・低×1対応)。**(1) strategy書込との直列化をrun内entryのattempt全期間へ拡張** (高): commit時gateの抑止条件を「`importContinuationActive` (両entry)」に加えて「run内entryの `importAttemptActive` 全期間」へ拡張し (Design・Change set・data flow)、「import開始前の書込W → validation成功 → `ImportSuccess(R1)` 表示中にW commit → R1差し替え → CTAで別runへattach」の競合を閉じる。`continueImport()` の `attachIntent` 呼出直前にowning runId再照合 (不一致はattachせずtyped failure settle) をstep 3へ追加。`onStrategySelected` への **書込開始gate** (run内attempt中/idle continuation中は書込を開始しない) を追加。AC-3/AC-5 oracleを (e)〜(h) へ再編 (旧(e)は `strategyWriteInFlight` CTA拒否と矛盾していたため分離)。**(2) Scopeのseam例外文言を確定契約へ統一** (中、spec)。**(3) Change setのsummary導入函数入力表記をDesign/AC-4と統一** (低): 入力 = `completed + session + globalPreference.minimizeMovement`。
