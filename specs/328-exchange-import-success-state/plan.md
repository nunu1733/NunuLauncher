...
- **`ui/preferences/destinations/ManualOrganizationPreferences.kt` — hosting調整 (Back interception + 競合freeze)**:
  ...
  - **strategy書込との相互排他gate (UI非依存)**: holderとhostingが共有するstate 2点を追加する。**(a) `importContinuationActive`** — holderのpredicateで、CTA flipからsettle終端までtrue。**(b) `strategyWriteInFlight`** — hostingのpredicateで、`execute { store.select(id) }` の開始からcommit/no-opまでtrue。この2つでhosting側の3つのgateを実装する:
    - **commit時gate**: `onStrategySelected` のcommit時 (`Committed` 分岐内、既存のrun active判定の直前) に、`importContinuationActive` (両entry) **または run内entryでは `exchangeHolder.importAttemptActive`** を検査し、いずれかがtrueなら `dismiss()` → `start(trigger)` を発火しない (strategyのcommit自体は有効で次回以降のrunに反映)。run内の「import開始前に始まった書込Wが成功状態表示中 (`continuing=false` を含む) にcommitしてowning runを差し替え、CTAが別runへintentをattachする」競合と、idleの「CTA直前に始まった書込がcontinuation中にcommitする」競合の双方を閉じる。
    ...
    いずれもUIのdisabled表示とは独立したstate gateである (disabledだけではCTA直前に開始済みの書込を止められない)。
...
| AC-5 | holder unit test: discard → invalidate未呼出・再取り込み往復成立・pending intent消滅・**continuing中のdiscard不受理**。idle競合row無効化 (**validation中含む**) + run内strategy picker凍結 (**validation中含む**) + **idle pickerのcontinuing中無効化** + **commit時gate** (run内attempt全期間 + 両entrycontinuation中の遅延commit → dismiss/start非発火。「W開始 → import成功 → W commit → R1維持」含む) + **`strategyWriteInFlight` 中のCTA拒否** + **書込開始gate** (run内attempt中/idle continuation中の `onStrategySelected` が書込を開始しない) のUI test + system Back確認dialog経路 (確定で破棄・キャンセルで維持) test | unit + instrumentation |
