...
- **`ui/preferences/destinations/ManualOrganizationPreferences.kt` — hosting調整 (Back interception + 競合freeze)**:
  ...
  - **strategy書込との相互排他gate (UI非依存)**: holderとhostingが共有するstate 2点を追加する。**(a) `importContinuationActive`** — holderのpredicateで、CTA flipからsettle終端までtrue。**(b) `strategyWriteInFlight`** — hostingのpredicateで、`execute { store.select(id) }` の開始からcommit/no-opまでtrue。この2つでhosting/holder側の4つのgateを実装する:
    - **commit時gate (2段構成 — Main上で同期heavy seamを実行しない)**: `onStrategySelected` のcommit後、**第1段 (Main-confined直列化点 `withContext(uiDispatcher)` 相当)** で `importContinuationActive` (両entry) **または run内entryでは `exchangeHolder.importAttemptActive`** を読み、いずれでもなければ **restart reservationを取得する** — 判定とreservation取得は同一直列化点上で原子的に行い、その間にMain側のimport開始は割り込めない。いずれかのgateがtrueならreservationを取得せずrestartを抑止する (strategyのcommit自体は有効で次回以降のrunに反映)。**第2段 (IO)** でreservation済みの場合のみ `coordinator.dismiss()` → `coordinator.start(trigger)` を実行する (`start` は同期heavyのためMainでは実行しない — 現行UIもIO wrapperで呼ぶ)。run内の「import開始前に始まった書込Wが成功状態表示中 (`continuing=false` を含む) にcommitしてowning runを差し替え、CTAが別runへintentをattachする」競合と、idleの「CTA直前に始まった書込がcontinuation中にcommitする」競合の双方を閉じる。
    - **`strategyWriteInFlight`/reservation のlifetime**: 書込開始から **第2段のIO実行完了 (または非commit・throw・cancel) まで** 維持する (commitで解除しない)。**解除は全ての終端で `finally` 相当からMain-confined直列化点へ戻して行い**、import/CTA拒否を残留させない (restart成功・非commit・`start()` throw・coroutine cancelのいずれでも)。
    - **import開始拒否**: holderの `import()` / `receiveAndImport()` は開始時に `strategyWriteInFlight`/reservation flagを検査し、trueなら新規importの開始自体をtypedに拒否する (再試行可。envelope gate以前の入口で検査 — validation coroutineを起動しない)。pre-existing書込とimport attemptの重なりを起点で排除する。
    - **書込開始gate**: `onStrategySelected` はrun内entryの `importAttemptActive` 中およびidle entryの `importContinuationActive` 中は **新しいstrategy書込 (`execute { store.select(id) }`) を開始しない** (UIのrecompose遅延・queued click等をUI disabledだけに依存せず遮断)。
    - **CTA側のin-flight拒否**: holderの `continueImport()` step 1が `strategyWriteInFlight` を検査し、trueならCTA開始を拒否する。
    いずれもUIのdisabled表示とは独立したstate gateである (disabledだけではCTA直前に開始済みの書込を止められない)。原子的直列化のdeterministicなtestのため、**strategy select/restartのarbiter (store呼出とrestart判定) を注入可能な小さなseamへ切り出す** (現行の `LayoutStrategySelectionModule.store(context)` 直呼出しはhard-wiredのため)。
...
| AC-3 | holder unit test: CTA → seam呼出 (`Started`/`Attached`)、`Busy`/`NotAttachable` でcontinuing解除+状態維持。**二重押下 (continuing中の再呼出) でseam 1回のみ**、**閉じた成功状態への遅延settle drop** (既存 `lateResultFromAnOldWriteCannotTouchANewerDisclosure` と同型test)、**ABA: 同一identityの回答を再importした新attemptに旧attemptの遅延settleが適用されない** (attempt token不一致drop)、**CTA処理中の破棄/Back不受理**、**seam例外** ((a) 非CE throw → failure settle、(b) **CE throwだがscope生存** → failure settleでcontinuing解除・取り残しなし、(c) scope自体をcancel → UI settle要求なし)、**strategy相互排他** (blocking fakeで (d) idle CTAの `start` をdetection/planning中に停止 → strategy変更試行 → dismiss/start非発火・validated run維持、(d2) **restart判定とimport開始の競合** — gate読取直後とimport開始を競合させ確定policyどおり全順序保証 (import先ならrestart抑止 / 判定先ならimport拒否)、(d3) in-flight/reservation中のimport開始typed拒否、(d4) **reservation解除** — fake `start` をIO上で停止中はimport/CTA typed拒否、restart成功・`start()` throwの双方でfinally解除され再開可能、(e) **pre-existing書込はimportと重ならない**: W開始 → import試行 → typed拒否・validation未開始 → W restart判定/完了 → 解除 → import再試行 → owning runでImportSuccess、(f) `strategyWriteInFlight` 中のCTA開始拒否、(g) continuation後の直接 `onStrategySelected` 呼出 → 書込/restartとも開始されない、(h) attach前owning runId再照合 → 不一致でattachせずfailure settle)、**runId不一致settle** (`Started.runId` ≠ 現在runのrunId → failure settle扱い)。`ManualOrganizationRunTest` 連携 | 同上 |
...
| AC-5 | holder unit test: discard → invalidate未呼出・再取り込み往復成立・pending intent消滅・**continuing中のdiscard不受理**。idle競合row無効化 (**validation中含む**) + run内strategy picker凍結 (**validation中含む**) + **idle pickerのcontinuing中無効化** + **commit時gate** (run内attempt全期間 + 両entrycontinuation中の遅延commit → dismiss/start非発火。「W開始 → import成功 → W commit → R1維持」含む) + **`strategyWriteInFlight` 中のCTA拒否** + **`strategyWriteInFlight`/reservation 中の新規import開始拒否** + **restart判定のMain-confined直列化とIO実行分離** (blocking fakeでgate読取直後とimport開始の競合 → 全順序保証。dismiss/startはIO実行) + **reservation/in-flightの全終端解除** (restart成功・throw・cancelで拒否が残留しない) + **書込開始gate** (run内attempt中/idle continuation中の `onStrategySelected` が書込を開始しない) のUI test + system Back確認dialog経路 (確定で破棄・キャンセルで維持) test | unit + instrumentation |
...
- strategy相互排他gateの実装漏れ (reservation取得のMain外実行、`strategyWriteInFlight` の解除忘れ、第2段restartのMain実行化) はimported run差し替えの再発・内部競合によるattempt黙drop・CTA/import永久拒否・UI thread上のheavy実行になる — AC-3/AC-5のblocking fake testで全順序保証・解除・IO実行分離を検証する。in-flight/reservation中はimport・CTAとも開始できない短い窓が生じる (typed案内で再試行を案内。strategy書込は通常即時完了)。
...
- [ ] summary純粋関数とholder遷移の失敗testを先行追加 (canonical計数: bare/omission、内訳4種算式、global-only intent、single-flight: 二重押下・遅延settle・ABA、validation段階のattempt anchor: cancel後遅延drop/A→B順不同/owning runId不一致、continuing中の破棄/Back不受理、seam例外settle: 非CE/CE-scope生存/scope cancel、strategy相互排他: idle CTA中restart非発火/pre-existing書込はimportと重ならない (W開始→import拒否→解除→再試行)/in-flight・reservation中のCTA・import拒否/書込開始gate/attach前owning runId再照合/restart判定とimport開始の全順序保証/reservation全終端解除 (成功・throw・cancel)/restart実行のIO分離)。
- [ ] Minimal implementation (attempt token採番/照合 + ImportSuccess (token継承) + single-flight CTA (例外failure settle・strategyWriteInFlight拒否・runId一致確認込み) + discard (continuing guard) + `importAttemptActive`/`importContinuationActive` + 画面level Back interception (continuing中no-op/非continuing確認dialog) + strings (D-2/D-3確定文言・内訳4種・全体方針行・seam例外typed status) + 競合row/picker無効化 (validation中含む・idle pickerはcontinuing中) + commit時gate (2段構成: Main上でreservation取得まで原子的・dismiss/start実行はIO・全終端finally解除) + import開始時のin-flight拒否 + strategy select/restart arbiter seam)。
