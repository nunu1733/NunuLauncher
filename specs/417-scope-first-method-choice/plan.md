...
- **export sessionのdurable entry originと、生存runへのattach authorityの2軸**
  ...
  - **run-owned transaction API（run lock所有者を越境させない・呼出し形状まで固定）**: run lockとactive `Operation`（`boundExportId`・`generationEpoch`の保持者）は`ManualOrganizationRun`のprivate authorityであり、storeと`ExchangeMutationGate`はcontroller/holder側が所有する。gateの`withGate { }`はcallback returnで解放されるため、「callback完了後にrun側がbindする」場所は存在しない。そこで向きを固定する: **run lockを保持した`ManualOrganizationRun`がgate-held transaction capability（holder/controller提供）を呼び、capability側が`withGate { durableMutation; onGateHeld(result) }`の内側でrun提供のbind/clear callbackを実行する**。bind/clearは常にgate保持内で行われ、gate→run lockの逆順取得は存在しない。3 seamは同一粒度で定義する:
    ...
    - (2) `cleanupBoundExport(expectedExportId, onGateHeld)` — encode失敗・pre-send discard等、「現在boundされたsession自体を失効させる経路」専用。capabilityが`withGate`内で`invalidateIf(expectedExportId)`を実行し、`onGateHeld`で結果を処理する。clearの条件はstore結果に加え**run lock下の`boundExportId == expectedExportId`**であり、既に別exportへ進んでいる場合は`Superseded`（typed no-op）として現在の束縛を保持する。`Committed`（かつ束縛一致）または`NoMatch`が破棄のcurrent bindingに対してのみ成功扱いでき、historical cleanupの`NoMatch`は束縛をclearしない。settle契約: encode失敗・pre-send discardとも、terminal（`Committed`/束縛一致`NoMatch`/`Superseded`）までは通常のsettle（失敗面遷移・T-16のclose）へ進まず、`WriteFailed`では同一expectedExportIdのcleanupを再試行可能なtyped永続化失敗として保持する（UI ownerを失わない）。
    - (3) `discardScopeBoundRequest(onGateHeld)` — scope-bound依頼破棄の1 seam。順序をoption (a)で固定する: **run lock取得後、capability呼出し前にrun側でcurrent `generationEpoch`を失効させる**（同runのgeneration commitはrun lockを必要とするため割込み不可。epoch失効がgate内の無効化へ先行することをoracle (v)で直接assert）→ capabilityの`withGate`内で`invalidateIf(boundExportId)` → `onGateHeld`で`Committed`/`NoMatch`時のみ従属pending処理（既存reconcile委譲）とbinding clear。
  ...
  - **scope-bound依頼破棄の原子境界**: 順序はrun-owned `discardScopeBoundRequest()`（上記3 seamの(3)）に統一される: run lock取得後・capability呼出し前にcurrent `generationEpoch`を失効 → gate内で`invalidateIf(boundExportId)` → `Committed`/`NoMatch`の場合のみ従属する取り込み済み提案の処理（既存in-process無効化＋read-time reconcile委譲）と束縛clear。session保存と束縛が原子であるため、束縛と異なる当該runのsessionが生存している状態は存在せず、`NoMatch`は「当該runのsessionは既に残っていない」ことの証明として成功扱いできる（この証明は破棄のcurrent bindingに限定）。`WriteFailed`ではsession・pending・`ScopeConfirmed`を保持して再試行する。
...
- **scope-bound依頼破棄（run-owned `discardScopeBoundRequest()` 1 seamに統一）**
  ...
  - 破棄の正本は`ManualOrganizationRun.discardScopeBoundRequest()`の1 seamである（UIの`ExchangeFlowStateHolder`呼出しをこのseamへ委譲し、holderがgate配下で単独に破棄する経路は作らない）。seam内の順序（3 seam定義(3)と同一）: (1) **run lock下・capability呼出し前に**current `generationEpoch`を失効させる（gate内でのepoch失効は行わない）、(2) capabilityの`withGate`内で`invalidateIf(boundExportId)`、(3) 結果が`Committed` / `NoMatch`（current binding）の場合のみ、従属する取り込み済み提案の処理（既存`invalidateImportedIntentForReplacement`と同一のin-process無効化＋durable側は既存read-time reconcile契約に委譲）と束縛clearをgate保持内のrun提供callbackで行い、UIへ成功を返す。
...
| `tests/organizer-instrumentation/...` | journey (a)〜(v)（AC-8定義どおり。legacy IDLE／legacy RUN_IN＋scope非空／legacy RUN_IN＋scope空の3 fixtureを含む）。AC-8は全journeyをconnected laneで固定する。このうち(g)(j)(k)(n)(p)(s)(t)(u)はconnectedに加えてunit oracleも持つ（unitは追加の早期検知であり、connected evidenceの代替ではない）。(a)〜(f)(h)(i)(l)(m)(o)(q)(r)(v)はconnected laneのみ | AC-8/9のevidence |
...
| AC-6 | unit: origin書込み・legacy decode規則（origin欠落＋scope非空＝RUN_IN／scope空＝IDLE）・`entryKind`導出（再取り込みで不変）・run-owned transaction APIのtyped result（cleanupは`boundExportId == expectedExportId`かつ`Committed`/`NoMatch`のときのみclear、不一致は`Superseded`で保持。scope-bound discardのcurrent-binding `NoMatch`とは別契約）・原子commit（save→bind窓なし）・他由来取り込み遮断・owner欠落RUN_IN pending保存とImportReview遷移・entry面creation不在。instrumentation: hub経由のlegacy import、(g)〜(q) journey | 両seam |
| AC-7 | unit: `CANDIDATE_SELECTION_STALE`既存契約回帰 + instrumentation: Home変化後typed再試行 | 両seam |
| AC-8 | instrumentation: (a)〜(v)。失敗注入: 破棄`WriteFailed`でsession/pending/凍結scope不変、owning run喪失後RUN_IN取り込みでdirect attach・direct-attach用成功状態不発、stale epoch完了zero-write破棄、save後・binding前process deathでownerless復帰、store保存失敗/encode失敗/pre-send discardの各経路でsession・binding非乖離、stale cleanup（E1→E2→E1 cleanup）はtyped no-opでE2維持、cleanup`WriteFailed`→retry→terminalでsession/束縛/UI owner一貫、置換commit後はactive session=E2・束縛=E2維持（E1 pendingのみ無効）、bind/clearはgate保持内で実行（gate→run lock逆順取得なし）、破棄はepoch失効→capability→invalidateIfの順（先行assert付き）、lock順序交差でdeadlockなし、upgrade fixture 3種（legacy IDLE／legacy RUN_IN＋scope非空／legacy RUN_IN＋scope空） | connected test lane |
