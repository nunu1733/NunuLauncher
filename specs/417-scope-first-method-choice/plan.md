...
> Status: draft
> Revision 5（2026-09-24）: PR #423 4回目review（Changes requested）対応。lock順序を正順（run lock → `ExchangeMutationGate`）に固定し、bindを二段階commitへ改訂、ownerless RUN_IN遷移の一意化、Non-goals整合。
...
- **export sessionのdurable entry originと、生存runへのattach authorityの2軸**
  ...
  - **束縛の正本と線形化点（lock順序を含む）**: 束縛の正本はactive `Operation`（`ScopeConfirmed`に公開される）が保持するprocess-localな`boundExportId: String?`である。全経路のlock順序は既存#375（`beginAdmission`がrun lock保持のままanchor内でgate取得）と同じ正順 **run lock → `ExchangeMutationGate`** に固定し、gate保持中にrun lockを取得する経路は作らない（ABBA deadlock防止）。bindは二段階commitとする: (i) 生成開始時にrunId＋凍結scope identity＋generation attempt tokenをsnapshotする、(ii) session保存をgate内で完了してgateを解放する、(iii) その後run lockを取得し、同じ正順でgateを再取得してactive sessionをfresh readし、「同一active operationかつ同一`ScopeConfirmed`かつ同一generation attempt、かつ読み直したsessionのexportIdが生成結果と一致」の場合にのみ`boundExportId = exportId`へbindする。保存後〜bind前にreplacement/invalidationが割り込んだ場合はfresh readの結果としてbindしない。(iv) 選択面への復帰・中断・run終了・scope-bound依頼破棄でclearする（破棄はrun lockを外側、gate内の`invalidateIf`を内側とし、`Committed`/`NoMatch`の場合だけ同じrun critical sectionでclear。`WriteFailed`では保持）、(v) 置換生成では旧束縛を置換する、(vi) 遅延して到着したstale settleが後続runへ束縛を付与することはない（(iii)の同一operation条件がこれをfenceする）。`beginImportAttempt`はlive owner判定を`boundExportId`との一致で行う。
...
- **gateを解放したうえで単独のrun lockだけでbindする実装**: saveとbindの間にreplacement/invalidationが割り込めるため「保存されたexact exportIdだけがauthority」という線形化を満たさない。二段階commit（正順でgateを再取得しfresh read検証）を採用する。
...
| `lawnchair/.../organizer/ui/ManualOrganizationRun.kt` | `State.ScopeConfirmed`新設（`boundExportId`を含むoperation保持fields）、`confirmSelection`分岐（manual限定・onboarding直行）、0候補state pass-through（manual限定）、`planWithConfirmedScope`、`attachIntent`拡張、`reopenSelection`（非空候補guard）、生成開始snapshotとrun lock下bindの線形化 | run state machineの唯一の正 |
| `lawnchair/.../organizer/ui/exchange/ExchangeFlowUi.kt` | durable originの記録/読み出し、`entryKind`導出変更（再取り込みで不変）、live owner（runId＋`boundExportId`一致）とdurable provenanceの分離保持、二段階commit bind（gate内保存→正順で再取得→fresh read検証）、durable save時owning-run fenceの更新（owner欠落RUN_IN pending保存）と保存成功後のImportReview遷移、`connectRun`の生存確認更新（`ScopeConfirmed`＋同runId＋束縛一致）、方法選択面の他由来取り込み遮断、scope-bound破棄operation（正順lock・mutation gate配下・束縛clearを含む）、scoped entry行撤去、entry面import-only hosting | exchange flow正本 |
...
| `tests/organizer-instrumentation/...` | journey (a)〜(k)（AC-8定義どおり）、legacy import、a11y（自動oracle＋実機evidence） | AC-8/9のevidence |
...
| AC-11 | 既存回帰（scope binding gate・durable import・rebind・selection既定値・onboarding D-16）＋新規failure注入oracle（破棄`WriteFailed`、owning run喪失後のRUN_IN取り込み、遅延settle、lock順序交差、process死を挟む同等シナリオ）＋「同一session再取り込みで`entryKind`不変」回帰（spec 375 SR-AC-07置換後） | 両seam |
