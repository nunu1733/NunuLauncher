...
> Status: draft
> Revision 10（2026-09-24）: PR #423 9回目review（Changes requested）対応。discardのepoch失効をrun lock下・capability呼出し前に確定、cleanupのexact-binding条件（stale cleanupはtyped no-op）と`WriteFailed`後のsettle/retry契約、#374置換文言の強度修正、oracle (t)(u)(v)とtest inventory統一。
...
| `tests/organizer-instrumentation/...` | journey (a)〜(v)（AC-8定義どおり。legacy IDLE／legacy RUN_IN＋scope非空／legacy RUN_IN＋scope空の3 fixtureを含む。unit/connectedの対応: (g)(j)(k)(n)(p)(s)(t)(u)はunitで観測可能、(a)〜(f)(h)(i)(l)(m)(o)(q)(r)(v)はconnected lane） | AC-8/9のevidence |
