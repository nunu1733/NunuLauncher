...
> Status: draft
> Revision 8（2026-09-24）: PR #423 7回目review（Changes requested）対応。run-owned transaction API（commit seam・cleanup・破棄の1 seam統合）の明示、legacy decode規則（origin欠落＋scope非空＝legacy RUN_IN／scope空＝IDLE）とupgrade fixture oracle (q)を追加。
...
- **export sessionのdurable entry originと、生存runへのattach authorityの2軸**
  ...
  - **run-owned transaction API（run lock所有者を越境させない）**: run lockとactive `Operation`（`boundExportId`・`generationEpoch`の保持者）は`ManualOrganizationRun`のprivate authorityであり、storeと`ExchangeMutationGate`はcontroller/holder側が所有する。この境界を越えるため、`ManualOrganizationRun`へrun-owned transaction APIを明示する: **(1) `commitGeneratedSession(prepare, durableMutation)`** — run lock下でepoch再検証を行い、durableMutation callback（内部でgate取得・session保存・#374旧pending無効化）を実行後、gate解放前にexact exportIdをbindし、1つのtyped resultで成否を返す（store保存失敗時はbindしない）。**(2) `cleanupBoundExport(expectedExportId)`** — encode失敗・pre-send discard等のcleanupを正順（run lock → gate）で条件付き無効化＋束縛clearする。**(3) `discardScopeBoundRequest()`** — scope-bound依頼破棄の1 seam（下記）。controllerは`Operation`へ直接触れず、これらのseam経由でのみrun側と協調する。typed resultは`Committed`/`NoMatch`のときだけbinding clear、`WriteFailed`ではrun state/bindingを保持することまで固定する。
  - **生成seamの責務分割（base実装のdurable-save所有者を跨ぐ）**: baseでは`ExchangeFlowController.generateForSelection()`がexport build → gate内session保存＋#374旧pending無効化 → encode → 結果返却、までを所有する。原子commit契約を満たすため、controllerのgeneration interfaceを分割する: **prepare（lock不要）** = export build／**durable mutation（`commitGeneratedSession`のcallback内でgate下で実行）** = new session保存 → #374の旧pending無効化。encodeは現行どおりcommit後に行う（spec 205の生成順序は変更しない）。encode失敗時の既存cleanup（session無効化）およびpre-send discard（`cancelDisclosure`）、置換時の#374旧pending無効化は、`cleanupBoundExport(expectedExportId)`経由で正順（run lock → gate）の下で当該exact exportIdを条件付き無効化し束縛を同時にclearする。いずれの経路でもactive sessionと`boundExportId`は乖離しない。
...
| `lawnchair/.../organizer/ui/ManualOrganizationRun.kt` | `State.ScopeConfirmed`新設（`boundExportId`・`generationEpoch`を含むoperation保持fields）、run-owned transaction API（`commitGeneratedSession`／`cleanupBoundExport`／`discardScopeBoundRequest`。typed resultで`Committed`/`NoMatch`時のみbinding clear）、`confirmSelection`分岐（manual限定・onboarding直行）、0候補state pass-through（manual限定）、`planWithConfirmedScope`、`attachIntent`拡張、`reopenSelection`（非空候補guard）、epoch claim/advanceとrun lock下commitの線形化 | run state machineの唯一の正 |
...
| AC-8 | instrumentation: (a)〜(q)。失敗注入: 破棄`WriteFailed`でsession/pending/凍結scope不変、owning run喪失後RUN_IN取り込みでdirect attach・direct-attach用成功状態不発、stale epoch完了zero-write破棄、save後・binding前process deathでownerless復帰、store保存失敗/encode失敗/pre-send discard/置換pending無効化の各経路でsession・binding非乖離、lock順序交差でdeadlockなし、upgrade fixture 3種（legacy IDLE／legacy RUN_IN＋scope非空／legacy RUN_IN＋scope空） | connected test lane |
