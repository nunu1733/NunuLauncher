...
> Status: **implemented** (2026-09-24) — Phase1 review 11回（head `bc459e9fa0` 時点で最終
> [Approved](https://github.com/nunu1733/NunuLauncher/pull/423#issuecomment-5802041454)。各roundの指摘と解消は
> [PR #423](https://github.com/nunu1733/NunuLauncher/pull/423) コメント群に記録）を受け、実装PR
> [#432](https://github.com/nunu1733/NunuLauncher/pull/432) merge（merge commit `c7bcc26e06`）で実装完了。
> 実装レビューは [Issue #417](https://github.com/nunu1733/NunuLauncher/issues/417) コメントで5回
> （round1〜3 Changes requested / round4 [Approved](https://github.com/nunu1733/NunuLauncher/issues/417#issuecomment-5807060470) /
> round5 delta re-review [Approved](https://github.com/nunu1733/NunuLauncher/issues/417#issuecomment-5809897430)）。
> 独立監査記録:
> [docs/assessment/pr-432-417-scope-first-method-choice.md](../../docs/assessment/pr-432-417-scope-first-method-choice.md)。
...
- **import provenanceとattach authorityの2軸分離**: (軸1) export sessionにscope origin（その依頼がscope選択ありのrun内で作られたか: `IDLE` / `RUN_IN`）をdurableに保持する（additive拡張）。`entryKind` の正本はこのdurable originであり、実行時のrun state推定は廃止する。(軸2) 生存runへのdirect attach authorityは、process-localな「このrunの`ScopeConfirmed`で生成したexact exportId」の束縛でのみ成立し、scope一致だけでは同一runと判定しない。束縛の正本はactive operation（`ScopeConfirmed`）が保持するprocess-localな`boundExportId`であり、生成はfirst-classなgeneration transactionとして管理する。`generationEpoch`（runId・operationId・凍結scope identity・epoch）をactive operationが所有し、生成開始・置換ごとにrun lock下でclaim/advanceし、選択面への復帰・中断・run終了・scope-bound依頼破棄で失効させる。RUN_IN生成では、export組成（pure処理）をlock外で行い、**durableなsession保存・`generationEpoch`再検証・`boundExportId`更新を同一のrun lock → `ExchangeMutationGate` critical sectionでcommitする**。session保存と束縛更新の間に窓は構造的に存在せず、保存されたsessionは必ず束縛を持つ。staleなepochの生成完了はzero-writeで破棄され、session保存も束縛も行わない。全経路でこの正順を固定し、gate保持中にrun lockを取得する経路は存在しない。選択面への復帰・中断・run終了・scope-bound依頼破棄で束縛をclearする。破棄は**run lock取得後にcurrent epochを失効させてから**capabilityへ入る（同runのgeneration commitはrun lockを必要とするため割込み不可。epoch失効がgate内の無効化へ先行することをoracleで固定する）。session保存と束縛が原子であるため、束縛と異なる当該runのsessionが生存している状態は存在せず、`NoMatch`は「当該runのsessionは既に残っていない」ことの証明として成功扱いできる（この証明は破棄のcurrent bindingに限定し、historicalなcleanupへは一般化しない。`WriteFailed`ではsession・pending・凍結scopeを保持）。cleanup（encode失敗・pre-send discard）は、**束縛が当該expectedExportIdと一致する場合のみ**clearを行い、既に別exportへ進んでいる（stale binding）場合はtyped no-opとして現在の束縛を保持する。置換生成ではepochがadvanceし、旧束縛は置換される。以上の原子性は**live ownerが存続する同一process内でのlinearization**であり、durable保存とprocess-local束縛はprocess deathに対してatomicではない。session保存がdurable化した直後に束縛更新の前にprocessが死んだ場合、再起動後はownerless RUN_IN sessionのみが残る（binding不在が正しい状態であり、hub/import-only経由 → ImportReview → rebindにのみ接続され、direct attach authorityは復活しない）。`NoMatch`を成功扱いできる証明も、liveなowning runのcritical section内に限定される。live ownerが存在する場合のみowning-run fence＋`attachIntent`を使い、ownerを欠くRUN_IN originの取り込み（process死後のhub経由等）はRUN_IN pendingとしてdurable保存され、その保存成功後の面はdirect-attach用の取り込み成功状態ではなく既存の取り込み済み提案の確認面（ImportReview、既存reconcile契約で読み直し）へ切り替わる。継続は `Hub → ImportReview` rebind 1経路に限定され、新規のrebind/start seamは作らない。加えて、方法選択面からの取り込みは「この面の確定scopeから作成された依頼」に限定し、同一scopeでも別run由来・legacy IDLE由来の依頼は依頼の作り直し（既存置換確認経由）へ案内する。
...
And 承認すると、`ExchangeMutationGate`配下で (1) current `generationEpoch`の失効（進行中の生成完了を以後zero-writeで棄却）、(2) session storeのfailure-aware無効化（`invalidateIf(boundExportId)`）、(3) 無効化が `Committed` / `NoMatch` だった場合のみ、従属する取り込み済み提案の既存reconcile契約に沿った処理、の順に実行され、その後に限り選択面が編集可能な状態で再表示される
...
Given 方法選択面でgeneration epochをclaimし、export組成（pure処理）を完了した後、原子commitへ入る前にrunを中断した（epochは失効済み）
When 遅延して到着した生成完了がcommitを試みる
Then epoch再検証で拒否され、session storeへの保存も束縛も行われない（zero-write）
And この依頼がprocess recreation後にstatus cardや取り込み対象として再出現することはない
...
Given 旧版で作成されたidle依頼（配置済みのみのexport scope、origin欠落＋scope空）がdurableに残っている
When hub status card（entry面のimport-only hosting）から依頼を開き回答を取り込み、提案で続行する
Then このimport recordの`entryKind`はIDLE（legacy decode規則どおり）として記録され、#375 の再開契約どおり fresh run admission → 検出 → 選択面（unchecked初期値・件数案内）→ 確認時の完全一致検証を経てplanningへ進む
And 依頼時scopeに候補が含まれないため、候補を選択した状態の確認は `SET_MISMATCH` としてzero-write失敗し、選択修正による継続が案内される

### Scenario: upgrade前のRUN_IN依頼の復元意味論は維持される

Given upgrade前にrun-inで候補を選択して作成された依頼（origin欠落＋`scopeCandidates`非空）がdurableに残っている
When 回答を取り込み、提案で続行する
Then legacy decode規則によりRUN_IN originとして復元され、#375 の選択復元初期値（依頼時の明示selectionを初期値とする＋明示確認1回）を持つrebind契約が維持される
And origin欠落＋scope空のrecordはこの復元を行わず（legacy IDLEへ安全側に倒す）、direct attach authorityはどちらのcaseでも復活しない
...
### Scenario: save後・束縛更新前のprocess deathはownerless RUN_INとして復帰する

Given 方法選択面で生成し、原子commit内でsession保存がdurable化した直後、束縛更新の前にprocessが死んだ
When processが再起動し、ユーザーがhub（entry面のimport-only hosting）から当該依頼を開く
Then process-localな束縛は存在しないのが正であり、この依頼はownerless RUN_INとして直接取り込みが行われた場合もImportReview → rebindにのみ接続され、どのrunへもdirect attachされない

### Scenario: 生成完了後の失敗・破棄経路でもsessionと束縛は乖離しない

Given 方法選択面のrunで依頼が束縛付きでdurable保存された
When encode失敗、pre-sendの破棄（`cancelDisclosure`）のいずれかが発生する
Then sessionを無効化する各経路は、正順（run lock → `ExchangeMutationGate`）で当該exact exportIdを条件付き無効化し、束縛が当該exportIdと一致する場合のみ同時にclearする
And 既に別exportへ置換が進んでいる（stale binding）場合はtyped no-opとして現在の束縛を保持し、active sessionと`boundExportId`が乖離した状態は残らない（store保存失敗時は束縛も更新しない）
And `WriteFailed`の場合は通常のsettle（失敗面への遷移・T-16のclose）へ進まず、同一expectedExportIdに対するcleanup再試行が可能な状態でtypedな永続化失敗を保持する
And 置換生成（E1→E2）のcommitはsession無効化ではなく正常commitであり、commit後もactive session=E2・束縛=E2が維持され、無効化されるのはE1に従属する旧pendingのみである（new session保存後に旧pending無効化を試行し、そのwrite failure・process death時もread-time reconcileが正本としてfail-closedに清掃する。失敗を理由にE2はrollbackしない）
...
- 永続化するdata: **export sessionへの1 field追加（durableなentry origin: `IDLE` / `RUN_IN`）のみ**。additive拡張であり、既存record（origin欠落）のlegacy読み替え規則は次のとおり: **origin欠落＋`scopeCandidates`非空 ＝ legacy RUN_IN**（既存durable情報から一意に復元でき、#375の選択復元初期値を持つrebind意味論を維持する）。**origin欠落＋`scopeCandidates`空 ＝ IDLE**へ安全側に倒す（legacy IDLEと「0件選択のRUN_IN」は区別不能なため。この場合はrebind unchecked復元となり、direct attach authorityは復活しない）。新規recordのoriginはsession生成時に1回だけ書かれ、以後不変である。選択状態・確定scope・direct attach authority（run↔exact exportId束縛）はprocess-localなままである（spec 228継続）。`entryKind`（IDLE/RUN_IN）は型と既存recordの読み取り互換を維持し、新規recordはdurable originから導出される（実行時run state推定は廃止）。
- scope-bound依頼破棄はdurable mutationを含む（failure-aware invalidationと既存reconcile primitiveの利用であり、layout/workspace DBへの書込みは生じない）。
- migration、backup/restore、rollbackへの影響: 追加fieldの下位互換は読み側で保証する——origin欠落は上記legacy decode規則（scope非空→RUN_IN、scope空→IDLE）で読む。**downgradeは現行codecの実態に従う**: 旧buildのJSON decoderは未知keyでdecode失敗するため、scope-firstで作成されたsessionは旧buildでは「no-session」としてfail-closedになり、従属するpendingもread-time reconcileで失効し得る。これ安全側への倒れ込みであることを明示的に許容する（既存`categoryRefs` fieldと同じ既知の挙動であり、`SET_MISMATCH`等のfail-closed gateは維持される）。schema破壊・data移行は生じない。
...
- [ ] AC-6: run外のidle依頼の新規作成入口は撤去され、entry面のexchange hostingはimport-only（既存依頼の状況表示と取り込みのみ）となる。export sessionはdurableなentry originを保持し、import recordの`entryKind`はoriginから導出される（実行時run state推定に依存しない。同一sessionへの再取り込みで`entryKind`が変化することはない）。legacy decode規則（origin欠落＋scope非空＝legacy RUN_IN、origin欠落＋scope空＝IDLEへ安全側倒し）でupgrade前recordのrebind意味論を維持する。direct attach authorityはprocess-localな「このrunの`ScopeConfirmed`で生成したexact exportId」の束縛（`boundExportId`）に限定される。生成は`generationEpoch`（active operation所有・生成開始/置換ごとにrun lock下でclaim/advance・復帰/中断/終了/破棄で失効）として管理され、RUN_INのsession保存・epoch再検証・束縛更新は同一の正順critical section（run lock → `ExchangeMutationGate`）で原子にcommitされる（save→bind窓なし）。staleなepochの完了はzero-writeで破棄される。この原子性はlive owner存続下の同一process内linearizationであり、process death後はbinding不在が正である（ownerless復帰契約はscenarioどおり）。全経路のlock順序は正順に固定され、gate保持中のrun lock取得は存在しない。scope一致だけでは同一runと判定しない。ownerを欠くRUN_IN originの取り込みはdirect attachせずRUN_IN pendingとして保存され、保存成功後はImportReviewへ切り替わり、継続はrebind 1経路のみ（既存#375 anchorの再利用、新seam不設置）である。方法選択面からの取り込みは「この面の確定scopeから作成された依頼」に限定され、同一scopeでも別run由来・legacy IDLE由来の依頼は作り直しへ案内される。既存のdurable依頼・取り込み済み提案は #374/#375 契約（`Hub → ImportReview` 1経路、IDLE由来のunchecked復元を含む）どおりimport・再開できる。
...
- [ ] AC-8: instrumentationで次のjourneyが固定される: (a) empty Home → 全候補選択 → AI依頼 → import → preview、(b) empty Home → 全候補選択 → このまま整理 → preview、(c) AI依頼存在下のBack → scope-bound依頼破棄（`Committed`後は選択面へ復帰、`WriteFailed`時は方法選択面に残留・凍結維持・再試行）、(d) Back/中断/process recreation後にscope ownership（どのscopeがどの依頼・runに紐づくか）が曖昧にならず、legacy IDLE依頼がactiveな状態でもmanual runはadmissionされ、その方法選択面からの取り込みは遮断される、(e) 候補0件runで方法選択面からBackすると選択面を経ずに中断される、(f) onboarding提案のrunに方法選択面が現れない、(g) 同一runで生成→import→attachが成功する、(h) RUN_IN依頼作成後にowning runを失ってhub経由で取り込むと、RUN_IN pendingが保存されImportReviewへ切り替わり（direct attach・direct-attach用成功状態とも不発）、rebindへ接続される、(i) 同一scopeでも別run由来のactive依頼は方法選択面からattachできない、(j) 生成epoch claim後・原子commit進入直前（run lock取得後・epoch再検証前）にbarrierを置いてrunを中断/再開し、遅延した生成完了がcommitを試みてもepoch再検証で拒否され、session保存・束縛ともzero-writeである。process recreation後にも当該依頼はstatus/取り込み対象として現れない、(k) 同一runでE1→E2の置換生成後、E1の回答はattachできずE2のみattachできる、(l) rebind admissionがrun lockを保持した状態と、生成commit/破棄がgate取得を試行する状態をbarrierで交差させても、lock順序は正順に固定され双方が完了し（deadlock不発生）、runは不変に保たれる、(m) E1束縛下で置換生成のepochをclaimした直後・E2 commit前にbarrierを置いてscope-bound依頼破棄を実行すると、current epochが失効してE2の完了はzero-writeで破棄され、E1は`Committed`で無効化されてから選択面へ復帰する。E2はprocess recreation後にもstatus/取り込み対象として再出現しない、(n) E1/E2の生成完了が順序反転で到着しても、current generation epochのcommitのみがsession保存と束縛を行い、stale epochの完了はzero-writeで破棄される、(o) 原子commit内でsession保存がdurable化した直後・束縛更新前にprocess deathを注入すると、再起動後はownerless RUN_INとしてImportReview → rebindにのみ接続され、direct attach authorityが復活しない、(p) store保存失敗・encode失敗・pre-send discardの各経路で、active sessionと`boundExportId`が乖離した状態が残らない、(q) upgrade fixture 3種（legacy IDLE／legacy RUN_IN＋scope非空／legacy RUN_IN＋scope空）で、(b)は#375の選択復元初期値が維持され、(a)(c)はunchecked復元かつdirect attach authorityが復活しない、(r) E1→E2の置換commit後、active session=E2・束縛=E2が維持され、無効化されるのはE1に従属する旧pendingのみである（E2がrollbackされないことの直接assert）、(s) bind/clearはいずれもgate保持内のrun提供callbackとして実行され、gate→run lockの逆順取得が存在しない、(t) E1 commit → E2 commit → E1の遅延cleanup（encode失敗/pre-send discard）で`invalidateIf(E1)=NoMatch`となっても、束縛一致条件によりactive session=E2・束縛=E2が維持される（stale cleanupはtyped no-op）、(u) encode失敗・pre-send discardのcleanupが`WriteFailed`となった場合、通常settleへ進まず同一expectedExportIdのcleanup再試行が可能な状態を保持し、retry後はterminalへ至る（失敗中もactive session/束縛/UI ownerが一貫する）、(v) scope-bound破棄では、run lock取得後・capability呼出し前にcurrent epochが失効しており、gate内の`invalidateIf`より先行することを直接assertする。
...
| AC-6 | unit: origin書込み・legacy decode規則（origin欠落＋scope非空＝RUN_IN／scope空＝IDLE）・`entryKind`導出（再取り込みで不変）・`generationEpoch`のclaim/advance/失効と原子commit（save→bind窓なし）・他由来取り込み遮断・owner欠落RUN_IN pending保存とImportReview遷移・entry面creation不在。instrumentation: hub経由のlegacy import、(g)〜(q) journey |
| AC-7 | unit: composition時stale検出の既存契約 + instrumentation: Home変化後のtyped再試行 |
| AC-8 | instrumentation: (a)〜(v)。失敗注入: scope-bound破棄の`WriteFailed`でsession/pending/凍結scopeが不変であること、owning run喪失後のRUN_IN取り込みでdirect attachとdirect-attach用成功状態が不発であること、stale epoch完了のzero-write破棄、save後・binding前のprocess deathでownerless復帰すること、cleanup（encode失敗・pre-send discard）で束縛一致時のみclear・stale時はtyped no-op、cleanup`WriteFailed`→retry→terminalでsession/束縛/UI ownerが一貫、lock順序交差でdeadlockしないこと |
...
| AC-11 | 既存回帰（scope binding gate・durable import・rebind・selection既定値・onboarding D-16） + 新規failure注入oracle（`WriteFailed`・owning run喪失・stale epoch完了・lock順序交差・process死） |
...
- 2026-09-24: PR #423 5回目review（Changes requested）対応。二段階commitが作るsave→bind窓と破棄`NoMatch`成功扱いの組合せ問題（置換後sessionが破棄後に残存し得る）を、session保存・`generationEpoch`再検証・`boundExportId`更新を同一正順critical sectionで原子commitする方式へ置換して解消。`generationEpoch`をactive operation所有のfirst-class transaction（claim/advance/失効）として定義し、stale epoch完了のzero-write破棄、破棄時のepoch先失効、oracle (m)(n)を追加。
- 2026-09-24: PR #423 6回目review（Changes requested）対応。旧二段階commit前提のscenario（遅延完了がsession保存成功）とAC-8(j)の「保存後〜bind前」oracleを原子commitモデルへ置換。生成seamの責務分割（lock不要のprepareと、run lock保持下で呼べるdurable commit。commit内でepoch再検証→session保存→#374旧pending無効化→束縛更新をtyped resultで返す）を契約化し、encode失敗・pre-send discard・置換pending無効化の各cleanup経路でのsession/束縛非乖離を明記。原子性をlive owner存続下の同一process内linearizationに限定し、save後・binding前process deathのcrash oracle (o)と非乖離oracle (p)を追加。
- 2026-09-24: PR #423 7回目review（Changes requested）対応。run lock所有者を越境させないrun-owned transaction API（commit seam・`cleanupBoundExport`・`discardScopeBoundRequest`。controllerはOperationへ直接触れない。`Committed`/`NoMatch`時のみbinding clear、`WriteFailed`ではrun state/binding保持）を明示し、plan内の重複した破棄記述を1 seamへ統合。legacy decode規則（origin欠落＋scope非空＝legacy RUN_IN／scope空＝IDLEへ安全側倒し）を定義し、upgrade fixture oracle (q)を追加。
- 2026-09-24: PR #423 8回目review（Changes requested）対応。transaction APIの呼出し形状を「gate所有側が`withGate`を開いたままrun提供callbackを実行する」向きへ固定（bind/clearはgate保持内で実行、gate→run lock逆順取得なし。oracle (s)）。#374置換時の旧pending無効化をsession cleanupから分離し、置換commitは正常commit（active session=E2・束縛=E2維持、E1 pendingのみ無効。oracle (r)）であることを明記。Migration節に残っていた旧「origin保持なし→IDLE」文をlegacy decode規則へ統一。
- 2026-09-24: PR #423 9回目review（Changes requested）対応。scope-bound破棄のepoch失効をrun lock下・capability呼出し前に確定（oracle (v)）。cleanupにexact-binding条件（`boundExportId == expectedExportId`のときのみclear、stale cleanupは`Superseded` typed no-opで現在束縛を保持）を導入し、`NoMatch`成功扱いを破棄のcurrent bindingに限定（oracle (t)）。encode失敗・pre-send discardのcleanup `WriteFailed`後は通常settleへ進まずretry可能なtyped永続化失敗を保持する契約（oracle (u)）を追加。#374置換文言を「新session保存後に旧pending無効化を試行し、write failure/process death時もread-time reconcileが正本としてfail-closed清掃、E2はrollbackしない」へ修正。
