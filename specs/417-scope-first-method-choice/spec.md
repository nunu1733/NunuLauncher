...
| AC-8 | instrumentation: (a)〜(n)。失敗注入: scope-bound破棄の`WriteFailed`でsession/pending/凍結scopeが不変であること、owning run喪失後のRUN_IN取り込みでdirect attachとdirect-attach用成功状態が不発であること、stale epoch完了のzero-write破棄、lock順序交差でdeadlockしないこと |
