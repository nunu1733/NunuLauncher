...
Then sessionを無効化する各経路は、正順（run lock → `ExchangeMutationGate`）で当該exact exportIdを条件付き無効化し、束縛を同時にclearする
And いずれの経路でも、active sessionと`boundExportId`が乖離した状態は残らない（store保存失敗時は束縛も更新しない）
And 置換生成（E1→E2）のcommitはsession無効化ではなく正常commitであり、commit後もactive session=E2・束縛=E2が維持され、無効化されるのはE1に従属する旧pendingのみである（#374どおり、旧pendingの物理削除はread-time reconcileに委譲される）
...
| AC-8 | instrumentation: (a)〜(q)。失敗注入: scope-bound破棄の`WriteFailed`でsession/pending/凍結scopeが不変であること、owning run喪失後のRUN_IN取り込みでdirect attachとdirect-attach用成功状態が不発であること、stale epoch完了のzero-write破棄、save後・binding前のprocess deathでownerless復帰すること、各cleanup経路でsession/bindingが乖離しないこと、lock順序交差でdeadlockしないこと |
