...
- 既存 instrumentation の検証パターン: `tests/organizer-instrumentation/app/lawnchair/organizer/ui/ManualOrganizationPreferencesInstrumentationTest.kt` — ja locale は `createConfigurationContext(Configuration().apply { setLocale(Locale.JAPAN) })` (`:696-697`)、200% font scale は `Density(1f, fontScale = 2f)` (`:295,:680`)、new-folder row の描画確認は `manual_organization_preview_new_folder_row` (`:421,:756`)。actual resource / locale 統合と 200% font scale 検証はこの surface に置く (unit test は fake string provider で resolver の純粋検証にとどめる)。
