...
> Date: 2026-08-25
> Fix commit: `3cc9cb742b` (`fix(onboarding): activate proposal actions on the first touch`) on `issue-137-proposal-touch-activation`
> Test commit: `b00b9802fc` (`test(onboarding): exercise proposal actions with real touch streams`)
> Release APK: `Lawnchair.15.Dev.(3cc9cb7).github.release.apk`
> APK SHA-256: `1f16426be8d090acbb179c4a37aba05fc1ced175fb3c9dfe6bb74724922516e3`
...
`nunu_qpr2_api36_1` and `issue108_api36_pixel_tablet` (API 36):
...
- `recreatingLauncherWhileProposalIsShownLeavesNoDuplicateOrOrganizerRun` —
  recreation while shown leaves no duplicate proposal, no outcome write,
  no admission.
...
|---|---|---|
| 1 | Cold start, no input | Proposal visible at the bottom-sheet position (`release-01-proposal-bottom-sheet.png`) |
...
| 8 | Final artifact rebuild sanity | Fresh install → proposal visible → single touch on `Later` → closed, `DEFERRED` |
...
- The proposal renders at its intended bottom-sheet position only when the
  layout params are created as `BaseDragLayer.LayoutParams` with
  `ignoreInsets=true`; plain `FrameLayout.LayoutParams` are converted,
  losing gravity, and inset adjustment pinned the popup under the status
  bar (fixed in the same change; before/after visible in
  `release-01-proposal-bottom-sheet.png` versus the Issue #137 dogfooding
  evidence).
