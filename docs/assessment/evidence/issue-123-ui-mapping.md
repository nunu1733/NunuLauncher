...
| 7 | HomeScreen設定entries ×4 | `HomeScreenPreferences.kt` | 上流 `NavigationActionPreference` | **変更不要**。上流componentのみ。**#367更新（2026-09-19）**: settings Home screenのorganizer系entries（placement locks / diagnostics / category overrides / custom categories）とPersonalization groupはOrganizer hub（row 8）へ集約され削除（TO-BE §5.2、spec 367 MAT-AC-02）。Home screenに残るorganizer由来rowはhub入口rowとmanual organization直行row（#370まで暫定併存）のみ。component・tokenの記述は引き続きhub経由の各destinationに適用される |
| 8 | Organizer hub（T-01、#366） | `ui/preferences/destinations/OrganizerHubPreferences.kt`（入口row・使用状況material行は `HomeScreenPreferences.kt` / `OrganizerUsageMaterialRows.kt`） | `PreferenceScaffold` + `PreferenceLazyColumn` + `PreferenceGroup` + `NavigationActionPreference`、run面（#2）と同一のbodyMedium+16dp情報行 | **収束実施**。新surfaceを既存component/theme tokenのみで構成。status行はspec 271のdurable status行と同一string resource、材料・診断導線行は既存label/summary stringを再利用。two-paneは `LocalIsExpandedScreen` 規約に従う。**#367更新（2026-09-19）**: 使用状況material行（T-06）の唯一のインスタンスがhub内となる（settings側Personalization groupは削除。`HomeScreenPreferences.kt`からの参照はrow 7の注記どおり解消）。**#376更新（2026-09-22）**: `ORGANIZED_RESTORABLE` 行に残時間行（既存bodyMedium + 16dp情報行token、新規format resource `manual_organization_recovery_remaining_hours` / `_under_one_hour`）と復元CTA（`PreferenceTemplate` + `clickable()`。labelは既存 `manual_organization_recovery` を再利用）を追加（D-15、[spec 376](../../../specs/376-durable-status-recovery-entry/spec.md)）。読み順は「状態→残期限→操作」（TO-BE §13-5）でcompose順に反映 |
