...
- private-profile follow-upのためLawnchair APKをinstallし、`adb shell cmd role add-role-holder --user 0 android.app.role.HOME app.lawnchair.debug` 後に `cmd role get-role-holders` → `app.lawnchair.debug`、`dumpsys package app.lawnchair.debug` → `ACCESS_HIDDEN_PROFILES: granted=true`（`ACCESS_HIDDEN_PROFILES_FULL` はgrantなし）を確認した。`dumpsys user` ではuser 11が `android.os.usertype.profile.PRIVATE` / `RUNNING_UNLOCKED` であり、user 11にも `com.android.camera2` が存在した。
- 上記条件で `adb shell am instrument -w -r -e class app.lawnchair.organizer.integration.ProductionOrganizationInputInstrumentationTest#productionComposerComposesEvidenceForEveryAvailableProfileWithoutPrivilegedCrossUserAccess app.lawnchair.debug.test/app.lawnchair.migration.DeckRetirementTestRunner` → `OK (1 test)`。logcatは `ISSUE129_EVIDENCE profiles=0,10,11 insertedRows=3 systemPackage=true ready=true` となり、同じ `com.android.camera2/com.android.camera.CameraLauncher` をcanonical rowのcomponentとしてpersonal/work/privateの3 profileへ挿入し、production composerが全rowのprofile-bound evidenceを通過させた。private-profileの必要条件（HOME role + `ACCESS_HIDDEN_PROFILES`）は[LauncherApps API contract](https://developer.android.com/reference/android/content/pm/LauncherApps#getApplicationInfo(java.lang.String,int,android.os.UserHandle))とも一致する。
...
- **private space user 11の扱い（follow-upで解消）:** 初回auditではuser 11がtest対象外だったためacceptance gapとして扱った。その後、Lawnchairを実際の`ROLE_HOME` holderにし、`ACCESS_HIDDEN_PROFILES`のgrantとunlocked private user 11を確認した。同じprofile間で実在する`com.android.camera2`を`LauncherApps.getActivityList` / `getApplicationInfo(..., user11)`で解決し、canonical rowをpersonal/work/privateへ1行ずつ挿入したproduction-seam testが `profiles=0,10,11` / `insertedRows=3` / `ready=true` で成功した。従ってprivate-profile evidenceは当該API 36.1 runtimeでsupportedであり、前回の「runtime unsupported」という記録は誤りとして撤回する。locked-privateのpreserved扱いは既存test (`productionComposerPreservesQuietPrivateDisabledAndUnavailableProfileWithoutEvidenceFallback`) が担保する。
