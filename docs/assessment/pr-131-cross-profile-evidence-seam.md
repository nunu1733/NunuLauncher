...
- **private space user 11の扱い:** 本監査のローカルAVDではuser 11(private space)がunlockedかつquiet mode disabledでありながら、`LauncherApps.getActivityList(null, user11)` は0件を返し、`launcherApps.getApplicationInfo("com.android.settings", 0, user11)` は `PackageManager.NameNotFoundException` を投げた。つまりAndroidのprivate spaceは、unlock状態でもpersonal profileのlauncherプロセスにパッケージ情報を公開しないplatform設計を持つ。このためproduction-seam regression testはevidence read対象としてprivate spaceを含められず、profiles=0,10にとどまっている。これは本修正の欠陥ではなく、ランタイムがサポートしないcellの正しいfail-closed結果である。locked-privateのpreserved扱いは既存test (`productionComposerPreservesQuietPrivateDisabledAndUnavailableProfileWithoutEvidenceFallback`) が担保し、本監査のinstrumentation再実行でも通過している。
