...
- Head SHA: 483038d94fd2d378692e54dd5bf2d80adf11d635
- CI run: https://github.com/nunu1733/NunuLauncher/actions/runs/34770025968
...
- Re-audit: 初回audit（head `0eb3355e0e`、CI run 34768482481）後、docs commit `483038d94f`（audit記録追加とspec statusの `implemented` への更新）が加わったため再監査。差分 `git diff 0eb3355e0e..483038d94fd2d378692e54dd5bf2d80adf11d635` は `docs/assessment/pr-314-nova-restore-capture.md`（追加）と `specs/299-nova-restore-capture-invalid/spec.md`（status行とChange history 1行のみ）の2ファイルで、production code・test・CI workflowへの変更は無い。受入条件（CI-AC-01..08）の定義内容は不変であり、初回auditの判定をそのまま承継する。

## Scope

対象はPR #314（base `main`、head branch `issue-299-spec-plan`、audited head `483038d94fd2d378692e54dd5bf2d80adf11d635`）。code本体はhead commit `0eb3355e0e` で不変であり、その後の差分 `git diff 0eb3355e0e..483038d94f` は本audit記録の追加とspecのstatus更新のみ（production code・test・CI workflowは無変更、下記Re-audit行参照）。`git merge-base origin/main` は `c5274b5d0d`（#313 merge後の現行main）であり、GitHub APIの取得したPR diffは18ファイル、+2454/−25。そのうちcode commit `0eb3355e0e` 自体は16ファイル、+428/−224で、残りはbranch上で先行commitされたspec（`specs/299-nova-restore-capture-invalid/spec.md`、`plan.md`）と調査assessment（`docs/assessment/issue-299-nova-restore-capture-invalid.md`）の追加である。task packetに記載のbase `37e3dd8feb` は現行merge-baseではなく（mainが#313で進行）、記録値「16ファイル +428/−224」はcode head commit単体のdiffと一致する。本auditはGitHub APIとlocal gitで確認した値を正本とする。
...
- **CI-AC-04（#185 reserved-QSB保護の非回帰）: 満たす**。既存coverage（`OrganizationInputComposerTest` の#185 case、`organizer-instrumentation-shared-writer-tests` の `LoaderCursorOverlapAcceptanceContractTest` 等）がCI merge gate run 34770025968ですべてsuccess。assessmentも全runでreservation違反・`CAPTURE_RESERVED_OVERLAP` 未発生を記録。
- **CI-AC-05（自動regressionの追加）: 満たす**。committed instrumentation family + 新CI lane `organizer-instrumentation-issue299-tests` が `final-status` のmerge gateへ接続され、run 34770025968で実際に実行・成功している。deterministicなtest seamが作れたケースに該当し、代替device evidenceへの依存は不要。codec/loggerのtyped化はJVM unit test（`DiagnosticsLoggerTest` / `OrganizationInputComposerTest` 拡張）でも覆盖される。
...
audit本人による独立再実行（実行時の本checkout code state = `0eb3355e0ef9937b9a4b99889303741aa86a4ce2`。現audited head `483038d94f` との差分はdocs・specのみのため同一code stateである。対象は既追跡分のみ・dirty fileなし）:
...
→ PASS: audit docs/assessment/pr-314-nova-restore-capture.md covers
  483038d94fd2d378692e54dd5bf2d80adf11d635 with independent CI evidence
...
- audited head `483038d94fd2d378692e54dd5bf2d80adf11d635` のmerge gateはrun 34770025968: `event=pull_request`、`pull_requests=[314]`、`head_branch=issue-299-spec-plan`、`head_sha=483038d94fd2d378692e54dd5bf2d80adf11d635`、`path=.github/workflows/ci.yml`、`status=completed`、`conclusion=success`。
- 同runのjob別conclusion（per_page=100で取得）: `final-status: success` を含む全jobがsuccess。source job `organizer-unit-tests` / `check-style` / `build-debug-apk` は実行済みsuccess、新lane `organizer-instrumentation-issue299-tests` も実行済みsuccess。
- 参照run: code commit `0eb3355e0e` 上の先行merge gate run 34768482481も同一条件（pull_request / PR #314関連付け / completed / success / 全job green）をaudit本人が確認済み。`0eb3355e..483038d94f` の差分はdocs・specのみのため、codeに対するCI証拠は両runで同等である。
...
- **再監査の成立条件**: 初回audit（head `0eb3355e0e`）後にdocs commit `483038d94f` が加わったが、その差分はaudit記録の追加とspec status更新（`accepted` → `implemented`、Change history 1行）のみでproduction code・test・CI workflowへの変更は無い（`git diff --name-status` で確認、Scope/Re-audit行参照）。audited headを `483038d94f` へ更新し、当該head上のmerge gate run 34770025968を新CI証拠とする。head以降にさらにcode変更が入った場合は本auditを無効とし再auditを要する（docs-only commitのみ監査有効性を維持）。
