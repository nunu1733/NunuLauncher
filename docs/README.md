...
| [assessment/organizer-as-is-ux-data-flow-audit.md](./assessment/organizer-as-is-ux-data-flow-audit.md) | Organizer再設計 (#356) Phase AのAS-IS事実モデル（View / ER / timing / state）は何か | #356 Phase Aの調査更新時。TO-BE decisionは #361 が正本 |
| [product/organizer-to-be-ux.md](./product/organizer-to-be-ux.md) | Organizer再設計のTO-BE情報設計・遷移・data timingは何か | TO-BE decision (#361) の改訂時 |
| [product/organizer-disposition-migration.md](./product/organizer-disposition-migration.md) | 既存Issue/specの処分（Continue/Amend/Defer/Retire）と実装migrationの順序は何か | 処分・依存順の変更時 (#362)。実装backlogは #365〜#377 |
