...
| FR-016 | Later | spec accepted ([spec 182](../../specs/182-layout-strategy-catalog/spec.md), [ADR-0012](../adr/0012-versioned-layout-strategy-catalog.md)); `GLOBAL_COMPACT_V2` は [spec 237](../../specs/237-global-compact-v2-folder-relocation/spec.md) で、widget移動対応の `STABLE_PAGE_TIDY_V2`/`BOTTOM_FIRST_V2` は [spec 235](../../specs/235-widget-strategy-placement/spec.md) で accepted | ユーザーがversion付きの組み込みlayout strategyを選択でき、有効strategyのidentityと結果 (移動件数、新規folder/page、strategy固定item、警告) を確認前にpreviewできる。選択はversion付きで検証され、unsupported/破損/newer選択はfail-closedする。 |
