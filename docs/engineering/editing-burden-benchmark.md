...
**共通開始条件**: 各課題の開始時、workspaceは **ページ0（1ページ目）を表示している**。これはベンチマークの共通setup/preconditionであり、課題開始前のagent手順（§7）で整える（seeding契約が検証するのは§5の配置であり、表示ページはsetupの対象）。例外はB5で、開始時の表示ページは直前の誤操作（B2/B4の1回）の結果として§3.1の視点移動規則から決まる（移動: drop先ページ、削除: 操作したページ）。B6は10個のinstall完了後にページ0へ戻すこともsetup（agent手順。課題コストに含めない）とする。
