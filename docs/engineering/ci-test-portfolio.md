...
| `organizer-instrumentation-shared-writer-tests` | shared-writer seam の coordinator/transaction/reload 回帰。Issue #113 の `MODEL_WRITER` self-deadlock regression (`ModelWriterTransactionReentryTest`)、lease admission/FIFO (`LayoutWriteCoordinatorTest`)、Binder future (`BinderOperationFutureTest`)、Issue #117 の whole-unit nested `SQLiteTransaction` contract (`NestedTransactionTest`)、Issue #119 の決定的なA→B reload supersession/stale completion/cancellation (`OrganizerReloadSupersessionTest`) を API 36 emulator で常設実行する | source-changing PR | PR #114 レビューで追加し、Issue #117/#119 でtransaction/reload回帰を同じAPI 36 laneへ追加。coordinator/transaction各caseはisolated fixture DBを使い、reload caseは実Launcher modelを使う。lane全体にはUI laneのapp stateとは独立したclean emulatorを与える。 |
