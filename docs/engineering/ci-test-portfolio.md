...
| `organizer-instrumentation-shared-writer-tests` | shared-writer seam の coordinator 回帰。Issue #113 の `MODEL_WRITER` self-deadlock regression (`ModelWriterTransactionReentryTest`)、lease admission/FIFO (`LayoutWriteCoordinatorTest`)、Binder future (`BinderOperationFutureTest`)、Issue #117 の whole-unit nested `SQLiteTransaction` contract (`NestedTransactionTest`) を API 36 emulator で常設実行する | source-changing PR | PR #114 レビューで追加。Issue #117 により NestedTransactionTest をこの clean API 36 laneへ追加し、SAVEPOINT isolation の誤期待を防ぐ。isolated fixture DB のみを使い、UI lane の app state とは独立した clean emulator を与える。 |
