...
- testの新規作成・変更・review・監査、またはCI test routingの変更では [test-audit skill](./.agents/skills/test-audit/SKILL.md) を適用し、protected contract・credible regression・primary owner boundary・既存coverageとの重複・impact surface・CI分類を先に確定する。skillは手順であり、正本はquality strategy / CI portfolio / workflowとする。
